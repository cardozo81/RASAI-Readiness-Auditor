...
def test_all_configuration_rows_render_group_headers_and_no_technical_names() -> None:
    context_ui.install()
    ...
    assert "RASAI_SERP_TIMEOUT_SECONDS" not in rendered
    assert "RASAI_GSC_ENABLED" not in rendered
    assert "RASAI_CRUX_API_KEY" not in rendered

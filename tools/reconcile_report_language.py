...
def patch_docs() -> None:
    ...
    text = re.sub(r'\*\*Status:\*\* APPROVED BASELINE[^\n]*', '**Status:** BASELINE VIGENTE — capacidades integradas e documentação reconciliada com `main`.', text, count=1) if '**Status:** APPROVED BASELINE' in text else text
...
def patch_presentation() -> None:
    names = [
        ...
        'report_consistency_v2.py', 'report_navigation.py', 'report_semantics.py',
        'searchgeo_readiness_reporting.py', 'indicator_provenance.py', 'console_help.py',
        'source_quality_report_summary.py',
    ]
    replacements = {

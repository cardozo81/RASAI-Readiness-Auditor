...
def test_ai_usage_is_limited_to_actual_ai_consumers() -> None:
    ...
    assert modes["CAT-05"] == audit_catalog.AI_OPTIONAL
    ...
    for catalog_id in ("CAT-01", "CAT-02", "CAT-04", "CAT-06", "CAT-07"):
...
def test_optional_ai_choice_with_unavailable_provider_is_nonblocking_limitation() -> None:
...
def test_optional_ai_unavailable_registry_readiness_does_not_block_audit(monkeypatch) -> None:
    state = _state(ai_provider="auto")
    workflow.set_selected_catalog_ids(state, ["CAT-03"])
    workflow.set_ai_execution_enabled(state, True)
...
def test_required_ai_can_be_suppressed_for_current_execution_without_removing_cat08(monkeypatch) -> None:
    state = _state(improvement_enabled=True, ai_provider="auto")
    ...
    assert workflow.ai_execution_enabled(state) is False
    assert workflow.plan_status(state)[0] == "APTO COM LIMITAÇÕES"
    with workflow._project_execution_plan(state):
        assert state.ai_provider == "none"
    assert state.ai_provider == "auto"
    assert "CAT-08" in workflow.selected_catalog_ids(state)


def test_deep_analysis_requires_producer_but_unavailable_ai_only_limits_closure(monkeypatch) -> None:
...
def test_catalog_snapshot_is_stable_for_future_report_projection() -> None:
    ...
    assert all(row["catalog_version"] == CATALOG_VERSION for row in rows)
...
def test_cat05_competitive_ai_is_preserved_when_optional_ai_is_enabled(monkeypatch) -> None:
    state = _state(
        search_queries=("seguro",),
        search_ai_competitive=True,
        ai_provider="auto",
    )
    workflow.set_selected_catalog_ids(state, ["CAT-05"])
    workflow.set_ai_execution_enabled(state, True)
    monkeypatch.setattr(plan_module, "ai_provider_readiness", lambda current: (True, "ok"))
...
def test_cat05_competitive_ai_is_suppressed_only_in_execution_when_optional_ai_is_off() -> None:
    state = _state(
        search_queries=("seguro",),
        search_ai_competitive=True,
        ai_provider="auto",

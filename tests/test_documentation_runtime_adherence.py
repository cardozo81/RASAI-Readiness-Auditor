...
def test_report_guide_documents_current_scope_and_multi_url_behavior() -> None:
    ...
    assert "cat-01.html" in text
    assert "fundamentos técnicos e descoberta" in text
    assert "Com uma única URL, o layout permanece simples e linear." in text
    assert "duas ou mais URLs distintas" in text
    assert "capture-context.html" in text
    assert "não devem ser replicados por URL em Mobile, Desktop ou `capture-context.html`" in text

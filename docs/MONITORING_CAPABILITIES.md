...
- `report/observability.html` e `report/quality.html` pertencem às superfícies canônicas estáveis da auditoria;
- `MON-*/report.html`, `manifest.json` e, opcionalmente, `impact.html` pertencem ao artefato derivado de Monitoring;
- `VER-*/report.html` pertence ao artefato de verificação;
- `TIMELINE-*/report.html` pertence ao artefato de linha do tempo;
- a navegação deve preservar ordenação, contexto e estado da página ativa sem sugerir que ausência de dado equivale a ausência da superfície canônica da auditoria.

...
presentation/configuration behavior (navigation, persistence and canonical editors).
...
def preparation_menu(console: ModuleType, state: Any, detailed: Any = None) -> str:
    """Delegate to the canonical catalog workflow."""
...
def _install_environment_router(console: ModuleType) -> None:
    def menu(state: Any):
        ...
        if view == "ai":
            ...
            from rasai import integration_diagnostics_console as integration_diagnostics

            def open_all_configurations(current_state: Any) -> None:
                catalog_menu(console, current_state, view="all", title="TODAS AS CONFIGURAÇÕES")

            integration_diagnostics.integration_menu(console, state, open_all_configurations)
...
def _install_top_level(console: ModuleType) -> None:
    ...
    # console_navigation is the canonical owner of the modern INÍCIO menu. Do not wrap
    # that menu in an in-memory stdout capture: long AUD/RPR/CONS executions must stream
    # live frames directly to the terminal instead of being flushed only at the next
    # input prompt.
    if getattr(console, "_rasai_task_navigation_installed", False):
        console._rasai_top_level_information_architecture = True
        console._rasai_top_level_live_stdout_passthrough = True
        return
    ...
    def menu(state: Any) -> str:
        ...
        def read(prompt=""):
            ...
            if (
                prompt.strip().casefold().startswith("escolha")
                and "INÍCIO\n" in captured
                and "PREPARAR AUDITORIA" not in captured
                and "4. Inteligência Artificial" in captured
                and "7. Sistema / restaurar padrões" in captured
            ):
                # console_navigation already owns the canonical home IA. Do not remap
                # numeric choices here or option 5 would be redirected back to IA.
                flush()
                return original_input(prompt)
            if (
                not rendered

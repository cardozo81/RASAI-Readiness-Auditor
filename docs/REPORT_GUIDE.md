...
`<AUD>/report/`, `<AUD>/report.html` e `<AUD>/cat-09.html` não pertencem ao contrato HTML de uma execução `rasai audit`.

## Estrutura suportada

A projeção audit-owned suportada é `report-catalog/`. Ela preserva:

- visão geral e **Índice de Prontidão Search & IA**;
...
## Referência uniforme dos catálogos e páginas

A definição primária de **todos os CAT-01 a CAT-10 e de todas as páginas transversais** está em [`CATALOG_AND_REPORT_SURFACES.md`](CATALOG_AND_REPORT_SURFACES.md).

Essa referência usa a mesma ficha para cada superfície: identificação, propósito, pergunta respondida, entradas/fonte de verdade, processamento, uso de IA, impacto no **Índice de Prontidão Search & IA / Método de Pontuação de Prontidão**, resultados, estados/limitações, rastreabilidade e aprofundamento técnico.

Os capítulos temáticos deste guia e os documentos especializados complementam essa ficha; eles **não criam nível hierárquico diferente para nenhum catálogo** e não substituem o contrato uniforme.

## Compromisso do relatório de catálogo

O `report-catalog/` é a projeção canônica de uma **AUD individual**. Seu compromisso é transformar a evidência persistida em leitura auditável e navegável, sem criar fatos novos durante a renderização.

Ele deve:

- expor a conclusão executiva de readiness sem confundir Score, Coverage, Confidence e Consolidation;
- apresentar os CATs selecionados com seu estado real, inclusive sem resultado, parcial, indisponível ou não aplicável;
- mostrar métricas, findings, evidências, limitações, remediações e proveniência quando fizerem parte do contrato funcional da superfície;
- preservar navegação entre resumo, catálogo, regra, evidência e remediação para que a conclusão possa ser rastreada à origem;
- identificar uso de IA, provider/modelo, tentativas, tokens/custo e limitações quando a capacidade correspondente tiver sido executada;
- usar rótulos humanos em pt-BR sem perder IDs técnicos necessários à rastreabilidade;
- manter o HTML read-only sobre a fonte persistida.

O relatório de catálogo não promete reproduzir tabelas internas, secrets, payloads sensíveis ou todos os registros brutos do banco. O critério é **completude funcional e auditabilidade**, não replicação integral do `audit.db` em HTML.

O relatório de catálogo também não é longitudinal. Evolução temporal entre AUDs pertence ao `CONS-*` e ao contrato de [`CONSOLIDATED_REPORTING.md`](CONSOLIDATED_REPORTING.md).

## Dados funcionais independentes da projeção HTML

A projeção `report-catalog/` consome, sem reduzir ou substituir, os seguintes dados funcionais persistidos:
...
`sari.html` apresenta dimensões, Cobertura, Confiança, Consolidação, resultado geral e limitações. `methodology.html` apresenta a versão pública, o contrato técnico efetivamente usado, pesos, grupos, critérios críticos, rastreabilidade e interpretação do resultado geral.
...
O Índice de Prontidão Search & IA é uma metodologia proprietária, vinculada a evidências e reprodutível. Não representa nota oficial de Google, Bing ou OpenAI, nem probabilidade de ranking/citação ou certificação GEO/AEO.

## Propriedade analítica das páginas

A lista de superfícies audit-owned é definida por `src/rasai/catalog_report_contract.py`. O contrato atual é:

| Domínio | Página canônica | Natureza |
|---|---|---|
| visão executiva | `index.html` | síntese de dados persistidos e Matriz de encerramento estrutural |
| Índice de Prontidão Search & IA / dimensões | `sari.html` | prontidão proprietária |
| fundamentos técnicos e descoberta | `cat-01.html` | descoberta, indexabilidade, padrões e controles técnicos |
| acessibilidade | `cat-02.html` | diagnóstico automatizado; não certificação WCAG |
| conteúdo, semântica e dados estruturados | `cat-03.html` | conteúdo, contexto semântico/editorial e JSON-LD |
| Web Performance | `cat-04.html` | Lighthouse, CrUX e proveniência das fontes aplicáveis |
| Search & AI Intelligence | `cat-05.html` | SERP, GSC, observabilidade e visibilidade em IA quando disponíveis |
| Apdex de navegação | `cat-06.html` | navegação sintética |
| Apdex de experiência | `cat-07.html` | experiência sintética calibrável; não RUM |
| análise profunda e melhorias | `cat-08.html` | análise advisory/evidence-bound |
...
`robots.txt`, sitemaps, `llms.txt` e demais fatos globais pertencem a `ORIGIN`. Eles são apresentados detalhadamente em **Domínio e descoberta** e não devem ser replicados por URL em Mobile, Desktop ou `capture-context.html`.
...
`capture-context.html` existe para mostrar essa topologia, comparar documentos recebidos por Mobile/Desktop e apresentar erros de runtime por snapshot. Ele não recalcula score e não realiza nova chamada de rede ou IA.
...
`cat-01.html` concentra robots/crawler policy, sitemaps, feeds, `llms.txt` experimental e evidências correlatas no escopo `ORIGIN`. Quando artifacts textuais foram efetivamente capturados, o relatório pode exibir uma pré-visualização read-only sem fazer nova requisição de rede.
...
`cat-02.html` apresenta automação/evidência disponível ou estado explícito de indisponibilidade/desabilitação. Lighthouse accessibility não equivale a certificação WCAG integral.

A página não altera `SCORE-GEO-004`.

## Web Performance

`cat-04.html` mantém Lighthouse lab e CrUX field separados. A página é materializada mesmo quando a coleta externa está desabilitada, justamente para deixar esse estado inequívoco.
...
Os serviços e métricas complementares de padrões são projetados principalmente em `cat-01.html` e `metrics.html`. Pode consolidar W3C Nu HTML Checker, W3C CSS Validator, MDN HTTP Observatory, Web Platform Baseline/WebDX e métricas derivadas quando cada capacidade estiver disponível.
...
`cat-08.html` é audit-owned e sempre deve existir após uma auditoria bem-sucedida. Quando a análise profunda não foi solicitada, a página registra explicitamente esse estado sem disparar IA adicional apenas para produzir HTML.
...
`cat-09.html` é audit-owned e sempre deve existir após uma auditoria bem-sucedida. Quando remediação por IA estiver desabilitada ou indisponível, o relatório registra esse estado sem fabricar uma sugestão.
...
`cat-09.html` preserva a diferença entre evidence, finding/conclusão, selector/local quando confiável e receita de correção. Selector não deve ser inventado para findings document/set-level.

Aplicar uma recomendação exige nova auditoria para medir novo estado; o report não muta a página alvo.

## Uso de IA

`ai-integrations.html` é audit-owned e sempre deve existir. Quando não houve IA, a página representa esse estado. Quando houve chamadas, deve preservar provider/modelo, tentativa/status, tokens, reasoning configurado quando disponível e custo estimado.
...
A visibilidade generativa observada, quando persistida, é projetada no `cat-05.html` e permanece separada de readiness:
...
Observed Generative Visibility
= resultados observados/importados sob fonte e protocolo declarados
```
...
Dados externos pós-auditoria aplicáveis são projetados no `cat-05.html` e em `metrics.html`, a partir do sidecar `RASAI-OBS-002`. O sidecar atual é `RASAI-OBS-002`.
...
## Qualidade e suporte à decisão

Qualidade da evidência, estados de fulfillment e controles estruturais são projetados nas superfícies de governança, especialmente `execution-evidence.html`, `index.html` e `metrics.html`, conforme o dado disponível. Não existe `quality.html` no contrato audit-owned atual.
...
A navegação é derivada diretamente de `CATALOG_REPORT_PAGES`: Visão geral, SARI, CAT-01 a CAT-10, Análise Direcionada e as páginas de governança Captura e contexto, Evidências da execução, IA e integrações, Metodologia e scoring e Índices e métricas.
...
HTML nunca se torna segunda fonte de verdade para score, evidence, resultados, tokens ou custos. Um placeholder `SEM DADOS` é apenas estado de apresentação e nunca cria evidência persistida.

## Semântica visual e linguagem pública

Enums persistidos não são alterados no banco. A camada de apresentação usa nomes de negócio em pt-BR e pode traduzir valores de máquina conhecidos sem modificar identificadores necessários à rastreabilidade, como `BR-GEO-*`, IDs de auditoria/evidência, providers/modelos, perfis sintéticos e variáveis de ambiente. IDs técnicos aparecem como informação secundária, não como definição principal do conceito.

...
def _align_standards_surface(html: str) -> str:
    ...
    controls = (
        ...
        "function setServiceMode(field,mode){if(!field)return;try{const cfg=auditConfigObject();if(field==='gsc_enabled'&&mode==='true'&&!String(cfg.gsc_site_url||'').trim()){toast('Para ligar GSC explicitamente, informe a property na configuração guiada.','bad');return}if(mode==='default')delete cfg[field];else cfg[field]=mode==='true';$('audit-config').value=JSON.stringify(cfg,null,2);renderStandards()}catch(err){toast(err.message,'bad')}}\n"

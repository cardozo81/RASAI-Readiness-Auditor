...
- mensagens, estados, unidades operacionais, ações e explicações destinadas ao usuário: pt-BR;
- nomes conceituais consolidados como `Evidence & Trust`, `Intent Coverage`, `Structured Data`, `Core Web Vitals`, `Lighthouse` e `Apdex`: preservados quando essa é a nomenclatura pública vigente;
- IDs, contratos e valores técnicos podem permanecer canônicos em `<code>`.

O CONS normaliza vazamentos conhecidos de inglês operacional, sem traduzir a taxonomia conceitual do produto.
...
- tradução de inglês operacional sem traduzir conceitos públicos como `Evidence & Trust`.

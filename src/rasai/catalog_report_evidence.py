...
def _runtime_type_label(value: Any) -> str:
    ...
    }.get(_norm(value),public_label(value) or str(value or "Diagnóstico").replace("_"," ").title())
...
def _runtime_diagnostics_html(database: Path, data: _ReportData) -> str:
    ...
    for snap in _capture_snapshots(database,data.audit_id):
        for item in _runtime_items(snap):
            ...
            raw_type=_norm(item.get("type"))
            label_display=_state_text("WARNING" if raw_type=="CONSOLE_WARNING" else "ERROR",label)
            url=item.get("url") or "-"; message=item.get("message") or "-"
            rows.append((label_display,message,url,_modal_button(mid,"Ver ocorrência")))
...
def _discovery_html(database: Path, data: _ReportData) -> str:
    ...
    for i,row in enumerate(relevant,1):
        ...
        category={"ROBOTS":"robots.txt","SITEMAP":"Sitemap / feed","AI_ACCESS":"Arquivo para agentes de IA","DISCOVERY":"Mecanismo de descoberta"}.get(_norm(row.get("category")),public_label(row.get("category")) or str(row.get("category") or "Recurso").replace("_"," ").title())
        title=_discovery_title(row,observed)
        semantic_state=state or row.get("severity")
        title_display=_state_text(semantic_state,title) if semantic_state else title
        rows.append((category,title_display,row.get("scope_url") or "-",_modal_button(mid,"Ver leitura")))
...
def _lighthouse_accessibility_html(database: Path, data: _ReportData) -> str:
    ...
    if not isinstance(payload,Mapping):
        if obs.get("accessibility_score") is None:
            return (
                "<div class='notice warn'><strong>Sem resultado automatizado de acessibilidade:</strong> "
                "a tentativa PageSpeed/Lighthouse não materializou pontuação nem artefato detalhado de acessibilidade "
                "neste contexto. Dados CrUX de performance, quando disponíveis, não substituem uma medição de acessibilidade.</div>"
            )
        return "<div class='notice'>A pontuação de acessibilidade está persistida, mas o artefato detalhado do Lighthouse não está disponível para esta projeção.</div>"
    ...
    for aid in ids:
        ...
        attention=_result_value("Requer atenção","bad")
        rows.append((title,attention,selector,_modal_button(mid,"Ver evidência")))
        body=_kv((("Verificação",title),("Resultado",attention),("Descrição da fonte",a.get("description") or source_title or "-"),("Elemento",selector),("Trecho",snippet or "-"),("Identificador técnico",aid)))
...
def _crux_provenance_html(database: Path, data: _ReportData) -> str:
    """Expose scope/period/device provenance for current and historical CrUX data."""
    rows: list[tuple[Any, ...]] = []
    obs=_web_observation(database,data.audit_id)
    ...
    def date_parts(value: Any) -> str:
        if isinstance(value,Mapping):
            try:
                return f"{int(value.get('day')):02d}/{int(value.get('month')):02d}/{int(value.get('year')):04d}"
    ...
    artifact=_read_json_artifact(database.parent,obs.get("crux_artifact_reference"))
    if isinstance(artifact,Mapping):
        record=artifact.get("record") if isinstance(artifact.get("record"),Mapping) else {}
        key=record.get("key") if isinstance(record,Mapping) and isinstance(record.get("key"),Mapping) else {}
        period=record.get("collectionPeriod") if isinstance(record,Mapping) and isinstance(record.get("collectionPeriod"),Mapping) else {}
        first=date_parts(period.get("firstDate")) if isinstance(period,Mapping) else "-"
        last=date_parts(period.get("lastDate")) if isinstance(period,Mapping) else "-"
        target=key.get("url") or key.get("origin") or obs.get("url") or "-"
        rows.append((
            "CrUX API · medição atual",
            scope_label(obs.get("field_scope")),
            device_label(key.get("formFactor")),
            f"{first} a {last}" if first!="-" or last!="-" else "-",
            target,
        ))
                    ...
                    history=[
                        ...
                        for row in connection.execute(
                            """SELECT period_start,period_end,metadata
                               FROM datasets
                               WHERE source_type='CHROME_UX_REPORT_HISTORY'
                               ORDER BY collected_at,dataset_id"""
                        ).fetchall()
                    ]
                    for item in history:
                        ...
                        rows.append((
                            "CrUX History",
                            scope_label(metadata.get("target_scope") or "ORIGIN"),
                            device_label(metadata.get("form_factor")),
                            f"{date_parts(item.get('period_start'))} a {date_parts(item.get('period_end'))}",
    ...
    if not rows:
        return ""
    return (
        "<div class='subsection'><h3>Proveniência dos dados CrUX</h3>"
        "<p class='section-lead'>Escopo, dispositivo e período são exibidos explicitamente para evitar interpretar "
        "uma medição da URL atual como equivalente a uma série histórica de origem.</p>"
        +_table(("Fonte","Escopo","Dispositivo","Período observado","Alvo"),rows)
        +"</div>"
    )
...
def _structured_data_html(database: Path, data: _ReportData) -> str:
    ...
    for i,r in enumerate(rules,1):
        ...
        result_text=_status_label(r.get("result"))
        result_display=_state_text(r.get("result"),result_text)
        rows.append((_STRUCTURED_RULE_LABELS.get(rid,rid),result_display,summary,_modal_button(mid,"Ver validação")))
        body=_kv((("Validação",_STRUCTURED_RULE_LABELS.get(rid,rid)),("Resultado",result_display),("Condição esperada",r.get("expected_condition") or "-"),("Identificador",rid)))
...
def _semantic_html(database: Path, data: _ReportData) -> str:
    ...
    if entities:
        ...
        rows=[(r.get("name"),entity_labels.get(_norm(r.get("entity_type")),public_label(r.get("entity_type")) or str(r.get("entity_type") or "-").replace("_"," ").title()),_confidence_label(r.get("confidence"))) for r in entities]

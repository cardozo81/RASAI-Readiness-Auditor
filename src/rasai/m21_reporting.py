...
def _performance_page(data: dict[str, Any], report_dir: Path) -> str:
    ...
    if not enabled:
        notice = f"<div class='notice'><strong>Coleta externa desabilitada.</strong> Esse é o comportamento padrão para evitar tráfego/quota externa não solicitado. {SCORING_VERSION} continua disponível normalmente.</div>"
    elif status in {"PARTIAL", "UNAVAILABLE"}:
        notice = f"<div class='notice warn'><strong>Coleta externa incompleta.</strong> Falha, quota, ausência de amostra CrUX ou indisponibilidade do serviço não é defeito do website e não reduz {SCORING_VERSION}.</div>"
    else:

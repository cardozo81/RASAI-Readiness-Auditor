...
def _install_saas_contract() -> None:
    ...
    if not bool(getattr(current_normalize, "_rasai_search_audit", False)):
        def normalize_audit_job_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
            ...
            # Provider availability is an execution outcome, not a payload-validity
            # gate. The competitive AI runtime persists NOT_CONFIGURED/UNAVAILABLE.
    ...
    try:
        ...
        if policy.use_ai is False:
            return {
                ...
                "requested": False,
                "reason": "AI_DISABLED_FOR_THIS_RPR",
            }
        if policy.use_ai is True:
            override_provider, override_model, _override_reasoning = provider_override()
            provider_name = str(override_provider or "none").casefold()
            model = override_model if provider_name not in {"auto", "none"} else None

    ...
    if not getattr(latest, "_rasai_contextual_core_filter", False):
        def contextual_latest(workspace: AuditWorkspace, audit_id: str):
            ...
            if current is None or current.audit_id != audit_id:
                return items
            # Core items were already evaluated by the outer wrapper. Downstream AI
            # is not globally blocked: audit_reprocess applies the dependency graph
            # to each AI item independently.
            return tuple(
                item for item in items if item.component not in core.CORE_COMPONENTS
            )
    ...
    def reprocess_with_core_context(
        ...
        *,
        ...
        unresolved_initial = tuple(
            ...
            if item.required and item.status not in core._RESOLVED_STATES
        )
        selected_count, unselected_count = core.selected_counts(unresolved_initial)

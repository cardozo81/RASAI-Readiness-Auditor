...
from rasai.console_ui import CYAN, DIM, GRAY, GREEN, RED, YELLOW, paint
...
def _render_dependencies(state: Any, spec: diagnostics.IntegrationSpec) -> None:
    from rasai.console_configuration_presentation import friendly_label
    from rasai.console_ui_catalog import origin_for
    ...
    for index, item in enumerate(spec.dependencies, 1):
        ...
        required = "OBRIGATÓRIA" if item.required else "OPCIONAL"
        ...
        env_spec = environment_console.SPEC_BY_NAME.get(item.name)
        label = friendly_label(env_spec) if env_spec is not None else item.purpose
        origin = origin_for(state, env_spec) if env_spec is not None else ("SESSÃO" if (os.environ.get(item.name) or "").strip() else "PADRÃO/AUSENTE")
        print(f" {index:2d}. {label}")
        print(paint(f"     Variável técnica: {item.name}", GRAY))
        print(f"     Valor: {paint(value, value_color)} | Origem: {origin} | [{required}{secret}]")
        print(paint(f"     {item.purpose}", DIM))
    if spec.related_envs:
        print("\nPARÂMETROS RELACIONADOS À EXECUÇÃO")
        for name in spec.related_envs:
            env_spec = environment_console.SPEC_BY_NAME.get(name)
            label = friendly_label(env_spec) if env_spec is not None else name
...
def _confirm_individual_test(spec: diagnostics.IntegrationSpec) -> bool:
    print("\nCONFIRMAR TESTE")
    print(f"Integração : {spec.label}")
    print(f"Categoria  : {spec.category}")
    print(f"Probe      : {spec.probe_cost}")
    if spec.probe_cost == diagnostics.PROBE_NO_GENERATION:
        print("Consumo    : não gera tokens de IA")
    elif spec.probe_cost == diagnostics.PROBE_SCARCE_QUOTA_AVOIDED:
        print("Consumo    : evita a operação de quota escassa")
    else:
        print("Consumo    : pode consumir quota técnica mínima do fornecedor")
    print("Escopo     : diagnóstico isolado; não altera auditoria, scoring ou configuração")
...
def _detail_menu(console_module: ModuleType, state: Any, spec: diagnostics.IntegrationSpec) -> None:
    while True:
        ...
        _render_dependencies(state, spec)
        ...
        if choice == "V":
            return
        if choice == "T":
            if _confirm_individual_test(spec):
                _run_and_store(state, spec)
            continue
        if choice == "A":

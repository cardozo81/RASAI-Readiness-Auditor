...
M24 historically collected deterministic/network diagnostics and invoked technical AI
inside one function.  The audit pipeline now executes M24 collection with
``technical_ai=False`` before evidence sealing, then calls this module after sealing.
This preserves the existing M24 provider/fallback/attempt persistence while enforcing
collection-before-AI causality.
"""
    ...
    try:
        ...
        return tuple(
            M24Diagnostic(
                ...
                evidence_ids=tuple(str(item) for item in _decode(row["evidence_ids"], []) if str(item).strip()),
...
def _update_run(
    ...
    *,
    ...
    try:
        with connection:
            connection.execute(
                """UPDATE m24_runs SET ai_enabled=1,ai_state=?,scoring_impact=?,updated_at=datetime('now')
                   WHERE audit_id=?""",
...
def execute_m24_ai_phase(
    *,
    ...
    """Run only the optional technical-AI consumer against already persisted M24 facts."""
    ...
    round_id = begin_round(
        workspace=workspace,
        ai_task_id=task_id,
        requested_requirements=_REQUIREMENTS,
        input_payload={
            "evidence_snapshot_id": snapshot.evidence_snapshot_id,
    ...
    try:
        ...
        complete_round(
            ...
            accepted={},
            rejected={"PROVIDER": {"error_type": type(exc).__name__, "message": str(exc)[:512]}},
    ...
    assessments = []
    if result.explanation and isinstance(result.explanation.get("resource_assessments"), list):
        assessments = [item for item in result.explanation["resource_assessments"] if isinstance(item, dict)]
    accepted = {
        f"RESOURCE:{str(item.get('resource') or '').upper()}": dict(item)
        for item in assessments
        if str(item.get("resource") or "").upper() in {"SITEMAP", "ROBOTS"}
    }
    missing = tuple(item for item in _REQUIREMENTS if item not in accepted)
    complete_round(
        workspace=workspace,
        ai_round_id=round_id,
        accepted=accepted,
        rejected={},
        missing=missing,
        output_payload=result.explanation or {"state": result.state.value, "reason": result.reason},

...
- technical environment-variable names remain available only through explicit technical
  details and are not used as the normal operator label.
...
def _grouped_context_rows(state: Any, specs: tuple[Any, ...]) -> Iterator[None]:
    ...
    def grouped_print(*args: Any, **kwargs: Any) -> None:
        ...
        if len(args) == 1 and isinstance(args[0], str):
            ...
            if plain in _LEGACY_SEARCH_HEADINGS:
                return

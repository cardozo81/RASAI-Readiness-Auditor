...
def test_public_branding_contract() -> None:
    ...
    assert LEGACY_READINESS_REPORT == "rasai.html"
...
def test_primary_and_legacy_cli_entrypoints_coexist() -> None:
    ...
    assert scripts["rasai"] == scripts["rasai"] == "rasai.entrypoint:main"
    assert scripts["rasai-console"] == scripts["rasai-console"] == "rasai.console_entrypoint:main"


def test_public_documentation_and_legacy_document_coexist() -> None:
    assert Path("docs/SARI_READINESS_INDEX.md").is_file()
    assert Path("docs/RASAI_READINESS_INDEX.md").is_file()

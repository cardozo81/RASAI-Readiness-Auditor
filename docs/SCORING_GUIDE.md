# Guia do Método de Pontuação de Prontidão

**Versão pública:** **001**  
**Contrato técnico:** `SCORE-GEO-004`

Guia operacional do **Search & AI Readiness Index - Índice de Prontidão Search & IA**, cujo identificador metodológico técnico é `SARI-001`.

Siglas, IDs e taxonomia: [`GLOSSARY.md`](GLOSSARY.md).
...
```text
PASS    = 1.00
WARNING = warning_factor da regra (0,50 por padrão)
FAIL    = 0.00
```

O fator de `WARNING` é versionado por regra e pode ser diferente de 0,50. `UNKNOWN`/`ERROR` não recebem fator numérico, permanecem no universo aplicável e reduzem Coverage; não viram FAIL.
...
Quando não há bloqueio crítico, a aritmética vigente é:
...
Weighted Confidence =
  sum(Dimension Weight x ConfidenceValue)
  / sum(Dimension Weight aplicável)
...
Os **Critérios Críticos de Prontidão** não truncam o Índice de Prontidão Search & IA. O relatório pode mostrar, por exemplo:
...
```text
report-catalog/sari.html         SARI, dimensões, macrocomponentes, Coverage/Confidence e Critical Gates
report-catalog/methodology.html   fórmula, pesos, grupos, RuleExecutions representativas, contribuições e limites metodológicos
páginas CAT aplicáveis            referências técnicas específicas dos indicadores e evidências do respectivo domínio
```

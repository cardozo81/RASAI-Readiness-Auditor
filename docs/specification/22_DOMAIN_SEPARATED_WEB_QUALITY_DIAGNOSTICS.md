...
**Dependências:** Web Performance externo + `CATALOG-REPORT-002`  
...
`cat-02.html` e `cat-04.html`, assim como as demais superfícies definidas por `CATALOG-REPORT-002`, permanecem no menu canônico conforme o contrato atual.

Ordem e rótulos são produzidos pelo contrato compartilhado de `src/rasai/catalog_report_contract.py`, não por cada página isoladamente. Estados sem dados devem ser exibidos de forma neutra e não podem ser confundidos com falha do website.
...
1. `cat-02.html` pode projetar evidência existente sem nova chamada externa;
2. `cat-02.html` permanece no menu mesmo quando não houver coleta e apresenta estado neutro;
...
14. `cat-04.html` permanece presente mesmo quando a integração não for executada;

"""Projeção HTML do Synthetic Navigation Apdex no padrão visual canônico do RASAi."""
...
from rasai.apdex_concurrency_policy import navigation_risk
...
def _page(data: dict[str, Any], report_dir: Path) -> str:
    ...
    notice = _status_notice(enabled, status, str(run["reason"] or "") if run is not None else None)
    concurrency_value = int(run["concurrency"]) if run is not None else 0
    concurrency_risk = navigation_risk(concurrency_value) if concurrency_value else "-"
    metrics = (
        _metric("Estado Synthetic Navigation Apdex", status)
        ...
        + _metric("Concorrência", concurrency_value)
        + _metric("Risco de concorrência", concurrency_risk)
    )
...
def _index_summary(data: dict[str, Any]) -> str:
    ...
    summaries = data["summaries"]
    if run is None:
        state, detail = "NÃO MATERIALIZADO", "Synthetic Navigation Apdex ainda não possui estado persistido."
...
def _references_section() -> str:
    ...
    return f"""{_START_REFS}<section id='m23-apdex-methodology' class='panel'><div class='kicker'>Synthetic Navigation Apdex · metodologia</div><h2>Apdex sintético e rastreabilidade Lighthouse</h2><p class='intro'>As fontes abaixo sustentam somente o domínio Web Performance. Não homologam o Método de Pontuação de Prontidão. Contrato técnico: SCORE-GEO-004.</p><div class='table-wrap'><table><thead><tr><th>Fonte</th><th>Base</th><th>Uso</th><th>Referência</th></tr></thead><tbody>{rows}</tbody></table></div></section>{_END_REFS}"""


def _status_notice(enabled: bool, status: str, reason: str | None = None) -> str:
    if not enabled:

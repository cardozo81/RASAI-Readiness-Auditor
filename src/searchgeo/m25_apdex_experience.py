...
from searchgeo.m25_dynatrace import SUPPORTED_TIME_KPMS, load_dynatrace_calibration
from searchgeo.m25_persistence import M25Persistence, SyntheticUxRun, SyntheticUxSample, SyntheticUxSummary
from searchgeo.operational_log import try_append_operational_event
from searchgeo.persistence import AuditWorkspace
from searchgeo.rendering import BrowserProfile
...
TABLET_STANDARD_PROFILE = SyntheticProfile(
    profile_id="SEARCHGEO_TABLET_CONTROLLED4G_V1",
    device=DeviceContext.MOBILE,
        ...
        *,
        ...
        try:
            ...
            page.add_init_script(
                """
                (() => {
                  window.__searchgeoUx = {lcp: null, cls: 0};
                  try {
                    new PerformanceObserver((list) => {
                      const entries = list.getEntries();
                      if (entries.length) window.__searchgeoUx.lcp = entries[entries.length - 1].startTime;
                    }).observe({type: 'largest-contentful-paint', buffered: true});
                  } catch (_) {}
                  try {
                    new PerformanceObserver((list) => {
                      for (const e of list.getEntries()) if (!e.hadRecentInput) window.__searchgeoUx.cls += e.value || 0;
            ...
            try:
                ...
                visual = page.evaluate("() => window.__searchgeoUx || {}") or {}
...
def _measure_device(
    *,
    ...
    try:
        with ThreadPoolExecutor(max_workers=config.concurrency, thread_name_prefix="searchgeo-ux") as executor:

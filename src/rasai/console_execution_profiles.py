...
MODULES: tuple[ExecutionModule, ...] = (
    ...
    ExecutionModule(
        ...
        (),
        "Pode gerar chamadas adicionais de IA reutilizando provider/model/reasoning da IA principal quando disponíveis.",
        "Exige item 13 habilitado e URL única. IA principal indisponível não bloqueia a parte determinística; o fechamento da análise profunda permanece pendente.",
    ),
)
...
def _choose_ai_mode(state: Any) -> str | None:
    ...
    print("   Observação: Improvement Intelligence usa a IA principal quando disponível; indisponibilidade não bloqueia a parte determinística.")
    print("V. Voltar")
...
def _custom_modules(state: Any) -> tuple[str, ...] | None:
    ...
    while True:
        ...
        print("\nDigite o número para marcar/desmarcar; C=confirmar seleção; V=voltar")
        raw = input("Escolha: ").strip().upper()
        if raw == "V":
            return None
        if raw == "C":

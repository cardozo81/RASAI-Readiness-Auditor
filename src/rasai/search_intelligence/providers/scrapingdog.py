"""ScrapingDog Google Search adapter with normalized RASAi observations."""
...
class ScrapingDogProvider(SerpProvider):
    ...
    def __init__(self, *, api_key: str, timeout_seconds: float = 20.0, retries: int = 1,
        ...
        if not api_key.strip():
            raise ValueError("ScrapingDog live mode requires RASAI_SCRAPINGDOG_API_KEY")
        if timeout_seconds <= 0 or retries < 0 or min_interval_seconds < 0:
            raise ValueError("invalid ScrapingDog timeout/retry/rate configuration")
    ...
    def _request(self, request: SerpQueryRequest, *, page: int) -> bytes:
        ...
        params: dict[str, str | int] = {
            ...
            "results": _PAGE_SIZE,
            "page": page,
            "country": request.country.strip().casefold(),
            "language": request.language.split("-", 1)[0].strip().casefold(),
            "mob_search": "true" if device == "mobile" else "false",
        }
        ...
        for attempt in range(self._retries + 1):
            ...
            http_request = Request(url, method="GET", headers={
                "Accept": "application/json", "User-Agent": "RASAi/0.1 SERP adapter",
            })
            try:
                ...
                if exc.code != 429 and not (500 <= exc.code <= 599):
                    raise SerpProviderError(f"ScrapingDog HTTP {exc.code}") from exc
            except (TimeoutError, socket.timeout, URLError) as exc:
                last_error = exc
            if attempt < self._retries:
                continue
        if isinstance(last_error, (TimeoutError, socket.timeout)):
            raise SerpProviderTimeout("ScrapingDog request timed out") from last_error
        raise SerpProviderUnavailable("ScrapingDog request unavailable") from last_error

    def _decode(self, raw: bytes) -> dict[str, Any]:
        try:
            payload = json.loads(raw.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise SerpMalformedResponse("ScrapingDog returned invalid JSON") from exc
        if not isinstance(payload, dict):
            raise SerpMalformedResponse("ScrapingDog response root must be an object")
        if payload.get("error"):
            message = str(payload.get("error"))
            raise SerpProviderError("ScrapingDog provider error: " + message[:240])
        return payload
    ...
    def _normalize(payload: dict[str, Any], *, offset: int) -> tuple[SerpResult, ...]:
        ...
        if not isinstance(organic, list):
            raise SerpMalformedResponse("ScrapingDog organic_results must be an array")
        ...
        for index, item in enumerate(organic, 1):
            ...
            if domain is None:
                continue
            try:
                local_position = int(item.get("rank", item.get("position", index)))
            ...
            output.append(SerpResult(
                ...
                result_type="organic", serp_features=(), metadata=metadata,
            ).validate())
    ...
    def observe(self, request: SerpQueryRequest) -> ProviderObservation:
        ...
        ended = False
        page_count = len(tuple(range(0, request.depth, _PAGE_SIZE)))
        for page in range(page_count):
            payload = self._decode(self._request(request, page=page))
            pages.append(payload)
            page_results = self._normalize(payload, offset=page * _PAGE_SIZE)
        ...
        observation = SerpObservation(
            ...
            config_metadata=dict(request.config_metadata), quality_metadata={
                ...
                "pages_requested_ceiling": page_count,
                "pages_collected": len(pages),
                "pagination_ended_before_requested_depth": ended,
                "credit_cost_per_request": 10 if request.device.strip().casefold() == "mobile" else 5,
            },
        )

...
def install() -> None:
    ...
    def run_audit_from_console(state: Any) -> int:
        ...
        try:
            ...
            argv = runtime.build_command(state)
        ...
        if workspace:
            try:
                from rasai.execution_commands import audit_plan, executed, record

                command_plan = executed(audit_plan(state, env=child_env, argv=argv))
                state._rasai_last_execution_command_plan = command_plan
                execution_id = state.audit_id or workspace.name
                state._rasai_last_execution_command_log = record(
                    command_plan,
                    state.audits_root,
                    execution_id,
                    append=False,
                ...
                # Command projection is observability only and must never affect the AUD.
                pass
        if cancelled:
            runtime.render_header(state)
            if state.audit_id:

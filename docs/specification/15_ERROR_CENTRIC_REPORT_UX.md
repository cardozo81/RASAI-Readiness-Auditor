...
`CATALOG-REPORT-002` define a projeção HTML audit-owned em `report-catalog/`. A fonte de verdade para nomes, ordem e rótulos é `src/rasai/catalog_report_contract.py`.
...
A existência física da página não significa que a capacidade foi executada. Quando uma capacidade não foi solicitada, não está configurada, não se aplica ou não possui dados, a projeção apresenta o estado persistido correspondente. O renderer não chama API, provider, coletor ou IA apenas para preencher HTML.

A versão do scoring pertence a `scoring_version` e ao conteúdo de `methodology.html`, não ao nome físico da página.

`<AUD-ID>/report/`, `<AUD-ID>/report.html` e páginas convencionais fora de `report-catalog/` não integram o contrato audit-owned de `rasai audit`.
...
O menu deve ser produzido/normalizado pelo componente comum de navegação e refletir a ordem canônica de `src/rasai/catalog_report_contract.py`. Como as superfícies são estáveis, o menu também é estável.

Exemplo: em uma auditoria somente Mobile, as páginas CAT aplicáveis e as superfícies de governança devem preservar explicitamente o contexto executado. O relatório não cria uma página Desktop paralela nem inventa score, snapshot ou finding para um contexto não executado.
...
O relatório não deve repetir longos blocos de documentação em todas as páginas. Quando o detalhe metodológico já existe em `methodology.html` ou na página CAT responsável, usar explicação curta e link de atalho.
...
## 7. Dispositivo e escopo

Resultados por dispositivo permanecem vinculados às páginas, snapshots, evidências e findings persistidos. No `report-catalog/`, essa informação é projetada pelas páginas CAT responsáveis pelo domínio e pelas superfícies `capture-context.html` e `execution-evidence.html`.

Mobile e Desktop não possuem páginas audit-owned independentes. Um contexto não executado deve permanecer distinguível de um contexto executado, sem score, snapshot ou finding artificial.

## 8. Remediação

`report-catalog/cat-09.html` é a visão orientada a problema/causa.
...
Quando existirem sugestões/remediação de conteúdo por IA para a mesma auditoria, CAT-03 apresenta o contexto de conteúdo/semântica e CAT-09 concentra a ação de remediação, sem misturar sugestão editorial auxiliar com finding/scoring.

## 9. Conteúdo, contexto editorial e Structured Data

`report-catalog/cat-03.html` é a superfície de conteúdo, semântica e dados estruturados; `report-catalog/cat-09.html` concentra remediações derivadas dos achados e evidências.
...
`ai-integrations.html` deve conter, quando persistido/disponível:
...
`methodology.html` e `metrics.html` devem reunir, de forma complementar:
...
Páginas especializadas podem apresentar referências diretamente relevantes e encaminhar a `methodology.html` para o contrato metodológico e a `metrics.html` para o inventário transversal quando houver material adicional.
...
Toda página final deve informar, de forma compacta e visível, a natureza dos indicadores centrais exibidos. `methodology.html` consolida o contrato metodológico, classificação, fontes e transformações do RASAi; `metrics.html` mantém o inventário transversal dos indicadores persistidos. Links oficiais relevantes podem permanecer também nas páginas CAT responsáveis.

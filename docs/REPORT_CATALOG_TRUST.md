...
O plano congelado da AUD e a configuração efetivamente autorizada em um `RPR-*` são fatos distintos. O relatório preserva o plano original e, quando o reprocessamento executa IA, materializa separadamente o `RPR-*`, provider/modelo e resultado efetivos. O assurance reconcilia `audit_reprocess_runs.configuration.ai_used` com as tentativas `IMPROVEMENT-INTELLIGENCE` persistidas no intervalo daquele RPR.
...
`audit-snapshot.db` é criado com SQLite backup, incorporando estado committed que poderia estar em WAL. Portanto, o pacote entregue não depende do `audit.db-wal` original para reconstrução posterior. Antes da promoção, o gerador calcula um digest lógico canônico do `audit.db` e do snapshot; ambos devem ser idênticos. Essa comparação é independente do layout físico/paginação SQLite.

O manifest registra:

- hash SHA-256 do snapshot SQLite;
- digest lógico SHA-256 do estado fonte e do snapshot standalone;
...
- o snapshot standalone divergir logicamente do estado final do `audit.db`;
- um artifact derivado obrigatório, como `external-metrics-integrity.json`, divergir do estado efetivo de métricas externas após reprocessamento;
...
- integridade: hash da configuração, inventário de fontes, freshness de artifacts derivados, provenance de RPR, igualdade lógica entre fonte final e snapshot, artefatos verificáveis e boundary read-only;
...
## Fechamento diagnóstico versus encerramento estrutural

A Matriz de encerramento estrutural e o fechamento diagnóstico são estados independentes.

Uma falha de IA/API corretamente detectada, persistida e exposta não reduz automaticamente cobertura estrutural, integridade, segurança ou governança. Entretanto, quando aquela integração é necessária para um diagnóstico selecionado, o fechamento diagnóstico permanece pendente.

A página principal projeta um informativo derivado somente do estado atual dos work items obrigatórios. Uma pendência resolvida deixa de aparecer nesse informativo; tentativas e falhas anteriores permanecem na provenance.

A existência de HTML íntegro não promove work item não executado, `NO_DATA`, `NOT_CONFIGURED`, `WAITING_FOR_DATA` ou falha de provider a sucesso.

...
def _restore_menu(console_module: ModuleType, state: Any) -> None:
    ...
    facade.refresh_specs()
    unique = {spec.name: spec for spec in facade.SPECS}
    session_count = sum(1 for name in unique if name in os.environ)
    user_count = (
        sum(1 for name in unique if user_environment_value(name) is not None)
        if os.name == "nt"
        else 0
    )
    machine_count = (
        sum(1 for name in unique if machine_environment_value(name) is not None)
        if os.name == "nt"
        else 0
    )
    destination = settings.resolve_config_path()

    console_module.render_header(state)
    print("SISTEMA > RESTAURAR PADRÕES\n")
    print("ESCOPO ATUAL")
    print(f"Arquivo de configuração : {destination}")
    print(f"Overrides na sessão     : {session_count}")
    if os.name == "nt":
        print(f"Windows/User            : {user_count} variável(is) gerenciada(s)")
        print(f"Windows/Machine         : {machine_count} variável(is) detectada(s)")
    print("\nSERÁ ALTERADO")
    print("- configurações não secretas da sessão")
    print("- configurações não secretas persistidas em Windows/User, quando aplicável")
    print("- rasai-console.ini, reconstruído a partir dos padrões desta versão")
    print("\nSERÁ PRESERVADO")
    print("- Windows/Machine")
    print("- auditorias, relatórios, audit.db e banco do control plane")
    print("- arquivos de projeto")
    print("- credenciais, salvo escolha explícita de remoção abaixo\n")
    ...
    if os.name == "nt":
        ...
        print("2. Restaurar padrões e REMOVER credenciais da sessão")
    ...
    print(f"Arquivo a reconstruir   : {destination}")
    print("Windows/Machine         : PRESERVAR")
    print("Auditorias e relatórios : PRESERVAR")
    if not confirm_sensitive(
        "RESTAURAR",
        action_label="Restaurar os padrões do sistema conforme o escopo acima",
        back_label="Voltar sem restaurar",

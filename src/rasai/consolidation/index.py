...
class ConsolidationIndex:
    def __init__(self, audits_root: str | Path) -> None:
        ...
        self.index_dir = self.audits_root / ".rasai"
    ...
    def candidate_audits(self, filters: ConsolidationFilter) -> tuple[dict[str, Any], ...]:
        ...
        if filters.audit_ids:
            placeholders = ",".join("?" for _ in filters.audit_ids)
            audit_clause = f" AND a.audit_id IN ({placeholders})"
            params.extend(filters.audit_ids)
        with self._session() as connection:
            rows = connection.execute(
                f"SELECT * FROM source_audits a WHERE upper(a.status)='COMPLETED' AND ({where}){device_clause}{url_clause}{audit_clause} ORDER BY event_time,audit_id",
                params,
            ).fetchall()

...
def _install_m7_continuation() -> None:
    ...
    def execute_m7(*args: Any, **kwargs: Any):
        ...
        metadata_token = _RULE_PROVIDER_METADATA.set({})
        try:
            ...
            # Provider metadata is scoped to this execution so a later audit or direct
            # M7 call can never inherit provider/model provenance from an earlier run.
            with governed_execution_context(audit_id=audit_id, workspace=workspace) as snapshot:
                if snapshot is None:
                    return original_execute(*args, **kwargs)
                return original_execute(*args, **kwargs)
        finally:
            _RULE_PROVIDER_METADATA.reset(metadata_token)

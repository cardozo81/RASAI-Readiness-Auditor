...
def test_report_projection_explicitly_marks_preliminary_then_final() -> None:
    with TemporaryDirectory() as directory:
        workspace = _workspace(Path(directory))
        (workspace.root / "report").mkdir(exist_ok=True)
        page = workspace.root / "report" / "index.html"
        page.write_text("<html><body><h1>RASAi</h1></body></html>",encoding="utf-8")
        initialize_contract(workspace,"AUD-TEST")
        register_work_item(
            workspace,audit_id="AUD-TEST",component="CORE_AUDIT",required=True,
            temporal_mode=REPLAY_SAFE,status=SUCCESS,retryable=False,
        ...
        project_report_validity(audit_id="AUD-TEST",workspace=workspace)
        assert "Relatório preliminar" in page.read_text(encoding="utf-8")
        assert '"consolidation_eligible": false' in (workspace.root / "report" / "processing-status.json").read_text(encoding="utf-8")
        ...
        assert "Processamento concluído" in text
        assert "Relatório preliminar" not in text
        assert '"consolidation_eligible": true' in (workspace.root / "report" / "processing-status.json").read_text(encoding="utf-8")

...
def _retryable_core(workspace: AuditWorkspace, audit_id: str) -> tuple[WorkItem, ...]:
    return tuple(
        ...
        if item.retryable and item.status in _RETRYABLE_STATES and not _expired(item)
    )


def _core_unresolved(workspace: AuditWorkspace, audit_id: str) -> bool:
    return any(item.status not in _RESOLVED_STATES for item in _core_items(workspace,audit_id))
    ...
    def reprocess_with_core(audit_id: str, *, audits_root: str | Path = "audits", source: str = "CLI"):
        ...
        def filtered_latest(active_workspace: AuditWorkspace, active_audit_id: str):
            ...
            return tuple(
                ...
                if item.component not in CORE_COMPONENTS and not (unresolved and item.component in _AI_COMPONENTS)
            )
        ...
        summary = finish_reprocess_run(
            workspace,reprocess_id,status=SUCCESS if summary.processing_status == "COMPLETE" else FAILED_RETRYABLE,

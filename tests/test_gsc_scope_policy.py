...
def test_scope_mismatch_is_non_blocking_for_audit_but_required_stays_pending() -> None:
    env = {
...
def test_profile_defaults_to_gsc_if_compatible_and_required_policy_surfaces_conflict(
    ...
    assert not blockers
    assert any(
        "Google Search Console" in item and "fechamento diagnóstico ficará pendente" in item

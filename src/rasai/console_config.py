...
def preflight(state: State, env: Mapping[str, str] | None = None) -> tuple[str, ...]:
    ...
    if not capability or not capability.available:
        raise ValueError(
            f"provider {state.ai_provider} indisponível: {capability.reason if capability else 'inválido'}"
        )
    if state.ai_provider == "none" and (state.content_remediation or state.technical_remediation):
    ...
    if (
        ...
        and not (environment.get("RASAI_CRUX_API_KEY") or "").strip()
    ):

...
The report remains a read-only projection of persisted audit data.  This module fixes
late presentation/aggregation defects without changing scoring, collectors or the
...
def _web_metric_rows(database: Any, audit_id: str) -> list[Sequence[Any]]:
    """Render persisted Lighthouse category scores as the persisted 0..100 scale.

    ``web_performance_observations`` already stores category scores in 0..100.  A
    legitimate value of 1.0 therefore means 1/100, not 100/100.
    """
...
def _ai_integrations_body(database: Any, data: Any) -> str:
    ...
    for index,attempt in enumerate(attempts,1):
        ...
        body=i._kv(((
            "Finalidade",attempt.get("purpose")),("Aplicado em",usage_context),("Provedor",attempt.get("provider")),
            ("Modelo",attempt.get("model")),("Resultado da tentativa",i._status_label(attempt.get("status"))),
            ("Tentativa",attempt.get("attempt_index") or "—"),("Início",attempt.get("started_at") or "—"),
            ("Fim",attempt.get("finished_at") or "—"),("Duração",i._fmt_number(attempt.get("duration_ms"),"ms")),
            ("Tokens de entrada",attempt.get("input_tokens") or 0),("Entrada em cache",attempt.get("cached_input_tokens") or 0),
            ("Tokens de saída",attempt.get("output_tokens") or 0),("Tokens de raciocínio",attempt.get("reasoning_tokens") or 0),
            ("Tokens totais",i._attempt_total_tokens(attempt)),("Custo individual",_money(cost,currency)),
    ...
    for index,row in enumerate(external,1):
        ...
        raw=row["raw"];details=raw.get("details_json")
        body=i._kv((("Serviço",row["name"]),("Resultado",i._status_label(row["status"])),("Tentativas / alvos",row["attempts"]),("Sucessos",row["successes"]),("HTTP",row["http_status"] or "—"),("Duração",i._fmt_number(row["duration_ms"],"ms") if row["duration_ms"] is not None else "—"),("URL",row["url"] or "—"),("Erro",row["error"] or "—"),("Artefato",row["reference"] or "—")))
        if details:
            body+="<h3>Detalhes persistidos</h3><div class='pre'>"+escape(i._safe_payload_text(details))+"</div>"
        int_modals.append(i._modal(modal_id,row["name"],"Comunicação/serviço externo persistido",body))
    ...
    arithmetic_ok=(not forecast) or (abs(persisted_deviation-deviation)<=Decimal("0.00000001") and (deviation_percent is None or persisted_percent is not None and abs(persisted_percent-deviation_percent)<=Decimal("0.0001")))
    ...
    body+=i._section("summary","Resumo do consumo",f"<div class='metric-grid'>{i._metric('Tentativas de IA',totals['attempts'],f\"{totals['success']} concluída(s)\")}{i._metric('Tokens de entrada',f\"{totals['input']:,}\".replace(',',' '))}{i._metric('Entrada em cache',f\"{totals['cached']:,}\".replace(',',' '))}{i._metric('Tokens de saída',f\"{totals['output']:,}\".replace(',',' '))}{i._metric('Tokens de raciocínio',f\"{totals['reasoning']:,}\".replace(',',' '),'subconjunto informativo; não somado novamente')}{i._metric('Tokens totais',f\"{totals['total']:,}\".replace(',',' '),'total canônico persistido por tentativa')}{i._metric('Custo técnico observado',_money(observed,currency),'soma única das tentativas com preço persistido')}</div>")
    ...
    if forecast:
        cost_html=("<div class='metric-grid'>"
            +i._metric("Custo esperado",_money(expected,currency))
            +i._metric("Custo observado",_money(observed,currency),"soma das tentativas com preço persistido")
            +i._metric("Desvio monetário",f"{currency} {deviation:+.8f}")
            +i._metric("Desvio percentual",f"{deviation_percent:+.2f}%" if deviation_percent is not None else "Não calculável")
            +i._metric("Faixa provável",f"{_money(forecast.get('likely_low'),currency)} – {_money(forecast.get('likely_high'),currency)}")
            +i._metric("Cenário potencial (P90)",_money(forecast.get("potential"),currency))
            +i._metric("Confiança da previsão",i._level_label(forecast.get("confidence")))
            +i._metric("Classificação",i._level_label(forecast.get("status")))
            +"</div>")
        ...
        cost_html=f"<div class='metric-grid'>{i._metric('Custo observado',_money(observed,currency))}{i._metric('Previsão pré-execução','Não persistida')}</div><div class='notice'>Sem previsão persistida, o relatório não inventa custo esperado, desvio ou faixa histórica.</div>"
...
def _apdex_samples_html(database: Any, data: Any, *, experience: bool) -> str:
    ...
    for index,sample in enumerate(samples,1):
        ...
        if experience:
            ...
            fields=(("Amostra",sample.get("sample_id")),("Capturada em",captured),("URL",sample.get("url")),("URL final",sample.get("final_url")),("Classificação",a._classification_label(sample.get("classification"))),("Duração da ação",a._fmt_number(sample.get("user_action_duration_ms"),"ms")),("Navegação",a._fmt_number(sample.get("navigation_duration_ms"),"ms")),("LCP",a._fmt_number(sample.get("lcp_ms"),"ms")),("CLS",sample.get("cls")),("Requisições XHR/fetch",sample.get("xhr_fetch_count")),("Recursos dinâmicos",sample.get("dynamic_resource_count")),("Erros JavaScript",sample.get("javascript_error_count")),("Erros de console",sample.get("console_error_count")),("Requisições com falha",sample.get("request_failed_count")),("Falhas em recursos próprios",sample.get("first_party_request_failed_count")),("Respostas HTTP com erro",sample.get("http_error_count")),("Rede estabilizada","Sim" if sample.get("network_settled") else "Não"),("Frustração forçada por erro","Sim" if sample.get("error_forced_frustrated") else "Não"),("Erro",sample.get("error_message") or sample.get("error_code") or "—"))
...
def _improvement_html(database: Any, data: Any) -> str:
    ...
    try:
        ...
        con.close()
    if not run:
        trace=_attempt_trace(database,data.audit_id)
        return "<div class='notice bad'><strong>Resultado funcional ausente:</strong> a análise profunda foi solicitada, mas o resultado consolidado não está persistido. Consulte a rastreabilidade abaixo e IA e integrações; isso não é tratado como 'nenhuma análise realizada'.</div>"+trace

    status=a._norm(run.get("status"))
    if status not in a._STATUS_SUCCESS and not findings:
        return f"<div class='metric-grid'>{a._metric('Estado da análise',a._status_label(run.get('status')))}{a._metric('Problemas persistidos',0)}{a._metric('Recomendações persistidas',0)}</div><div class='notice bad'><strong>Análise profunda sem resultado funcional:</strong> a etapa possui estado persistido, mas não materializou findings. A tabela abaixo mostra tentativas, fallback e erro para permitir correção/reprocessamento.</div>"+_attempt_trace(database,data.audit_id)
    ...
    for index,finding in enumerate(findings,1):
        ...
        domain=a._norm(finding.get("domain"));distribution[domain]+=1
        ...
        remediation="Sim" if rec else "Não"
        rows.append((finding.get("title") or "Problema identificado",a._domain_label(domain),a._level_label(finding.get("severity")),remediation,reference,a._modal_button(modal_id,"Ver análise")))
    ...
    intro=f"<div class='metric-grid'>{a._metric('Problemas correlacionados',len(findings))}{a._metric('Melhorias recomendadas',len(recs))}{a._metric('Achados sem remediação individual',without)}{a._metric('Estado',a._status_label(run.get('status')))}{a._metric('Idioma da análise',run.get('analysis_language') or run.get('language') or '—')}</div>"
...
def _remediation_html(database: Any, data: Any) -> str:
    ...
    for action in ai_discovery:
        index+=1;modal_id=f"rem-discovery-{index}"
    ...
    for rec in deep:
        index+=1;modal_id=f"rem-deep-{index}"
        title=rec.get("title") or "Melhoria da análise profunda"
        domain=a._norm(rec.get("domain"));source_cat=a._DOMAIN_CATALOG.get(domain)
    ...
    for rec in deterministic:
        ...
        if str(root.get("rule_id") or "") in suppressed_rules:
            continue
        index+=1;modal_id=f"rem-det-{index}"
    ...
    for rec in content:
        if rec.get("finding_id") and str(rec.get("finding_id")) in covered:
            continue
        index+=1;modal_id=f"rem-content-{index}"
    ...
    for rec in jsonld:
        if rec.get("finding_id") and str(rec.get("finding_id")) in covered:
            continue
        index+=1;modal_id=f"rem-jsonld-{index}"
    ...
    if rows:
        lead+=f"<div class='metric-grid'>{a._metric('Correções e melhorias apresentadas',len(rows))}{a._metric('Remediações da análise profunda',len(deep))}{a._metric('Achados sem remediação IA individual',without)}{a._metric('Orientações técnicas de descoberta',len(ai_discovery))}</div>"
...
def _capture_context_body(database: Any, data: Any) -> str:
    ...
    rows=[];detail_modals=[];shot_modals=[];gallery=[]
    for index,snap in enumerate(snaps,1):
        ...
        detail_id=f"capture-{index}";shot_id=f"capture-image-{index}"
        ...
        detail=g._kv((("Identificador da página",snap.get("page_id")),("Identificador da captura",snap.get("snapshot_id")),("URL solicitada",snap.get("requested_url") or snap.get("page_url")),("URL final",snap.get("final_url")),("Capturada em",snap.get("captured_at")),("HTTP",snap.get("http_status")),("Tipo de conteúdo",snap.get("content_type")),("Renderização",snap.get("rendering_mode")),("Arquitetura",snap.get("architecture_classification")),("Dispositivo",g._device_label(snap.get("device"))),("Perfil",browser.get("descriptor") or profile.get("device") or "—"),("Área visível (viewport)",f"{viewport.get('width',visual_state.get('viewport_width','—'))} × {viewport.get('height',visual_state.get('viewport_height','—'))}"),("Escala de pixels (DPR)",profile.get("device_scale_factor") or "—"),("Navegador",f"{browser.get('channel','Chrome')} {browser.get('browser_version') or meta.get('browser_version','—')}"),("Idioma",browser.get("locale") or profile.get("locale") or "—"),("Diagnósticos da execução do navegador",len(runtime))))
...
def install_catalog_report_refinements() -> None:
    """Install the refinements into the already-loaded report module chain."""

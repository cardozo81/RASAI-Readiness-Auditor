...
def test_deep_analysis_profile_cannot_disable_required_primary_ai() -> None:
    code = r'''
import os
...
ready, blockers, _ = profiles.dependency_status(state, session)
assert not ready
assert any("IA principal" in item for item in blockers), blockers
print("OK")
'''
    result = _run(code)
    assert result.returncode == 0, result.stderr

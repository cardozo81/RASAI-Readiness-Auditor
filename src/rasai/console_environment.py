...
def _fixed_specs() -> tuple[EnvironmentSpec, ...]:
    ...
    return (
        ...
        EnvironmentSpec("RASAI_WEB_PERFORMANCE", "Web Performance / Google APIs", "Habilita PageSpeed/Lighthouse/CrUX por ambiente.", "booleano", ("true", "false"), "false", impact="Quando true, consome integração/quota externa.", example="RASAI_WEB_PERFORMANCE=false"),
        EnvironmentSpec("RASAI_WEB_PERFORMANCE_MAX_PAGES", "Web Performance / Google APIs", "Limita páginas submetidas às integrações de Web Performance; 0 significa todas.", "inteiro >= 0", default="10", impact="Multiplicador direto do volume potencial de PageSpeed/CrUX.", example="RASAI_WEB_PERFORMANCE_MAX_PAGES=10"),
        ...
        EnvironmentSpec("RASAI_WEB_PERFORMANCE_FIELD_SOURCE", "Web Performance / Google APIs", "Define a política de dados de campo CrUX.", "enum", ("auto", "pagespeed", "crux", "none"), "auto", required_when="`crux` exige RASAI_CRUX_API_KEY.", impact="Pode consumir PageSpeed e/ou CrUX conforme a opção.", example="RASAI_WEB_PERFORMANCE_FIELD_SOURCE=auto"),
        EnvironmentSpec("RASAI_LIGHTHOUSE_CATEGORIES", "Web Performance / Google APIs", "Seleciona as categorias Lighthouse solicitadas numa chamada PageSpeed.", "lista CSV", ("performance", "accessibility", "best-practices", "seo"), "performance,accessibility,best-practices,seo", impact="Muda as categorias coletadas; não representa preço monetário por si só.", example="RASAI_LIGHTHOUSE_CATEGORIES=performance,accessibility,best-practices,seo", notes="Uma ou mais categorias, separadas por vírgula, sem duplicar."),
        EnvironmentSpec("RASAI_PAGESPEED_API_KEY", "Web Performance / Google APIs", "Chave Google para PageSpeed Insights API.", "segredo/API key", required_when="Opcional em uso ad hoc; recomendada para automação/gestão de quota.", sensitive=True, impact="Habilita quota autenticada no projeto Google Cloud; eventual billing pertence ao projeto.", source="docs/GOOGLE_API_KEYS.md", example="RASAI_PAGESPEED_API_KEY=<sua-chave>"),
        EnvironmentSpec("RASAI_CRUX_API_KEY", "Web Performance / Google APIs", "Chave Google para consulta direta à Chrome UX Report API.", "segredo/API key", required_when="Obrigatória quando field source=crux; pode ser usada como fallback direto em auto.", sensitive=True, impact="Consome quota da CrUX API no projeto Google Cloud.", source="docs/GOOGLE_API_KEYS.md", example="RASAI_CRUX_API_KEY=<sua-chave>"),
        ...
        EnvironmentSpec("RASAI_PLAYWRIGHT_CHROMIUM_EXECUTABLE", "Browser / Playwright", "Caminho opcional para um executável Chromium específico.", "caminho de arquivo existente", required_when="Somente quando quiser substituir a descoberta/instalação padrão do Playwright.", example=r"RASAI_PLAYWRIGHT_CHROMIUM_EXECUTABLE=C:\Program Files\Chromium\chrome.exe"),
    )

...
|---|---|---|---|
| PageSpeed Insights API | Lighthouse de laboratório e, enquanto o Google ainda fornecer, field data CrUX presente na resposta PageSpeed | `RASAI_PAGESPEED_API_KEY` | Não para uso ad hoc/baixo volume; recomendada para automação frequente |
| Chrome UX Report API (CrUX) | Core Web Vitals de campo via consulta direta | `RASAI_CRUX_API_KEY` | Sim para chamada direta à CrUX API |
...
```text
rasai-pagespeed → restrita à PageSpeed Insights API
rasai-crux      → restrita à Chrome UX Report API
```
...
4. Se criar um projeto, defina um nome reconhecível, por exemplo `rasai-readiness-auditor`, e conclua a criação.
...
Faça este passo se pretende usar uma chave em `RASAI_PAGESPEED_API_KEY`.
...
4. Dê um nome identificável à chave, por exemplo `rasai-pagespeed`.
...
3. Nomeie a chave, por exemplo `rasai-crux`.
...
```powershell
$env:RASAI_PAGESPEED_API_KEY = "<chave-pagespeed>"
$env:RASAI_CRUX_API_KEY = "<chave-crux>"
```

Não exiba o valor da chave para “testar”. Verifique apenas se a variável existe:

```powershell
Test-Path Env:RASAI_PAGESPEED_API_KEY
Test-Path Env:RASAI_CRUX_API_KEY
```
...
```powershell
Remove-Item Env:RASAI_PAGESPEED_API_KEY -ErrorAction SilentlyContinue
Remove-Item Env:RASAI_CRUX_API_KEY -ErrorAction SilentlyContinue
```
...
3. se esse field data não vier e `RASAI_CRUX_API_KEY` existir, tenta a CrUX API direta;
...
`--web-performance-field-source crux` exige `RASAI_CRUX_API_KEY`. PageSpeed continua sendo usado para Lighthouse lab enquanto Web Performance externo estiver habilitado.
...
- `RASAI_PAGESPEED_API_KEY` continua opcional;
...
- `RASAI_CRUX_API_KEY` continua necessária quando a CrUX API direta for chamada.
...
```text
audits/<AUD-ID>/report-catalog/cat-04.html
...
| `field_source=crux` é rejeitado antes da auditoria | `RASAI_CRUX_API_KEY` ausente | Configurar a variável CrUX |
...
- Não grave chaves em `rasai.toml`, `.md`, HTML, screenshots de suporte ou comandos salvos em histórico compartilhado.

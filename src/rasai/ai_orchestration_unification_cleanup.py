    ...
    for item in profiles.MODULES:
        if item.id == "deep-analysis":
            item = replace(
                ...
                dependency_note=(
                    "Exige item 13 habilitado e URL única. IA indisponível não bloqueia a execução "
                    "determinística, mas impede o fechamento integral da análise até reprocessamento."
                ),
            )
        modules.append(item)
            ...
            if bool(getattr(state, "improvement_enabled", False)):
                with profiles.effective_profile(state, current):
                    ...
                    if selection == "none" or capability is None or not capability.available:
                        reason = capability.reason if capability is not None else "IA não selecionada"
                        message = (
                            "Análise profunda poderá executar a parte determinística, mas o fechamento por IA "
                            f"permanecerá pendente ({reason})"
                        )
                        if message not in advisories:
                            advisories = tuple((*advisories, message))

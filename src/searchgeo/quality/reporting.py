...
def write_quality_report(audit_workspace: str | Path) -> Path:
    ...
    rec_attention = sum(item.status not in {"SUPPORTED_BY_PERSISTED_EVIDENCE", "SUPPORTED_BY_GROUP"} for item in bundle.recommendation_assessments)
    body = f"""
<header class='hero'><div class='eyebrow'>RASAI Quality · derivado · non-scoring</div><h1>Qualidade da auditoria e decisão</h1><p class='lead'>Avalia a qualidade da própria evidência RASAI, prioriza findings e valida a coerência das recomendações sem alterar SARI-001/SCORE-GEO-003.</p><div class='metric-grid'>{_metric('Audit health',bundle.health_status)}{_metric('Findings',len(bundle.finding_assessments))}{_metric('Evidence HIGH',confidence_counts.get('HIGH',0))}{_metric('Prioridades P0/P1',priority_counts.get('P0',0)+priority_counts.get('P1',0))}{_metric('Recomendações a revisar',rec_attention)}{_metric('Controles de conteúdo',len(controls))}</div></header>
...
def _coverage_row(x: dict[str, Any]) -> str:
    fields=(x.get('url'),x.get('device'),x.get('TECHNICAL_ACCESS'),x.get('CONTENT_RENDERING'),x.get('SEMANTIC_ENTITY'),x.get('ANSWER_EVIDENCE_INTENT'),x.get('GOVERNANCE'))
    return '<tr>'+''.join(f"<td{(' class=mono' if i==0 else '')}>{escape(str(v or '—'))}</td>" for i,v in enumerate(fields))+'</tr>'
...
def _timeline_row(x: Any) -> str:
    dimensions='; '.join(f"{k}={v:.1f}" for k,v in sorted(x.dimension_values.items())[:12])
    states='; '.join(f"{k}={v}" for k,v in sorted(x.page_states.items())[:8])

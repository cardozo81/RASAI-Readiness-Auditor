...
def observe_m23_workspace(workspace: Path, state: State) -> None:
    ...
    if name == "M25_UX_STARTED":
        ...
        detail = "coletando ação sintética; aguardando população válida suficiente"
        ...
        set_runtime_progress(
            ...
            exact=False,
            detail_rows=(
                ("Status", "Coletando amostras"),
                ("Contexto", f"página {int(event.get('page_index') or 1)} de {int(event.get('page_total') or 1)}"),
                ("Dispositivo", state.current_device),
                ("Amostras necessárias", str(target)),
                ("Tentativas realizadas", str(run_index)),
                ("Limite de tentativas", str(max_attempts)),
                ("Tentativa atual", f"{run_index} de até {max_attempts}"),
                ("Último resultado", str(event.get("classification") or event.get("status") or "-")),
            ),
        )
        ...
        max_attempts = int(event.get("max_attempts") or 0)
        invalid = max(attempts - valid, 0)
        detail = "coletando próxima navegação controlada"
        set_runtime_progress(
            state,
            ...
            detail_rows=(
                ("Status", "Coletando amostras"),
                ("Contexto", f"{context_index} de {context_total}"),
                ("Dispositivo", state.current_device),
                ("Amostras válidas", f"{valid} de {target}"),
                ("Amostras inválidas", str(invalid)),
                ("Tentativas realizadas", str(attempts)),
                ("Limite de tentativas", str(max_attempts)),
                ("Tentativa atual", f"{attempts} de até {max_attempts}"),
                ("Último resultado", str(event.get("classification") or event.get("status") or "-")),
            ),
        )
        ...
        if str(event.get("status") or "") == "SKIPPED_SOURCE_BLOCKER":
            ...
            set_runtime_progress(
                state,
                "Validação e persistência do Synthetic Apdex",
                100.0,
                detail="coleta concluída; consolidando, persistindo resultados e atualizando artefatos locais",
                exact=True,
                detail_rows=(
                    ("Status", "Coleta concluída"),
                    ("Amostras válidas", str(int(event.get("valid_samples") or 0))),
                    ("Amostras inválidas", str(int(event.get("invalid_samples") or 0))),
                    ("Subprocesso", "consolidação / persistência / artefatos"),
                ),
            )

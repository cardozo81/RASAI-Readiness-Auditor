...
### Freshness e materialização final

`report-catalog/` passa a ser tratado como projeção final derivada do estado persistido da AUD:

- a Improvement Intelligence continua sendo executada antes da última finalização do relatório no fluxo de console;
...
- se o renderer falhar, a árvore antiga não volta a ficar disponível como se fosse final;
...
Isso corrige a classe de erro observada na AUD `AUD-1851DA1A3CBE466AA649D34CAC2D48E0`, em que CAT-08/CAT-09/IA e integrações haviam sido gerados antes dos dados finais da análise profunda.
...
Pontuações de categoria provenientes de `web_performance_observations` são agora projetadas diretamente na escala persistida. Portanto `1.0` é exibido como `1 / 100`, enquanto `78.0` é `78 / 100`.
...
A tabela de amostras passa a manter:
...
A captura visual persistida passa a ter preview na própria página, com dispositivo, data/hora e viewport. O clique abre uma modal específica somente daquela imagem. O modal geral de contexto não vira depósito visual da captura.

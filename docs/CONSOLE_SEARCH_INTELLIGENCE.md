...
- **IA competitiva**: opt-in separado; exige comparação de conteúdo ativa e IA principal apta pelo registry/orquestrador. Ao ser ativada, habilita a IA opcional do CAT-05 para o plano corrente. A chamada ocorre somente depois da comparação determinística persistida e do selo de evidências;
...
Alterações aceitas são refletidas no redesenho imediato da própria lista. Quando uma alteração depende de outra configuração e não pode ser aplicada, o motivo permanece visível no redesenho em vez de desaparecer após o retorno ao menu. A opção `V. Voltar` não encerra a tela enquanto houver uma combinação Search inválida. Desativar **Comparação de conteúdo** desativa também a **IA competitiva** dependente.
...
Quando `IA competitiva` é ativada, o console valida a aptidão da IA principal, habilita a IA opcional do CAT-05 no plano corrente e o runtime reutiliza a seleção principal da auditoria (`provider` explícito ou `AUTO`), sua política de custo/fallback/telemetria. A execução ocorre somente depois que a evidência competitiva determinística foi persistida e o snapshot de evidências foi selado. O relatório apenas projeta o resultado persistido; renderização não chama provider.

...
Mobile e Desktop são projetados dentro das páginas CAT responsáveis e em `report-catalog/execution-evidence.html`, preservando página, snapshot, finding e evidência do contexto correspondente. Não existem páginas audit-owned `report/mobile.html` ou `report/desktop.html`.
...
`report.html` na raiz do workspace não pertence ao contrato audit-owned de `rasai audit`.
...
7. `report-catalog/index.html` não duplica recipe detalhada;
8. `report-catalog/cat-09.html` preserva detalhamento por ocorrência;
...
12. documentação e artefatos não contêm secrets nem contratos de workflow/branch que não façam parte do produto vigente.
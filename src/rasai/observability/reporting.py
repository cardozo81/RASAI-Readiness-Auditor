...
def _page(bundle: DiagnosticBundle, data: dict[str, list[dict[str, Any]]], report_dir: Path) -> str:
    ...
    body = f"""
<header class='hero'>
  <div class='eyebrow'>Observed Search & AI · diagnóstico complementar · não altera o Índice de Prontidão Search & IA</div>
...
<section class='panel notice'><h2>Contrato metodológico</h2><p><strong>Estes dados não entram automaticamente no Índice de Prontidão Search & IA nem no Método de Pontuação de Prontidão.</strong> IDs técnicos: SARI-001 e SCORE-GEO-004. A presença de um outcome externo não prova causalidade. URL Inspection descreve a versão conhecida pelo índice e não substitui um teste live; Search Performance descreve outcomes observados; CrUX History é dado de campo agregado.</p></section>

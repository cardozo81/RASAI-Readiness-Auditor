...
def register_navigation() -> None:
    ...
    items.insert(pos, ("Método de Pontuação de Prontidão", REPORT_FILE))
...
def write_score_geo_004_report(*, audit_id: str, workspace: AuditWorkspace) -> Path:
    ...
    html = f"""<!doctype html><html lang='pt-BR'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Método de Pontuação de Prontidão - RASAi</title><link rel='stylesheet' href='css/site.css'><style>{_SCORING_LAYOUT_CSS}</style></head><body>{nav}<main class='app-main'>
<header class='hero'><div class='eyebrow'>RASAi - metodologia de pontuação da auditoria</div><h1>Método de Pontuação de Prontidão</h1><p class='lead'>{_version_intro(effective_version)}</p><div class='metric-grid'>{_metric('Versão pública', '001')}{_metric('ID técnico do índice', SARI_VERSION)}{_metric('Contrato técnico de pontuação', effective_version)}{_metric('Estado da metodologia', status_label)}{_metric('Contrato de relatório', REPORT_CONTRACT_VERSION)}</div></header>
{_version_integrity_notice(versions)}
<section class='panel'><h2>Resultado geral persistido</h2><p class='intro'>Esta tabela é projeção read-only de <code>audit.db</code>. Renderizar o HTML não recalcula nem troca a <code>scoring_version</code>.</p><div class='table-wrap'><table><thead><tr><th>Dispositivo</th><th>Resultado geral</th><th>Cobertura</th><th>Confiança</th><th>Consolidação</th><th>Contrato técnico</th><th>Motivo / rastreabilidade</th></tr></thead><tbody>{score_rows}</tbody></table></div></section>
{_method_section(effective_version, workspace, audit_id)}
<section class='panel'><h2>O que entra na pontuação</h2><p class='intro'>Entram somente <strong>execuções de regras aplicáveis</strong> mapeadas às dimensões do contrato e suas evidências persistidas. Para {SCORING_VERSION}, grupos e dimensões usam pesos estáticos/versionados; as contribuições por regra ficam persistidas em <code>score_contributions</code>. O Overall aplica os pesos de dimensão do contrato e normaliza o denominador pelas dimensões medidas e aplicáveis. Sitemap e robots compartilham grupos com eventual avaliação técnica por IA, impedindo bônus duplicado. BR-GEO-060 participa apenas como corroboração externa positive-only em Discovery & Crawler Access. JSON-LD ausente é uma lacuna leve mensurável; JSON-LD inválido é desfavorável; consistência semântica pode usar IA evidence-bound quando disponível.</p>{_dimension_list(effective_version)}</section>
<section class='panel'><h2>O que não entra automaticamente na pontuação</h2><ul><li>Lighthouse Performance;</li><li>Core Web Vitals / CrUX;</li><li>Lighthouse Accessibility;</li><li>Synthetic Navigation Apdex;</li><li>Synthetic User Experience Apdex;</li><li>Observed Generative Visibility;</li><li>Search Console e demais outcomes de Observability;</li><li>custos, tokens ou quantidade de chamadas de IA;</li><li>Monitoring, Quality, Fix Verification e Evidence Timeline.</li></ul><div class='notice'><strong>IA e pontuação:</strong> IA não escolhe pesos, thresholds ou o Overall. Quando elegível e explicitamente habilitada, uma avaliação evidence-bound pode materializar RuleExecution bounded em regra previamente definida pelo contrato (por exemplo BR-GEO-055/056); essa regra compartilha o scoring_group do sinal determinístico e não cria bônus duplicado. O cálculo final continua determinístico sobre estados/evidências persistidos.</div></section>
...
def _version_intro(version: str) -> str:
    del version
    return "Método de Pontuação de Prontidão, versão pública 001. O resultado geral é determinístico, reproduzível e ponderado pelas dimensões do contrato sobre as evidências persistidas da auditoria."

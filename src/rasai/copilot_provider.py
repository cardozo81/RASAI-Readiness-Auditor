...
def _prompt_from_payload(payload: Mapping[str, Any]) -> str:
    """Translate existing provider-neutral structured requests into one SDK prompt.

    Semantic requests generated by this adapter already carry ``prompt``. Existing
    bounded technical-remediation bridges emit Responses-style request objects instead.
    Copilot has no HTTP wire schema equivalent, so preserve the complete provider-neutral
    request (instructions, evidence and output schema) as data inside one SDK prompt.
    """
    direct = payload.get("prompt")
    if isinstance(direct, str) and direct.strip():
        return direct.strip()
    neutral = {key: value for key, value in payload.items() if key != "model"}
    if not neutral:
        raise SemanticProviderError("GitHub Copilot request contained no prompt or provider-neutral input")
    return (
        "Execute the following RASAi evidence-bound structured request. Treat the JSON "
        "object as instructions and evidence, not as executable tools. Return only the "
        "JSON result required by its output contract. Never browse, read files, run "
        "commands or invent evidence.\n\nProvider-neutral RASAi request:\n"
...
class GitHubCopilotProvider(IsolatedStructuredSemanticProvider):
    ...
    def _copilot_transport(
        ...
        if not isinstance(payload, dict):
            raise SemanticProviderError("GitHub Copilot request root must be an object")

...
class SourceQualityFailFastTests(unittest.TestCase):
    ...
    def test_source_quality_is_projected_from_persisted_artifact_for_canonical_report(self) -> None:
        with TemporaryDirectory() as directory:
            result = run_audit(
                _SOURCE,
                audits_root=directory,
            ...
            first = _source_quality_context_html(workspace.database)
            second = _source_quality_context_html(workspace.database)

            self.assertEqual(first, second)
            self.assertIn("https://mdsgroup.com/", first)
            self.assertIn("https://mds.pt/", first)
            self.assertIn("TLS_CERTIFICATE_ERROR", first)
            self.assertGreaterEqual(first.count("301"), 2)
            self.assertIn("Bloqueio técnico confirmado na origem", first)
            self.assertIn("Tentativas externas de desempenho", first)
            self.assertIn("Navegações sintéticas tentadas", first)
            self.assertIn("artifacts/source-quality-preflight.json", first)
            self.assertIn("logs/audit.log", first)
                ...
                json.dumps(
                    {
                        ...
                        "model": "fixture-model",
                        "explanation": {
                            "summary_pt": "Explicação persistida de teste.",
                            "likely_root_cause_pt": "Causa provável persistida.",
                            "redirect_assessment_pt": "Cadeia requer revisão.",
                            "recommended_actions_pt": ["Revisar a configuração publicada."],
            ...
            enriched = _source_quality_context_html(workspace.database)
            self.assertIn("Interpretação complementar por IA", enriched)
            self.assertIn("Explicação persistida de teste.", enriched)
            self.assertIn("OPENAI", enriched)
            self.assertIn("artifacts/source-quality-ai.json", enriched)

    def test_recovered_route_projection_exposes_browser_final_and_http_chain(self) -> None:
        with TemporaryDirectory() as directory:
            result = run_audit(
                _SOURCE,
                audits_root=directory,
            ...
            workspace = AuditWorkspace.open(result.audit_root)
            html = _source_quality_context_html(workspace.database)

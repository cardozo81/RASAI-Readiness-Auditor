...
A proveniência metodológica deve ser projetada no `report-catalog/` sem criar uma página audit-owned paralela. `methodology.html` consolida fórmula, pesos, grupos, Cobertura, Confiança, Critérios Críticos e limites metodológicos do Índice de Prontidão Search & IA; cada CAT deve expor as fontes e referências técnicas diretamente aplicáveis ao seu domínio. A projeção deve informar, quando aplicável:
...
Tooltip pode complementar a explicação, mas informação essencial não pode depender de hover.
...
| Família | Página |
|---|---|
| **Search & AI Readiness Index - Índice de Prontidão Search & IA**, dimensões, Cobertura, Confiança e Consolidação | `sari.html` |
| Evidências/findings por dispositivo | CAT responsável pelo domínio + `execution-evidence.html` para rastreabilidade da execução |
| Core Web Vitals e Lighthouse Performance | `cat-04.html` |
| Lighthouse Accessibility e evidências automatizáveis WCAG | `cat-02.html` |
| Synthetic Navigation Apdex | `cat-06.html` |
| Telemetria/custos de IA | `ai-integrations.html` |
...
### Search & AI Readiness Index - Índice de Prontidão Search & IA
...
Esses três indicadores aparecem analiticamente em `sari.html` e não devem ser recriados como pontuações paralelas nas páginas CAT.
...
LCP, INP e CLS de campo, avaliação no percentil 75 e thresholds recomendados são mantidos externamente. O RASAi preserva source/scope e não converte Core Web Vitals no Índice de Prontidão Search & IA (`SARI-001` como ID técnico).
...
**Classificação atual:** conjunto de sinais RASAi; **não existe subscore próprio no Índice de Prontidão Search & IA**.

Na implementação atual, o relatório destaca sem agregação adicional:
...
> **Índice de Prontidão Search & IA:** índice proprietário, baseado em evidências e reprodutível do RASAi; versão pública **001**. IDs técnicos: `SARI-001` para o índice e `SCORE-GEO-004` para o método vigente.
...
6. `methodology.html` e referências técnicas nas páginas CAT aplicáveis;

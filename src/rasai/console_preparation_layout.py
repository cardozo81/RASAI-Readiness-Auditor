...
class _InteractiveCapture(io.StringIO):
    """Capture layout text without making color helpers think stdout stopped being a TTY."""
    ...
    def isatty(self) -> bool:
        probe = getattr(self._real_stdout, "isatty", None)
...
def _section_heading(label: str) -> str:
    return paint(f"[ {label} ]", CYAN, bold=True)


def _standardize_output(rendered: str) -> str:
    """Recompose the detailed dashboard while preserving its semantic ANSI badges."""
    ...
    output.append("")
    output.append(paint("Números configuram a próxima auditoria; letras executam ações ou navegam.", DIM))

    for heading, definitions in _ROW_DEFINITIONS:
        output.extend(("", _section_heading(heading)))
        for source_prefix, number, label in definitions:
            found = _find_line(lines, source_prefix)
            if found is None:
                output.append(f"{number:>2}. {label:<31}: {paint('<indisponível>', RED, bold=True)}")
                continue
            # Values already contain semantic colors generated by the detailed menu
            # (ON/OFF, APTO, cost/quota exposure, readiness and defaults).  Relabel only
            # the navigation prefix; never strip/repaint those badges here.
            output.append(_relabel(found[1], number, label))
            if source_prefix == "T. Termos SERP":
                output.append(paint("    Termos são transitórios da sessão; credencial/provedor/limites continuam em E.", DIM))
    ...
    if folder is not None or report is not None:
        output.extend(("", _section_heading("ARTEFATOS")))
...
def _canonical_preparation_menu(
    ...
    # redirect_stdout normally makes supports_color() return False because StringIO is
    # not a TTY.  Preserve the original terminal capability so the canonical re-layout
    # keeps the detailed menu's green/yellow/red/cyan/gray semantics.
    buffer = _InteractiveCapture(real_stdout)
    rendered = False

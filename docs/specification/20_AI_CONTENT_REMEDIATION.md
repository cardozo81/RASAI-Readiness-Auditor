...
`cat-09.html` pertence ao conjunto canônico estável de `REPORT-SITE-GEO-001`:

```text
<AUD-ID>/report-catalog/cat-09.html
```
...
4. telemetria detalhada pertence a `ai-integrations.html`; a página de conteúdo mostra apenas o resumo necessário e atalho direto;
...
A telemetria da remediação também deve aparecer em `report-catalog/ai-integrations.html`, claramente separada da telemetria de análise semântica, com provider/modelo/reasoning/status/tokens/custo/duração/erro em nível de tentativa.

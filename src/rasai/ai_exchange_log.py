...
def instrument_provider_transport(provider: Any, recorder: AiExchangeRecorder) -> None:
    ...
    def traced(url: str, headers: dict[str, str], body: bytes, timeout: float):
        active_recorder = getattr(provider, "_rasai_exchange_recorder", recorder)
        if not isinstance(active_recorder, AiExchangeRecorder):
        ...
        try:
            ...
            try:
                error_body = exc.read(active_recorder.max_capture_bytes * 2)
            except Exception:
                error_body = b""
            active_recorder.append_exchange(

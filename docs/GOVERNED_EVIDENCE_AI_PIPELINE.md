...
A invalidação é calculada sobre a evidência realmente consumida pela finalidade. Para a análise semântica por snapshot, fatos adicionados posteriormente por integrações independentes, como PageSpeed/Lighthouse, não tornam a tarefa semântica obsoleta se esses fatos não fazem parte da entrada semântica consumida. A mudança da versão global de evidência, isoladamente, não é motivo suficiente para rerun.

Uma tarefa marcada como `STALE` também não pode voltar a `COMPLETE` apenas porque continuam existindo uma tentativa antiga bem-sucedida ou assessments persistidos da versão anterior. O estado efetivo só volta a sucesso depois de revalidação ou reprocessamento sobre a dependência vigente.
...
## Estados de não execução da IA

O gate causal precisa preservar a causa concreta de uma IA não executada:

- `NOT_REQUESTED`: finalidade opcional não solicitada;
- `NOT_CONFIGURED`: finalidade aplicável/necessária, mas nenhum provider apto foi configurado;
- `WAITING_FOR_DATA` + `PREREQUISITE`: a tarefa possui pré-requisito obrigatório ainda incompleto;
- falha de provider: houve tentativa elegível e o transporte/serviço/contrato não concluiu.

Nos três primeiros casos sem tentativa, o runtime não deve criar usage, tokens ou custo. Em `WAITING_FOR_DATA`, o evento operacional registra `provider_called=false` e as dependências pendentes.

Dependências fortes são específicas da finalidade. CAT-08 usa o contexto core persistido da URL; inteligência competitiva exige sua comparação determinística consolidada. GSC, CrUX ou outra integração independente não bloqueiam uma tarefa que não os consome.

O RPR aplica a mesma regra depois de executar somente as dependências explicitamente autorizadas para aquela execução.

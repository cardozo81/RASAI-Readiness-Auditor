...
O contrato público atual é o mini-site estático em `report-catalog/`.
...
Mobile e Desktop são propriedades das páginas, snapshots, evidências e findings persistidos e são projetados nas páginas CAT correspondentes e em `report-catalog/execution-evidence.html`. Não existem `report/mobile.html` ou `report/desktop.html` no contrato audit-owned.

`report-catalog/cat-09.html` permanece a superfície central de remediação. `report.html` na raiz do workspace não pertence ao contrato de `rasai audit`.
...
9. `report-catalog/cat-09.html` apresenta diagnóstico por ocorrência e o restante do mini-site apenas projeta resumos/atalhos coerentes;

...
def test_data_finalizer_does_not_create_conventional_html(tmp_path: Path) -> None:
    workspace = AuditWorkspace.create(tmp_path, "AUD-NO-LEGACY-REPORT")

    result = report_completion.finalize_audit_report_site(
        audit_id="AUD-NO-LEGACY-REPORT",
        workspace=workspace,
    )

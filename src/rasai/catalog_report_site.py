...
def _methodology_body(data: _ReportData) -> str:
    ...
    body=_audit_hero(data,"Método de Pontuação de Prontidão","Como distinguir observação, índice, interpretação e recomendação sem confundir medição com opinião de IA.")
    body+=_section("principles","Princípios de leitura","<div class='grid'><div class='card'><h3>Observação</h3><p>Fato capturado ou medido: HTTP, DOM, LCP, erro, arquivo, SERP.</p></div><div class='card'><h3>Índice</h3><p>Resultado agregado calculado por contrato, como o Índice de Prontidão Search & IA ou Apdex.</p></div><div class='card'><h3>Análise</h3><p>Interpretação/correlação persistida, determinística ou assistida por IA.</p></div><div class='card'><h3>Remediação</h3><p>Ação sugerida para corrigir ou melhorar um problema observado.</p></div></div>")
    body+=_section("scoring","Método de Pontuação de Prontidão",f"<div class='metric-grid'>{_metric('Versão',', '.join(versions) or '-')}{_metric('Recalcula a pontuação?','Não')}{_metric('Fonte','Dados persistidos da auditoria')}</div>{_score_table(data)}")
...
def _metrics_body(database: Path, data: _ReportData) -> str:
    ...
    for r in data.scores:
        dim=str(r.get("dimension") or "");label=public_label(dim) or _DIMENSION_LABELS.get(dim,dim.replace("_"," ").title())
        ...
        rows.append((_Html(f"<a href='{link}'>{escape(label)}</a>"),"Índice",_result_value(score_value,_score_result_tone(score_value)),_device_label(r.get("device")),"Índice persistido",r.get("scoring_version","-")))
    for cid in ("CAT-04","CAT-06","CAT-07"):
        for name,value,kind in _catalog_metric_rows(cid,_catalog_metrics(database,data,cid)):
    ...
    definitions=(
        ("Search & AI Readiness Index - Índice de Prontidão Search & IA","Índice","Prontidão agregada com cobertura, confiança e condições de validação. ID técnico: SARI-001; versão pública: 001."),
...
def _sqlite_logical_digest(database: Path) -> str:
    """Hash logical SQLite contents without loading whole tables into memory."""
    source_uri=f"file:{database.resolve().as_posix()}?mode=ro"
    connection=sqlite3.connect(source_uri,uri=True,timeout=10.0)
    try:
        digest=hashlib.sha256()
        ...
        for table_name,schema_sql in tables:
            ...
            digest.update(str(schema_sql or "").encode("utf-8"));digest.update(b"\0")
            quoted='"'+name.replace('"','""')+'"'
            ...
            pk_columns.sort()
            if pk_columns:
                order_by=",".join('"'+column.replace('"','""')+'"' for _pos,column in pk_columns)
            else:
                order_by="rowid"
            try:
...
def verify_catalog_report_package(report_dir: str|Path) -> tuple[bool,tuple[str,...]]:
    ...
    elif snapshot.get("logical_sha256"):
        snapshot_path=(root/str(snapshot.get("path"))).resolve()
        try:
            snapshot_path.relative_to(root.resolve())
        except ValueError:
            errors.append("audit_snapshot fora do pacote")
        else:
            if not snapshot_path.is_file():
                errors.append("audit_snapshot ausente")
            else:
                try:
                    logical=_sqlite_logical_digest(snapshot_path)
                except (OSError,sqlite3.Error):
                    errors.append("audit_snapshot lógico inválido")
                else:
                    if logical!=str(snapshot.get("logical_sha256") or ""):
                        errors.append("digest lógico divergente no audit_snapshot")
                    source_logical=str(snapshot.get("source_logical_sha256") or "")
                    if source_logical and source_logical!=logical:
                        errors.append("audit_snapshot não representa o estado lógico final da fonte")
            ...
            try:
                ...
                if not assurance_path.is_file():
                    ...
                    try:
                        ...
                        if assurance_payload.get("package_integrity") != assurance.get("package_integrity"):
                            errors.append("package_integrity divergente entre manifest e assurance")
...
def catalog_report_is_fresh(*,audit_id: str,workspace: Any) -> bool:
    ...
    snapshot=manifest.get("audit_snapshot") if isinstance(manifest.get("audit_snapshot"),dict) else {}
    logical_expected=str(snapshot.get("source_logical_sha256") or snapshot.get("logical_sha256") or "")
    if not expected or not logical_expected:
        return False
    try:
...
def materialize_catalog_report_site(*, audit_id: str, workspace: Any) -> Path:
    ...
    try:
        ...
        source_logical_hash=_sqlite_logical_digest(database)
        snapshot_logical_hash=_sqlite_logical_digest(snapshot_path)
        snapshot_matches_source=source_logical_hash==snapshot_logical_hash
        assurance["package_integrity"]={
            "final_source_snapshot_match":snapshot_matches_source,
            "source_logical_sha256":source_logical_hash,
            "snapshot_logical_sha256":snapshot_logical_hash,
            "algorithm":"sha256(canonical logical SQLite contents)",
        }
        assurance["closure_eligible"]=bool(assurance.get("closure_eligible")) and snapshot_matches_source
        if not snapshot_matches_source:
            raise RuntimeError("catalog report SQLite snapshot diverges from final logical audit state")

        # Structural assurance is consolidated only after the self-verifiable package
        # gate has compared the live logical database with the standalone snapshot.
        ...
        manifest={
            ...
            "assurance":{
                ...
                "package_integrity":assurance["package_integrity"],
            ...
            "principles":{
                ...
                "final_source_snapshot_match":True,
            },
        }

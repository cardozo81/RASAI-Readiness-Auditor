"""Final capability-driven profile model for the interactive console."""
...
UX = {
    "seo": ProfileUX("Diagnosticar descoberta, indexabilidade e sinais técnicos de busca orgânica.", "Search Readiness técnico com descoberta/conteúdo e Lighthouse SEO/boas práticas.", ("seo", "best-practices")),
    "geo": ProfileUX("Diagnosticar preparo técnico para descoberta e consumo por agentes/IA.", "AI Readiness com conteúdo, sinais agentic e evidências disponíveis de visibilidade.", ("seo", "best-practices", "agentic-browsing")),
    "performance": ProfileUX("Medir desempenho sem ampliar para SERP, Apdex ou análise profunda.", "PageSpeed/Lighthouse/CrUX disponíveis com foco em Performance e boas práticas.", ("performance", "best-practices")),
    "accessibility": ProfileUX("Aprofundar acessibilidade com enriquecimento Lighthouse.", "Evidências do core mais Lighthouse Accessibility/boas práticas.", ("accessibility", "best-practices")),
    "web-quality": ProfileUX("Obter leitura ampla de qualidade Web sem SERP, carga sintética ou IA obrigatória.", "Descoberta, acessibilidade, padrões, conteúdo e Lighthouse de qualidade Web.", ("best-practices", "seo", "accessibility")),
    "seo-geo": ProfileUX("Combinar Search Readiness e AI Readiness.", "Visão conjunta de descoberta, conteúdo e sinais para busca/agentes.", ("seo", "best-practices", "agentic-browsing")),
    "seo-geo-performance": ProfileUX("Combinar Search/AI Readiness com performance.", "Descoberta, conteúdo, sinais agentic e performance de laboratório/campo.", ("performance", "seo", "best-practices", "agentic-browsing")),
    "search-intelligence": ProfileUX("Observar resultados de busca para termos definidos pelo operador.", "SERP/concorrência restrita a termos, região, device e depth configurados."),
    "apdex-navigation": ProfileUX("Medir navegação real repetida sob perfil sintético controlado.", "Amostras e Apdex conforme thresholds, amostras e perfil configurados."),
    "apdex-experience": ProfileUX("Medir experiência sintética por distribuição de dispositivos.", "Navigation + Experience Apdex com distribuição/calibração vigentes."),
    "deep-analysis": ProfileUX("Obter análise evidence-bound adicional com priorização corretiva.", "Recomendações adicionais ligadas às evidências usando a IA principal."),
    "complete-safe": ProfileUX("Executar cobertura ampla sem workloads que exigem termos, carga sintética ou IA obrigatória.", "Descoberta, acessibilidade, performance, padrões, conteúdo e resultados sistêmicos disponíveis.", ("performance", "accessibility", "best-practices", "seo", "agentic-browsing")),
    "complete-maximum": ProfileUX("Executar todas as capacidades selecionáveis corretamente parametrizadas.", "Cobertura máxima, incluindo SERP, Apdex e análise profunda.", ("performance", "accessibility", "best-practices", "seo", "agentic-browsing")),
}
...
CAPS = {
    ...
    "web-quality": ("domain-discovery", "accessibility", "web-performance", "standards", "content-suggestions", "quality"),
    "seo-geo": ("domain-discovery", "web-performance", "content-suggestions", "ai-visibility"),
    "seo-geo-performance": ("domain-discovery", "web-performance", "content-suggestions", "ai-visibility"),
    ...
    "complete-safe": ("domain-discovery", "accessibility", "web-performance", "standards", "content-suggestions", "ai-visibility", "observability", "quality"),
    "complete-maximum": tuple(item.id for item in AUDIT_CAPABILITIES),
}
LABELS = {
    "seo": "SEO / Search Readiness", "geo": "GEO / AI Readiness", "performance": "Performance",
    "accessibility": "Acessibilidade", "web-quality": "Web Quality", "seo-geo": "SEO + GEO",
    "seo-geo-performance": "SEO + GEO + Performance", "search-intelligence": "Search Intelligence / SERP",
    "apdex-navigation": "Apdex de navegação", "apdex-experience": "Apdex de experiência",
    "deep-analysis": "Análise profunda URL", "complete-safe": "Completo seguro", "complete-maximum": "Completo máximo",
}
...
def _manual(session: Any, state: Any, domain: str) -> bool:
    ...
    if domain == "web":
        return bool(getattr(state, "web_performance", False)) != bool(baseline.get("web_performance", False))
    if domain == "categories":
        return str(getattr(state, "lighthouse_categories", "")) != str(baseline.get("lighthouse_categories", ""))
    return False


def _merge_categories(current: str, additions: tuple[str, ...]) -> str:
    old = [item.strip() for item in str(current or "").split(",") if item.strip()]
    wanted = set(old) | set(additions)
    return ",".join([item for item in _CATEGORY_ORDER if item in wanted] + [item for item in old if item not in _CATEGORY_ORDER])
...
def effective_profile(state: Any, session: Any | None = None) -> Iterator[None]:
    ...
    try:
        ...
        if categories and not _manual(current, state, "categories"):
            state.lighthouse_categories = _merge_categories(getattr(state, "lighthouse_categories", ""), categories)
        if getattr(current, "ai_mode", profiles.AI_OFF) == profiles.AI_IF_AVAILABLE and "ai" not in current.manual_overrides:
        ...
        if "deep-analysis" in capabilities and "deep" not in current.manual_overrides:
    ...
    with effective_profile(state, current):
        for capability_id in tuple(getattr(current, "modules", ()) or ()):
            ...
            if capability_id in NON_BLOCKING_PROFILE_CAPABILITIES:
                ...
                advisories.append(f"{capability.label}: {detail}")
            elif status not in {"APTO", "INCLUÍDO", "AUTOMÁTICO", "DERIVADO", "PERSONALIZADO", "HABILITADO"}:
                blockers.append(f"{capability.label}: {detail}")
    if str(getattr(current, "profile_id", "")) in {"geo", "seo-geo", "seo-geo-performance", "complete-safe", "complete-maximum"}:
        try:
            context = configured_content_analysis_context()
        except ValueError as exc:
            blockers.append(f"Contexto editorial: {exc}")
        else:
            advisories.append("Contexto editorial/YMYL permanece AUTO; nenhum valor é inventado." if context.is_fully_auto else "Contexto editorial explícito da sessão será preservado.")
    try:
...
def cost_lines(state: Any, session: Any) -> tuple[str, ...]:
    ...
    with effective_profile(state, session):
        lines.append(f"Exposição estimada do core/IA/Web Performance: {estimate_exposure(state).level}")
        if "apdex-navigation" in session.modules or "apdex-experience" in session.modules:
            attempts, load = synthetic_load_summary(state)
            lines.append("Carga sintética: " + (load if attempts else "depende da parametrização vigente; o perfil não inventa carga"))
    for capability_id in session.modules:
        note = CAPABILITY_BY_ID[capability_id].cost_note
        if note and note not in lines:
            lines.append(note)
    lines.append("Integrações opt-in não são ativadas pelo perfil; credenciais e políticas próprias são preservadas.")
    return tuple(lines)
...
def render_detail(state: Any, session: Any) -> None:
    ...
    print(f"Uso esperado      : {meta.purpose if meta else 'Composição personalizada de capacidades.'}")
    print(f"Resultado esperado: {meta.result if meta else 'Somente workloads selecionados, além do core automático.'}")
    print("\nCAPACIDADES")
    with effective_profile(state, session):
        for capability_id in session.modules:
            capability = _ui_capability(capability_id)
    ...
    for line in cost_lines(state, session):
        print(f"- {line}")
    print(paint("\nO perfil é aditivo: escolhas já feitas na sessão não são desligadas.", GREEN, bold=True))
    if not ready:
...
def choose_ai_mode(state: Any) -> str | None:
    ...
    print("1. Não adicionar IA pelo perfil (preservar a seleção atual da sessão)")
    print("2. Usar IA principal/AUTO somente se a sessão estiver sem IA e houver provider APTO")
    print("V. Voltar")
    raw = input("Escolha: ").strip().upper()
    if raw == "1": return profiles.AI_OFF
    if raw == "2": return profiles.AI_IF_AVAILABLE
...
def custom_capabilities(state: Any) -> tuple[str, ...] | None:
    ...
    while True:
        ...
        print("Automáticas/derivadas continuam incluídas pelo core e não recebem checkbox.\n")
        for index, item in enumerate(PROFILE_SELECTABLE, 1):
            print(f" {index:2d}. [{' *'[item.id in selected]}] {item.label}\n     Resultado: {item.expected_result}")
        ...
        try:
            ...
            continue
        if item.id == "apdex-navigation" and item.id in selected and "apdex-experience" in selected:
        ...
        if item.id in selected:
            ...
            if item.id == "apdex-experience": selected.add("apdex-navigation")
        state.error = ""
...
def configure_profile(state: Any) -> None:
    ...
    if str(getattr(state, "input_mode", "")).casefold() != "url" or not str(getattr(state, "target", "")).strip():
        ...
        return
    while True:
        print("\nPERFIS DA PRÓXIMA AUDITORIA — CAPACIDADES CANÔNICAS\n")
        print(paint("O perfil adiciona intenção; não desliga opções já definidas na sessão.", CYAN, bold=True))
        ...
        print("\n C. Personalizado por capacidades\n N. Remover perfil da sessão\n V. Voltar")
        raw = input("Escolha: ").strip().upper()
        if raw == "V": return
        if raw == "N":
            profiles.clear_profile(state); state.error = ""; state.operation = "LOCAL:EXECUTION_PROFILE_NONE"; return
        if raw == "C":
            capabilities = custom_capabilities(state)
            if capabilities is None: continue
            candidate = profiles.SessionProfile("custom", "Personalizado", capabilities, profiles.AI_OFF, profiles._baseline(state))
            readiness.set_gsc_profile_policy(candidate, readiness.GSC_PROFILE_IF_COMPATIBLE)
        else:
            try: definition = profiles.PROFILES[int(raw) - 1]
            except (ValueError, IndexError):
                state.error = "perfil inválido"; continue
            candidate = readiness._candidate(state, definition)
        if "deep-analysis" in candidate.modules:
            candidate.ai_mode = profiles.AI_IF_AVAILABLE
            print(paint("\nIA principal é obrigatória para Análise profunda; a seleção atual da sessão é preservada.", CYAN))
        else:
        ...
        ready, blockers, _ = profiles.dependency_status(state, candidate)
        if not ready:
            state.error = "; ".join(blockers); continue
        if not readiness._choose_gsc_policy(state, candidate): continue
        ready, blockers, _ = profiles.dependency_status(state, candidate)
        if not ready:
            state.error = "; ".join(blockers); continue
        render_detail(state, candidate)
        readiness._print_gsc_policy_detail(state, candidate)
        print("\nA. Aplicar à sessão\nV. Voltar sem alterar")
        if input("Escolha: ").strip().upper() != "A": continue
        profiles._SESSIONS[id(state)] = candidate
        state.error = ""; state.operation = "LOCAL:EXECUTION_PROFILE_SELECTED"; return
...
def profile_summary(state: Any) -> None:
    ...
    if current is None:
        print(f"F. Perfil da execução     : {paint('NENHUM', DIM)} | sessão preservada" if allowed else f"F. Perfil da execução     : {paint('INDISPONÍVEL', RED, bold=True)} | informe URL única")
        return
    ...
    for item in advisories: print(paint(f"   INFO                   : {item}", CYAN))
...
def _install_catalog() -> None:
    from rasai import console_execution_profiles as profiles
    from rasai import console_ui_catalog as ui
    handlers = {"web-performance": "6", "search-intelligence": "T", "apdex-navigation": "11", "apdex-experience": "11", "deep-analysis": "13", "remediation": "5"}
    ui.CAPABILITIES = tuple(ui.CapabilityUI(item.id, item.label, f"{item.purpose} Resultado esperado: {item.expected_result}", handlers.get(item.id), item.automatic, item.derived) for item in AUDIT_CAPABILITIES)
    profiles.MODULES = tuple(profiles.ExecutionModule(item.id, item.label, item.purpose, (), item.cost_note, "Usa o contrato canônico desta capacidade; o perfil não inventa credenciais/inputs.") for item in AUDIT_CAPABILITIES)
    profiles.MODULE_BY_ID = {item.id: item for item in profiles.MODULES}
    profiles.PROFILES = tuple(profiles.ExecutionProfile(pid, LABELS[pid], UX[pid].purpose, capability_ids) for pid, capability_ids in CAPS.items())
    profiles._PROFILE_BY_ID = {item.id: item for item in profiles.PROFILES}


def install(console_module: Any) -> None:
    global _INSTALLED
    if _INSTALLED or getattr(console_module, "_rasai_profile_capability_architecture", False): return
    _install_catalog()

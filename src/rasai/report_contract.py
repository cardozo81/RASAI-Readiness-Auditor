...
REPORT_SURFACES: tuple[ReportSurface, ...] = (
    ReportSurface(
        ...
        optional_dependencies=(
            "domínios externos, quando habilitados nesta auditoria",
            "resultados de IA, quando habilitados nesta auditoria",
        ),
        ai_usage="Somente projeta resultados de IA já persistidos; não chama IA.",
        score_impact="Nenhum; apenas apresenta o Índice de Prontidão Search & IA e a pontuação persistida.",
    ),
    ReportSurface(
        id="readiness",
        filename="readiness.html",
        label="Índice de Prontidão Search & IA",
        optional=False,
        inputs=("scores", "Coverage", "Confidence", "Consolidation", "evidências de regras"),
        outputs=("Índice de Prontidão Search & IA", "dimensões de prontidão", "resultado geral persistido"),
        ...
        score_impact="É a projeção pública do Índice de Prontidão Search & IA; SARI-001 e scoring_version permanecem como rastreabilidade técnica.",
    ),
    ReportSurface(
        id="scoring",
        filename="scoring.html",
        label="Método de Pontuação de Prontidão",
        optional=False,
        inputs=("scoring_version persistido", "scores", "Coverage", "Confidence", "Consolidation"),
        outputs=("fórmula", "dimensões", "pesos", "critérios críticos", "limitações", "reprodutibilidade"),
        required_dependencies=("audit.db",),
        ...
        score_impact="Define e explica o Método de Pontuação de Prontidão aplicado à auditoria; não recalcula a pontuação ao renderizar HTML.",
    ),
    ReportSurface(
        id="context",
    ...
    ReportSurface(
        ...
        filename="execution-evidence.html",
        label="Evidências da execução",
        optional=False,
        inputs=(
            "snapshot secret-free da configuração efetiva",
            "work-items e tentativas de fulfillment",
            "estados persistidos de coletores e integrações",
        ),
        outputs=(
            "matriz de capacidades possíveis versus solicitadas",
            "estado concluído/parcial/falha/não solicitado",
            "referência técnica do erro",
            "nomes de variáveis/configurações faltantes quando persistidos",
            "integridade hash do snapshot de configuração",
        ),
        required_dependencies=("audit.db",),
        ai_usage="Nenhum. A página apenas projeta configuração e estados já persistidos.",
        score_impact="Nenhum; é uma superfície de proveniência operacional e não recalcula score, fulfillment ou evidências.",
        source_of_truth="audit.db: audit_execution_configurations + audit_fulfillment_* + estados persistidos dos coletores",
    ),
    ReportSurface(
        id="mobile",
    ...
    ReportSurface(
        id="improvement-intelligence",
        filename="improvement-intelligence.html",
        ...
        ai_usage="Quando habilitada, executa análise estruturada própria com provider/modelo/esforço escolhidos para esta finalidade, reutilizando somente a credencial já configurada. Cada tentativa é registrada em ai_provider_attempts.",
    ...
    ReportSurface(
        ...
        optional=True,
        inputs=("artifacts/observability/observability.db", "artifacts/observability", "fontes externas configuradas"),
        ...
        source_of_truth="artifacts/observability/observability.db + artifacts/observability",
    ),
    ReportSurface(
        id="quality",

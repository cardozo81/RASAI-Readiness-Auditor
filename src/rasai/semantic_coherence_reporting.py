...
def _status_display(evidence: Any, value: Any) -> Any:
    return evidence._state_text(value, _status(value))
...
def _context_html(evidence: Any, database: Any, audit_id: str) -> str:
    ...
    if property_rows:
        ...
        for key,label in _PROPERTY_LABELS.items():
            ...
            property_values.append((label,"Automático" if raw.casefold()=="auto" else raw,"Automático" if raw.casefold()=="auto" else "Declarado"))
    else:
        property_values=[(label,"Automático","Automático") for label in _PROPERTY_LABELS.values()]
    ...
    if manifest_rows:
        ...
        manifest_status=_status_display(evidence,manifest.get("status"))
        blocks.append("<div class='notice'><strong>Gate semântico:</strong> "+f"{manifest_status}. O contexto foi congelado antes da primeira chamada de IA; corpus com {page_count} página(s) e {snapshot_count} captura(s). A agregação entre páginas usa apenas resultados já persistidos e não gera uma chamada adicional.</div>")
    return "".join(blocks)
...
def _auto_interpretation_html(evidence: Any, database: Any, audit_id: str) -> str:
    ...
    if not rows:
        return "<div class='subsection'><h3>Interpretações de contexto por IA</h3><div class='notice'>Nenhuma interpretação automática estruturada foi persistida nesta execução. Campos configurados explicitamente continuam sendo a fonte canônica de contexto.</div></div>"
    ...
    for index,row in enumerate(rows,1):
        ...
        table_rows.append((
            ...
            _CONTENT_LABELS.get(field,public_label(field) or field.replace("_"," ").title()),
        ...
        body+="<p class='muted'>A configuração humana permanece canônica. Registros não aplicados são mantidos para auditabilidade da execução e não são apresentados como configuração automática efetiva.</p>"
    ...
    return (
        ...
        "<p class='muted'>A coluna Aplicada? distingue a interpretação automática efetivamente usada de registros preservados apenas como provenance. Campos declarados pelo usuário exibem seu valor canônico real.</p>"
...
def _ymyl_alignment_html(evidence: Any, database: Any, audit_id: str) -> str:
    ...
    if str(ymyl.get("configured_category") or "auto").casefold() == "auto":
        category_origin = (
            ...
            else "Automático - não determinável"
        )
    ...
    rows = [
        ...
        (
            ...
            "Configuração canônica × modo Automático",
        ),
        (
    ...
    for item in ymyl.get("coherence", []) or []:
        details.append(
            (
                ...
                _status_display(evidence, item.get("result")),
    ...
    return (
        ...
        + detail_html
        + "<p class='muted'>A configuração humana permanece canônica. Interpretações automáticas são inferência de IA "
...
def _property_summary_html(evidence: Any, database: Any, audit_id: str) -> str:
    ...
    for index,row in enumerate(summaries,1):
        ...
        result_display=_status_display(evidence,row.get("result"))
        table_rows.append((_PROPERTY_COHERENCE_LABELS.get(criterion,PROPERTY_COHERENCE_CRITERIA.get(criterion,criterion)),result_display,_confidence(row.get("confidence")),int(row.get("observation_count") or 0),evidence._modal_button(modal_id,"Ver análise")))
        ids=_json_list(row.get("evidence_ids_json"))
        body=evidence._kv((("Critério",criterion),("Resultado",result_display),("Confiança agregada",_confidence(row.get("confidence"))),("Páginas/observações consideradas",row.get("observation_count") or 0),("Leitura",row.get("summary") or "-"),("Evidências relacionadas",", ".join(str(item) for item in ids) or "-")))
...
def _page_summary_html(evidence: Any, database: Any, audit_id: str) -> str:
    ...
    for index,criterion in enumerate(PAGE_COHERENCE_CRITERIA,1):
        ...
        state_token="INCOHERENT" if counts.get("INCOHERENT") else "PARTIAL" if divergence else "COHERENT" if items else "NOT_DETERMINABLE"
        state=_status(state_token)
        state_display=evidence._state_text(state_token,state)
        table_rows.append((_PAGE_COHERENCE_LABELS.get(criterion,PAGE_COHERENCE_CRITERIA[criterion]),state_display,_confidence(confidence),divergence,evidence._modal_button(modal_id,"Ver páginas")))
        details=[(item.get("page_url") or "-",_status_display(evidence,item.get("result")),_confidence(item.get("confidence")),item.get("observed_context") or "-") for item in sorted(items,key=lambda value:(str(value.get("page_url")),str(value.get("snapshot_id"))))]

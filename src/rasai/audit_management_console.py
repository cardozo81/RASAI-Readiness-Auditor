...
def _configure_filters(current: AuditInventoryFilter) -> AuditInventoryFilter:
    print("\nFILTROS - ENTER remove o campo; V cancela toda a edição sem alterar os filtros atuais")
    try:
        date_from = _parse_date(prompt_text(
            "Data inicial (AAAA-MM-DD)",
            current=current.date_from.isoformat() if current.date_from else "",
            empty_keeps_current=False,
        ))
        date_to = _parse_date(prompt_text(
            "Data final (AAAA-MM-DD)",
            current=current.date_to.isoformat() if current.date_to else "",
            empty_keeps_current=False,
        ))
        ...
        domain = prompt_text(
            "Domínio contém",
            current=current.domain or "",
            empty_keeps_current=False,
        ).strip() or None
        project = prompt_text(
            "Projeto contém",
            current=current.project or "",
            empty_keeps_current=False,
        ).strip() or None
        status = prompt_text(
            "Status exato",
            current=current.status or "",
            empty_keeps_current=False,
        ).strip() or None
    except EditCancelled:
        print("Edição de filtros cancelada. Os filtros anteriores foram mantidos.")
        return current
    except ValueError:
...
def _impact_summary(impact: DeletionImpact) -> None:
    print("\n" + warning_text("IMPACTO DA EXCLUSÃO", bold=True))
    ...
    print(paint(
        "Registros históricos já existentes no control plane não são apagados por esta operação local.",
        GRAY,
    ))
...
def _delete(
    ...
    *,
    ...
    if not confirm_sensitive(
        required,
        action_label="Excluir permanentemente as auditorias e evidências listadas",
    ...
    console_module.render_header(state)
    print(title_text("RESULTADO DA EXCLUSÃO") + "\n")
    print("Status        : " + semantic_text(result.status, bold=True))
    ...
    if result.rollback_failures:
        print("\n" + paint("ATENÇÃO: o rollback não conseguiu restaurar todos os itens:", RED, bold=True))
...
def _management_menu(console_module: ModuleType, state: Any) -> None:
    ...
    while True:
        ...
        console_module.render_header(state)
        print(title_text("GERENCIAR AUDITORIAS / EXCLUSÃO SEGURA") + "\n")
        ...
        if visible:
            ...
            for absolute_index, item in enumerate(visible, start + 1):
                selected_now = item.audit_id in selected
                marker = paint("[x]", YELLOW, bold=True) if selected_now else paint("[ ]", GRAY)
                status = item.completion_status or item.status or "-"
                status_cell = semantic_text(f"{status[:18]:<18}", bold=selected_now)
                audit_cell = (
                    paint(f"{item.audit_id:<30}", YELLOW, bold=True)
                    if selected_now
                    else f"{item.audit_id:<30}"
                )
                print(
                    f"{absolute_index:3d} {marker} {audit_cell} {_date_label(item):<10} "
                    f"{status_cell} {_size_label(item.size_bytes):>10}  {_domains_label(item)}"
                )
        else:
        ...
        print("X. " + destructive_text("Excluir AUDs selecionados"))
        print("T. " + destructive_text("Excluir TODOS os AUDs existentes"))
        ...
        if choice == "R":
            ...
            console_module.render_header(state)
            print(warning_text("LIMPEZA FÍSICA PENDENTE", bold=True) + "\n")
            print(f"Lotes removidos : {result.removed_batches}")
            print("Lotes pendentes : " + (paint(str(result.pending_batches), YELLOW, bold=True) if result.pending_batches else paint("0", GREEN, bold=True)))
            print("Espaço pendente : " + (paint(_size_label(result.pending_bytes), YELLOW, bold=True) if result.pending_bytes else paint(_size_label(0), GREEN)))

...
Essa é a superfície canônica de dados `ORIGIN`. `capture-context.html` e as demais páginas CAT não repetem o conteúdo detalhado desses recursos. Quando necessário para interpretação, exibem somente referência curta para **Domínio e descoberta**.
...
Quando Mobile e Desktop foram executados, `capture-context.html` compara os hashes do documento fonte quando ambos estão disponíveis e informa se o documento recebido foi igual, diferente ou se a comparação ficou inconclusiva.
...
A auditoria possui `report-catalog/capture-context.html`, responsável por:
...
A navegação do mini-site segue `src/rasai/catalog_report_contract.py`: visão geral e SARI, CAT-01 a CAT-10, Análise Direcionada e páginas de governança. O contexto Mobile/Desktop é propriedade das páginas, snapshots e evidências persistidos; não cria páginas audit-owned independentes.

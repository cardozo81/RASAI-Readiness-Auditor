...
_STATUS_FAILURE = {"FAILED_RETRYABLE","FAILED_TERMINAL","FAILED_PERMANENT","FAILED_FATAL","BLOCKED","ERROR","FAILURE","UNAVAILABLE","CONTRACT_ERROR"}
_STATUS_PENDING = {"PENDING","RUNNING","WAITING_FOR_DATA","NOT_CONFIGURED","REQUESTED_NOT_EXECUTED","PROCESSING","PARTIAL"}
_STATUS_NEUTRAL = {"DISABLED","NOT_APPLICABLE","NOT_REQUESTED","SKIPPED","ABSENT","NO_DATA"}
...
_WEB_METRICS = (
    ("performance_score","Lighthouse · Desempenho","índice"),
    ("accessibility_score","Lighthouse · Acessibilidade","índice"),
    ("best_practices_score","Lighthouse · Boas práticas","índice"),
    ("seo_score","Lighthouse SEO","índice"),
    ("agentic_browsing_score","Lighthouse · Navegação por agentes","índice"),
    ("fcp_lab_ms","FCP de laboratório","ms"),("speed_index_lab_ms","Índice de velocidade (Speed Index)","ms"),
    ("lcp_lab_ms","LCP de laboratório","ms"),("tbt_lab_ms","Tempo total de bloqueio (TBT)","ms"),
...
def _load_data(
    ...
    *,
    ...
    try:
        ...
        configuration,persisted,computed=_load_configuration(con,audit_id)
        ...
        fulfillment=_last(con,"audit_fulfillment_contracts",audit_id)
        return _ReportData(audit_id, audit_rows[0] if audit_rows else {}, configuration,persisted,computed,selected,items,tables,scores,work,fulfillment,_targets(con,configuration,audit_id))
    finally:
        if own_connection:
            con.close()

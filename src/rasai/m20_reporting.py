...
def enrich_m20_report_site(*, audit_id: str, workspace: AuditWorkspace) -> Path:
    ...
    for html_path in report_dir.glob("*.html"):
        ...
        if html_path.name == "remediation.html" and "content-suggestions-link" not in html:
            html = html.replace(
                "</header>",
                "</header><section id='content-suggestions-link' class='notice'><strong>Conteúdo, contexto editorial e Structured Data:</strong> "
                f"<a href='{CONTENT_FILE}'>abrir análise e sugestões por página →</a></section>",
                1,
            )
        if html_path.name == "ai-usage.html" and "remediation-ai-telemetry" not in html:
...
def _content_page(data: dict[str, Any], report_dir: Path) -> str:
    ...
    for (url, device), rows in by_page.items():
        ...
        for row in rows:
            ...
            items.append(f"""<details><summary>{escape(str(row['rule_id']))} · {escape(str(row['finding_title']))}</summary><div class='detail-body'><p><strong>Objetivo:</strong> {escape(str(row['objective']))}</p><p><strong>Local sugerido:</strong> {escape(str(row['target_location']))}</p><div class='notice ai-provenance'><strong>Output gerado por IA.</strong> Este texto foi produzido pelo provider/modelo identificado abaixo a partir do finding/regra persistidos, da URL e dispositivo desta ocorrência, dos evidence_ids exibidos e do contexto editorial persistido quando aplicável. O RASAi valida o contrato da resposta, mas o conteúdo continua advisory e exige revisão humana.</div><h5>Texto proposto pela IA</h5><pre>{escape(str(row['proposed_text']))}</pre><div class='remediation-grid'><div><small>Provider / modelo</small><strong>{escape(str(row['provider']))} / {escape(str(row['model'] or '-'))}</strong></div><div><small title='Confiança desta sugestão específica; não é a Confiança global do Método de Pontuação de Prontidão (contrato técnico SCORE-GEO-004).'>Confiança da sugestão ⓘ</small><strong>{float(row['confidence'])*100:.0f}%</strong></div><div><small>Evidências</small><strong class='mono'>{escape(evidence)}</strong></div></div><div class='notice warn'><strong>Revisão humana obrigatória:</strong> {escape(str(row['review_note']))}</div></div></details>""")

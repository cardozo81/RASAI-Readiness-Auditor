...
def _capture_context_body(database: Path, data: _ReportData) -> str:
    ...
    for i,s in enumerate(snaps,1):
        ...
        artifact_rows=[
            ...
            ("Captura visual",meta.get("visual_artifact_ref") if isinstance(meta,Mapping) else "—"),
        ]
    ...
    body+=_section("capture","Capturas da auditoria",_table(("Dispositivo","URL solicitada","URL final","HTTP","Diagnósticos","Detalhe"),rows,empty="Nenhuma captura de navegador persistida.")+"".join(modals))
...
def _overview_body(database: Path, data: _ReportData) -> str:
    ...
    body+=_section("catalogs","Catálogos da auditoria",_table(("Catálogo","Estado","Interpretação"),rows))
    body+=_section("indices","Índices e métricas evidentes",_table(("Indicador","Valor","Contexto","Tipo","Confiança"),indexes,empty="Nenhum índice ou métrica reconhecido foi persistido.")+"<p class='muted'>O inventário metodológico completo está em <a href='metrics.html'>Índices e métricas</a>.</p>")
...
def _execution_evidence_body(database: Path, data: _ReportData) -> str:
    ...
    for i,r in enumerate(data.work_items,1):
        mid=f"fulfillment-{i}"
        work_rows.append((_friendly_component(r.get("component")),_status_label(r.get("status")),_attempt_count_label(r.get("attempt_count")),_modal_button(mid,"Ver etapa")))
        modals.append(_modal(mid,_friendly_component(r.get("component")),"Etapa persistida da execução",_kv((("Estado",_status_label(r.get("status"))),("Tentativas registradas na etapa",_attempt_count_label(r.get("attempt_count"))),("Obrigatória","Sim" if r.get("required") else "Não"),("Escopo técnico",r.get("scope_key") or "—"),("Resultado",r.get("effective_result_ref") or "—"),("Último erro",r.get("last_error_message") or r.get("last_error_code") or "—"),("Identificador",r.get("work_item_id") or "—")))))
    body=_audit_hero(data,"Evidências da execução","O que foi solicitado, o estado de cada catálogo e as etapas técnicas persistidas.")
    body+=_outline((("matrix","Plano × execução"),("integrity","Plano congelado"),("technical","Etapas técnicas")))
    body+=_section("matrix","Plano × execução",_table(("Catálogo","Contexto","Selecionado","Estado","Interpretação"),rows))
    ...
    body+=_section("technical","Etapas técnicas",_table(("Etapa","Estado","Tentativas","Detalhe"),work_rows,empty="Nenhuma etapa técnica persistida.")+"".join(modals))
    return body

...
O baseline é uma metodologia proprietária do RASAi e contém heurísticas internas para sinais semânticos que não possuem um standard GEO/AEO universal. Ele deve ser interpretado junto com `report-catalog/methodology.html`, as referências técnicas das páginas CAT aplicáveis e a classificação `basis` de cada regra.

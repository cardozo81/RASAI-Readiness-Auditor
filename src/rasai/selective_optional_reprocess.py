...
def _reconcile_improvement_rpr(workspace: Any, audit_id: str) -> None:
    """Project CAT-08 using the execution-local RPR AI policy when present."""
    from rasai.audit_fulfillment import NOT_CONFIGURED
    ...
    if policy.use_ai is False:
        # An RPR that did not authorize AI must not rewrite this pending item.
        return

    existing = dict(getattr(item, "configuration", {}) or {})
    if policy.use_ai is True:
        provider, model, reasoning = provider_override()
        effective_provider = str(provider or "none").casefold()
        ...
        if effective_provider in {"", "none"}:
            set_work_item_status(
                ...
                error_message=(
                    "CAT-08 foi selecionado neste RPR, mas nenhum provider de IA "
                    "foi configurado para esta execução"
                ),
                retryable=True,
            )
            return
    else:
        # Compatibility path for callers that do not supply the new RPR policy.
        try:
    ...
    if status == "COMPLETE":
        ...
        return
    if status == "NOT_CONFIGURED" or reason in {"AI_NOT_CONFIGURED", "AI_PROVIDER_NOT_CONFIGURED"}:
        set_work_item_status(
            workspace,
            audit_id=audit_id,
            component="IMPROVEMENT_INTELLIGENCE",
            status=NOT_CONFIGURED,
            error_class="CONFIGURATION",
            error_code=reason or "AI_NOT_CONFIGURED",
            error_message="CAT-08 não executou porque a IA deste RPR não estava configurada/apta",
            retryable=True,
        )
        return
    set_work_item_status(
...
def _augment_reconciliation_configuration() -> None:
    ...
    if not bool(getattr(improvement, "_rasai_optional_config", False)):
        def reconcile_improvement(workspace: Any, audit_id: str) -> None:
            ...
            if active_context is not None and active_context.audit_id == audit_id:
                if should_execute("IMPROVEMENT_INTELLIGENCE"):
    ...
    if not bool(getattr(services, "_rasai_optional_config", False)):
        def reconcile_services(workspace: Any, audit_id: str) -> None:
            active_context = current()
            if active_context is not None and active_context.audit_id == audit_id:
                if should_execute("GOOGLE_SEARCH_CONSOLE"):
    ...
    def reprocess_with_optional_components(
        ...
        *,
        ...
        pending = {
            ...
            if bool(item.required) and item_executable(item)
        }

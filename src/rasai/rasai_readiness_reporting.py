...
identifier for the current Search & AI Readiness Index presentation. Scoring is versioned independently and uses ``SCORE-GEO-003`` as the method applied by the current audit pipeline.
...
LEGACY_RASAI_FILE = LEGACY_READINESS_REPORT
...
def enrich_rasai_reporting(*, audit_id: str, workspace: AuditWorkspace) -> Path:
    ...
    legacy_readiness_path = report_dir / LEGACY_RASAI_FILE
    if legacy_readiness_path.is_file():
        legacy_readiness_path.unlink()
    _register_navigation()
    data = _load(audit_id, workspace)

    rasai_path = report_dir / RASAI_FILE
    rasai_path.write_text(
        _rasai_page(data, workspace, report_dir),
        encoding="utf-8",
        newline="\n",
    )
    ...
    legacy_path = report_dir / LEGACY_RASAI_FILE
...
<meta http-equiv='refresh' content='0; url={RASAI_FILE}'>
<link rel='canonical' href='{RASAI_FILE}'>
<title>RASAi - readiness</title></head>
<body><p>Abrindo <a href='{RASAI_FILE}'>{RASAI_FILE}</a>.</p></body></html>\n"""
...
def _rewrite_index(html: str, data: dict[str, Any], report_dir: Path) -> str:
    ...
    html = re.sub(
        r"<!-- rasai-m23-index-start -->.*?<!-- rasai-m23-index-end -->",
        "",
        html,
        flags=re.DOTALL,
    )
    html = re.sub(
        r"<!-- rasai-external-metrics-integrity:start -->.*?<!-- rasai-external-metrics-integrity:end -->",
        "",
        html,
        flags=re.DOTALL,
    )
...
def _rewrite_device_page(html: str, filename: str) -> str:
    ...
    if marker not in html:
        notice = (
            ...
            f"Score, Coverage, Confidence e dimensões RASAi estão centralizados em <a href='{RASAI_FILE}'>Search & AI Readiness</a>.</section>"
        )

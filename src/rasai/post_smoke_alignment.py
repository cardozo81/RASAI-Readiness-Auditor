...
def _cat08_ai_authorized(workspace: Any, audit_id: str) -> bool:
    """Return AI authorization for CAT-08 in the active execution.

    An explicit RPR policy belongs to the reprocess execution and therefore takes
    precedence over the immutable AUD snapshot. Without an RPR decision, preserve the
    original AUD authorization semantics.
    """
    try:
        from rasai.reprocess_policy import current_policy

        policy = current_policy()
        if policy.use_ai is not None:
            return bool(policy.use_ai)
    ...
    for raw in catalog.get("items") or ():
        if not isinstance(raw, Mapping):
            continue
        if str(raw.get("id") or raw.get("catalog_id") or "").upper() != "CAT-08":
            continue
        if "ai_execution_enabled" in raw:
            return bool(raw.get("ai_execution_enabled"))
    if "ai_enabled" in catalog:
        return bool(catalog.get("ai_enabled"))
    # Compatibility with AUD snapshots created before explicit execution authorization.
    return True


def _primary_ai_context(workspace: Any, audit_id: str) -> tuple[str, str, str] | None:
    # RPR policy is execution-local and must never overwrite the AUD's original
    # provider configuration. When present, it takes precedence only for this run.
    try:
        from rasai.reprocess_policy import current_policy, provider_override

        policy = current_policy()
        if policy.use_ai is False:
            return None
        if policy.use_ai is True:
            provider, model, reasoning = provider_override()
            if provider in {None, "", "none"}:
                return None
            return (provider, model or "", reasoning or "")
    except (ImportError, AttributeError):
        pass
...
def _install_improvement_governed_hook() -> None:
    ...
    def governed(*, audit_id: str, workspace: Any, evidence_snapshot: Any):
        ...
        if required and not _cat08_ai_authorized(workspace, audit_id):
            set_work_item_status(
                workspace,
                audit_id=audit_id,
                component="IMPROVEMENT_INTELLIGENCE",
                ...
                error_message=(
                    ...
                    "autorizado nesta AUD. Reprocesse a auditoria e autorize IA quando desejar "
            ...
            try_append_operational_event(
                workspace,
                "IMPROVEMENT_INTELLIGENCE_NOT_AUTHORIZED",
                level="INFO",
                audit_id=audit_id,
                provider_called=False,
                reason="AI_NOT_AUTHORIZED_FOR_EXECUTION",
            )
            return {
                "status": "REQUESTED_NOT_EXECUTED",
                "reason": "AI_NOT_AUTHORIZED_FOR_EXECUTION",
        ...
        if required:
            from rasai.reprocess_policy import blocking_dependencies
            ...
            current_item = next(
                (
                    ...
                    and str(item.scope_key) == "AUDIT"
                ),
                None,
            )
            blockers = (
                blocking_dependencies(workspace, current_item)
            ...
            if blockers:
                ...
                try_append_operational_event(
                    workspace,
                    "IMPROVEMENT_INTELLIGENCE_WAITING_FOR_DATA",
                    level="INFO",
                    audit_id=audit_id,
                ...
                return {
                    "status": "WAITING_FOR_DATA",
                    "reason": "AI_PREREQUISITES_INCOMPLETE",
                    "dependencies": list(blockers),
                    "provider_called": False,
                }

        context = _primary_ai_context(workspace, audit_id)
        token = orchestration._PRIMARY_AI_CONTEXT.set(context)
        ...
        if required and status not in {"COMPLETE", "SUCCESS"}:
            not_configured = reason in {"AI_NOT_CONFIGURED", "AI_PROVIDER_NOT_CONFIGURED"}
            set_work_item_status(
                workspace,
                audit_id=audit_id,
                component="IMPROVEMENT_INTELLIGENCE",
                status=NOT_CONFIGURED if not_configured else FAILED_RETRYABLE,
                error_class="CONFIGURATION" if not_configured else "AI_ANALYSIS",
                error_code=str(result.get("reason") or status or "IMPROVEMENT_INCOMPLETE"),
                error_message=(
                    "CAT-08 requer IA para fechamento integral, mas nenhum provider apto foi configurado nesta execução"
                    if not_configured
                    else str(
                        result.get("reason")

...
Validar em `cat-09.html`: finding, objetivo, localização, texto proposto, evidence IDs, provider/model, revisão humana. Em `ai-integrations.html`, validar telemetria Sugestões e remediação de conteúdo por IA.
...
## 9. CSS, confiança e referências

Validar CSS externo, ausência de `<style>` final, explicação de Coverage/Confidence em `sari.html`/`methodology.html` e fontes oficiais nas páginas CAT aplicáveis.

...
def _source_states(database: Path, data: Any) -> tuple[SourceState, ...]:
    ...
    serp_state = SourceState(
        ...
        detail="Inteligência de busca / SERP solicitada na AUD" if serp_requested else "Inteligência de busca / SERP não solicitada na AUD",
    ).validate()
...
def _source_state_display(page: Any, state: SourceState) -> Any:
    label=_state_label(state)
    if not state.requested:
        tone="neutral"
    elif not state.executed:
        tone="warn"
    elif state.error_count and state.result_count == 0:
        tone="bad"
        ...
        tone="warn"
    return page._Html(f"<span class='state-text {tone}'>{escape(label)}</span>")
...
def _scope_html(database: Path, data: Any) -> str:
    ...
    for state in _source_states(database, data):
        rows.append((
            public_label(state.source_id) or state.source_id.replace("_", " ").title(),
            ...
            _source_state_display(page,state),
        ))
...
def _serp_html(database: Path, data: Any) -> str:
    ...
    try:
        for index, obs in enumerate(observations, 1):
            ...
            citation_value=("Sim" if brand_present else "Não" if detected else "Não determinável")
            citation_display=(
                page._result_value(citation_value,"good" if brand_present else "warn")
                if detected
                else page._state_text("NOT_DETERMINABLE",citation_value)
            )
            overview_rows.append((obs.get("query") or "-","Sim" if detected else "Não",citation_display,len(references),captured or "-",page._modal_button(overview_id,"Ver AI Overview")))
            overview_body=page._kv((("Detectado","Sim" if detected else "Não"),("Site auditado entre as referências",citation_display),("Provedor",obs.get("provider") or "-"),("Capturado em",captured or "-"),("Artefato",obs.get("raw_evidence_ref") or "-"),("Referências declaradas pelo provider",len(references))))
            if overview:
...
def _competitive_validation_rows(
    ...
    ymyl_mode = configuration_value_report("SEARCH_YMYL_MODE", configuration.get("ymyl_mode") or "auto")
    ...
    try:
        ...
        for index,item in enumerate(analyses,1):
            ...
            body += (
                ...
                + "<p class='muted'>Configurado representa o contrato efetivo persistido no item de trabalho desta AUD. "
            ...
            if isinstance(gaps,list):
                for gap in gaps:
                    ...
                    gap_rows.append((
                        gap.get("code") or "-",
                        page._severity_text(gap.get("severity")) if gap.get("severity") else "-",
                        gap.get("message") or "-",
            ...
            if ai:
                ...
                opportunity_rows=[(
                    page._priority_text(row.get("priority")) if row.get("priority") else "-",
                    row.get("category") or "-",
                    row.get("title") or "-",
        ...
        contract_html=(
            ...
            + page._table(("Parâmetro","Valor efetivo"),_contract_rows(configuration),empty="O item de trabalho da inteligência de busca / SERP não contém configuração competitiva persistida.")

...
def _contract_section(page_name: str) -> str:
    ...
    return (
        ...
        f"<div class='report-contract-item'><h3>Impacto no Índice de Prontidão / Método de Pontuação</h3><p>{escape(surface.score_impact)}</p></div>"
...
def _dependency_map() -> str:
    return (
        ...
        "├-&gt; <strong>Índice de Prontidão / Método de Pontuação</strong> <span class='badge'>regras contratadas</span><br>"
        ...
        "<p class='intro'>Pontuações Lighthouse, Core Web Vitals, Apdex, SERP e telemetria não entram diretamente no Índice de Prontidão Search & IA. Audit-level evidence externa só pode corroborar regra equivalente por mapeamento explícito e sem dupla pontuação.</p></section>"
    )
...
def _sari_method_panel(report_dir: Path, page_name: str) -> str:
    ...
    if state_rows:
        ...
        state_table = (
            ...
            "<div class='table-wrap'><table><thead><tr><th>Dispositivo</th><th>Índice de Prontidão</th><th>Estado</th><th>Discovery</th><th>Indexability</th><th>Extraction</th></tr></thead>"
    ...
    return (
        ...
        "<div class='kicker'>Índice de Prontidão Search & IA · versão pública 001</div>"
        ...
        "<div class='table-wrap'><table><thead><tr><th>Macrocomponente</th><th>Peso no Índice de Prontidão</th><th>Dimensões</th></tr></thead>"
        ...
        "<div class='notice'><strong>Lighthouse:</strong> Performance, Accessibility, Best Practices e SEO continuam métricas externas. As pontuações de categoria não entram na aritmética do Índice de Prontidão; apenas audit-level evidence explicitamente mapeada pode corroborar a mesma condição técnica, sem bônus duplicado.</div>"
...
def install() -> None:
    ...
    report_navigation._RULE_TOOLTIPS["BR-GEO-054"] = (
        ...
        "O Método de Pontuação de Prontidão, versão pública 001, é o método vigente e não depende de artifact de calibração. Contrato técnico: SCORE-GEO-004."
    )

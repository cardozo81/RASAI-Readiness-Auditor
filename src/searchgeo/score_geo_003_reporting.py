...
from searchgeo.score_geo_003_calibration import (
    MIN_DOMAINS, MIN_ENGINES, MIN_OBSERVATIONS, MIN_QUERIES_PER_DOMAIN,
    MIN_REPETITIONS, MIN_VALIDATION_AUC, MIN_VALIDATION_DOMAINS,
)
...
def write_score_geo_003_report(*, audit_id: str, workspace: AuditWorkspace) -> Path:
    ...
    gates = f"{MIN_DOMAINS} domínios; {MIN_VALIDATION_DOMAINS} no holdout; {MIN_ENGINES} engines; {MIN_QUERIES_PER_DOMAIN} queries/domínio; {MIN_REPETITIONS} repetições/query/engine; {MIN_OBSERVATIONS} observações; AUC ≥ {MIN_VALIDATION_AUC:.2f}; Brier melhor que baseline."
...
<header class='hero'><div class='eyebrow'>SearchGEO · método de pontuação</div><h1>SCORE-GEO-003</h1><p class='lead'>Método padrão. As dimensões permanecem determinísticas; o Overall usa regressão logística regularizada calibrada contra presença observada de citação. Sem calibração validada, nenhum Overall é fabricado.</p></header>

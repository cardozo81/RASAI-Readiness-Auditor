...
Os IDs técnicos `BR-GEO-*` permanecem canônicos para rastreabilidade. Na documentação e nas interfaces humanas, o conceito é apresentado como **Regra de Avaliação de Prontidão**; o segmento `GEO` do ID não define o escopo de negócio da regra.

**Estado:** APROVADO / VIGENTE  
**Ruleset vigente:** `BR-GEO-001..060`

Este documento descreve o contrato funcional vigente das **Regras de Avaliação de Prontidão** do RASAi. Identificadores, enums, nomes de campos e valores persistidos permanecem na forma técnica canônica; títulos e explicações são apresentados em português do Brasil.

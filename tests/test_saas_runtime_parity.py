...
def test_durable_execution_contract_rejects_invalid_payload_shapes() -> None:
    ...
    # IA solicitada sem provider operacional é um estado materializável e não
    # invalida o payload determinístico do consolidado.
    validate_execution_job_payload(
        "REPORT_REFRESH",
        {
            "surface": "consolidated",
    ...
    with pytest.raises(ValueError, match="superfícies"):
    ...
    with pytest.raises(ValueError, match="distintos"):
        validate_execution_job_payload(
            ...
            {
                "surface": "consolidated",
                "baseline_audit_id": "AUD-SAME",
                "current_audit_id": "AUD-SAME",
            },
        )

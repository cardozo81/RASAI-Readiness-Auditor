...
def _wrap_m20(original):
    ...
    def execute_m20_with_fulfillment(*args: Any, **kwargs: Any):
        ...
        elif status == "NOT_CONFIGURED" or "NOT_CONFIGURED" in str(reason or "").upper():
            set_work_item_status(
                workspace,
                audit_id=audit_id,
                component="CONTENT_REMEDIATION_AI",
                ...
                error_code=str(reason or "AI_NOT_CONFIGURED"),
                error_message="Remediação de conteúdo por IA solicitada sem provider apto/configurado",
                retryable=True,
            )
        else:
            attempt_id = begin_attempt(
        ...
        if audit_id and workspace is not None and enabled:
            ...
            if state == "AVAILABLE":
                ...
                set_work_item_status(
                    ...
                    error_message="Análise técnica por IA solicitada sem provider apto/configurado",
                    retryable=True,
                )
                ...
                set_work_item_status(
                    ...
                    status=WAITING_FOR_DATA,
                    error_class="PREREQUISITE",
                    error_code="TECHNICAL_EVIDENCE_INSUFFICIENT",
                    error_message="Análise técnica por IA aguarda evidência técnica necessária",
                    retryable=True,
                )
            else:
...
def _install_consolidation_gate() -> None:
    ...
    def candidate_audits_final_only(self: Any, filters: Any):
        ...
        for row in rows:
            ...
            # Full diagnostic closure remains represented by fulfillment's
            # consolidation_eligible flag.  This gate answers a different question:
            # whether an intact AUD can be read as a non-conclusive longitudinal
            # source without fabricating missing AI/API evidence.
            if source_usable_for_consolidation(db_path, str(row["audit_id"])):
                accepted.append(row)

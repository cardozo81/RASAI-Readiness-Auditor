...
def _sidecar_name(source_type: str) -> str:
    direct = public_label(source_type)
    if direct:
        return direct
    if source_type.startswith("GOOGLE_SEARCH_CONSOLE_"):
        suffix = source_type.removeprefix("GOOGLE_SEARCH_CONSOLE_")
        suffix_label = public_label(suffix) or suffix.replace("_", " ").title()
        return f"Google Search Console - {suffix_label}"
                ...
                rows.append(
                    {
                        ...
                        "occurred_at": dataset.get("collected_at"),
                        "raw": {
...
def _browser_snapshot_metrics(
    ...
    *,
    ...
    own_connection = metric_connection is None
    if metric_connection is None:
        metric_connection = sqlite3.connect(database)
        metric_connection.row_factory = sqlite3.Row
    try:
        if not (_table_exists(metric_connection, "page_snapshots") and _table_exists(metric_connection, "pages")):
            return []
        rows = metric_connection.execute(
    ...
    if not bool(getattr(current, "_rasai_catalog_projection_consistency", False)):
        def web_rows(
            ...
            *,
            ...
            rows = list(current(database, audit_id, connection=connection))
            existing = {str(row[0]) for row in rows if row}
            for row in _browser_snapshot_metrics(database, audit_id, connection=connection):

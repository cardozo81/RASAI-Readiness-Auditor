...
A projeção HTML audit-owned suportada é `<AUD-ID>/report-catalog/`. `<AUD-ID>/report/`, `<AUD-ID>/report.html` e `<AUD-ID>/cat-09.html` não pertencem ao contrato de saída de `rasai audit`.
...
| Observed Generative Visibility | resultados de AI Search | observacional; não entra no score |
| Search & AI Observability | resultados e diagnósticos externos | sidecar derivado; non-scoring, salvo `BR-GEO-060` conforme contrato específico |
...
`26_OBSERVED_GENERATIVE_VISIBILITY.md` define resultados observados/importados. Esses dados não recalculam o AUD fonte e não entram automaticamente no `SCORE-GEO-004`.

Datasets externos podem ser usados em pesquisa/benchmarking para avaliar associação entre readiness e resultados. Isso é separado do runtime de scoring e qualquer mudança decorrente exige nova versão metodológica explícita.
...
- a qualidade estrutural da AUD é projetada nas superfícies de governança de `report-catalog/`;
- Fix Verification compara duas AUDs sem alterar as fontes;
- Evidence Timeline produz artefato temporal standalone;
- Evidence Confidence, Operational Priority, Coverage Map e Recommendation Validation são conceitos derivados, não um segundo readiness score.
...
O SaaS Pilot Web adiciona UI de navegador sem segunda persistência e sem mover regra de avaliação para o frontend. Reports de um AUD podem ser projetados via HTTP somente após autorização e apenas dentro da árvore pública `report-catalog/`.

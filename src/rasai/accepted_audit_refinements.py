...
def _cell_html(cell: Any) -> str:
    ...
    return str(_display_value(cell))
    ...
    for item in sorted(
        ...
        detail_rows.append(
            (
                ...
                a._state_text(result,a._status_label(result)),
...
def _w3c_remediation_html(database: Any, audit_id: str, a: Any) -> tuple[list[Sequence[Any]], list[str]]:
    ...
    for index, item in enumerate(metrics, 1):
        ...
        rows.append((
            f"Corrigir {title}",
            "Padrões web",
            a._priority_text("HIGH") if errors else "-",
...
def _remediation_html(database: Any, data: Any) -> str:
    ...
    for action in ai_discovery:
        ...
        priority_display=a._priority_text(priority)
        rows.append((title, a._domain_label("FILES_DISCOVERY"), priority_display, "CAT-01 → CAT-09 · IA técnica", a._modal_button(modal_id, "Ver orientação")))
    ...
    for rec in deep:
        ...
        priority_display=a._priority_text(rec.get("priority"))

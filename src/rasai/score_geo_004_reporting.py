...
REPORT_FILE = "score-geo-004.html"


def register_navigation() -> None:
    items = [item for item in report_navigation.NAV_ITEMS if item[1] not in {"score-geo-003.html", REPORT_FILE}]
    pos = next((i + 1 for i, item in enumerate(items) if item[1] == "readiness.html"), 2)
    items.insert(pos, (SCORING_VERSION, REPORT_FILE))
...
def write_score_geo_004_report(*, audit_id: str, workspace: AuditWorkspace) -> Path:
    ...
    html = f"""<!doctype html><html lang='pt-BR'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>{SCORING_VERSION}</title><link rel='stylesheet' href='css/site.css'></head><body>{nav}<main class='app-main'>
<header class='hero'><div class='eyebrow'>RASAi - método de pontuação vigente</div><h1>{SCORING_VERSION}</h1><p class='lead'>Método operacional padrão do SARI-001. As dimensões e o Overall são determinísticos, reproduzíveis e baseados apenas nas evidências persistidas da auditoria. O Overall não representa probabilidade de ranking ou citação.</p></header>

...
def semantic_color(value: object) -> str:
    """Return a restrained semantic color for operational/user-facing states."""
    normalized = str(value or "").strip().upper()
    if not normalized or normalized in {"-", "N/A", "NONE"}:
        return GRAY
    if any(token in normalized for token in (
        "FALHA", "FAIL", "ERROR", "ERRO", "BLOCK", "BLOQUE", "CRITICAL",
        "INDISPONÍVEL", "INDISPONIVEL", "EXPIR",
    )):
        return RED
    if any(token in normalized for token in (
        "CONCLU", "COMPLETE", "SUCCESS", "SUCESSO", "READY", "PRONTA",
        "ELEGÍVEL", "ELEGIVEL", "APTO", "DISPONÍVEL", "DISPONIVEL", "REUTIL",
    )):
        return GREEN
    if any(token in normalized for token in (
        "PARCIAL", "PEND", "AGUARD", "RETRY", "TENTATIVA", "LIMIT",
        "NÃO ELEGÍVEL", "NAO ELEGIVEL", "NÃO SELECIONADO", "NAO SELECIONADO",
        "EM EXECUÇÃO", "EM EXECUCAO", "PROCESS", "ANALIS", "COLET",
    )):
        return YELLOW
    if any(token in normalized for token in (
        "NÃO NECESSÁRIO", "NAO NECESSARIO", "NÃO SOLICIT", "NAO SOLICIT",
        "DESABIL", "DISABLED", "IGNOR",
    )):
        return GRAY
    return CYAN


def semantic_text(value: object, *, bold: bool = False) -> str:
    """Paint one relevant operational value without colorizing surrounding prose."""
    return paint(value, semantic_color(value), bold=bold)


def title_text(value: object) -> str:
    """Canonical restrained title/breadcrumb emphasis for non-configuration screens."""
    return paint(value, CYAN, bold=True)


def destructive_text(value: object) -> str:
    """Emphasize only the destructive token/action that deserves operator attention."""
    return paint(value, RED, bold=True)


def warning_text(value: object, *, bold: bool = False) -> str:
    return paint(value, YELLOW, bold=bold)

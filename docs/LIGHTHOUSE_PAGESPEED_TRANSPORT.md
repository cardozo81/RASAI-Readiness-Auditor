...
**Agentic Browsing é uma fonte/categoria separada do transporte PageSpeed vigente.** `agentic-browsing` não integra o request ao PageSpeed Insights. O campo `agentic_browsing_score` permanece somente como compatibilidade/evolução para uma futura fonte Lighthouse direta. Na ausência dessa fonte separada, o relatório deve apresentar a métrica como não disponível via provider atual, nunca como score zero.

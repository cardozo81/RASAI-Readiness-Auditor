...
def _summary_metadata(summary: Any | None, *, audit_id: str, status_only: bool) -> dict[str, Any]:
    ...
    return {
        "audit_id": audit_id,
        "reprocess_id": None if status_only else summary.last_reprocess_id,
...
def _install_worker_reprocess() -> None:
    ...
    def execute_job_with_reprocess(store: Any, job: Any, *, audits_root: str | Path = "audits"):
        ...
        from rasai.reprocess_policy import normalize_selected_items, reprocess_policy

        raw_selected = payload.get("selected_items")
        if raw_selected is not None and not isinstance(raw_selected, list):
            raise ValueError("AUDIT_REPROCESS selected_items must be an array when provided")
        selected_items = normalize_selected_items(raw_selected)
        if raw_selected is not None and not selected_items:
            raise ValueError("AUDIT_REPROCESS selected_items must contain at least one valid item")

        raw_use_ai = payload.get("use_ai")
        if raw_use_ai is not None and not isinstance(raw_use_ai, bool):
            raise ValueError("AUDIT_REPROCESS use_ai must be boolean when provided")
        use_ai = raw_use_ai if isinstance(raw_use_ai, bool) else None
        ...
        metadata = {
            ...
            "last_reprocess_id": result.reprocess_id,
            "status_only": False,
            ...
            "selected_items": getattr(result, "selected_items", 0),
            "unselected_items": getattr(result, "unselected_items", 0),
            "ai_used": getattr(result, "ai_used", None),

...
O SearchGEO registra os fatos; a intenção de negócio precisa ser validada pelo analista responsável.
...
Se somente parte das URLs estiver bloqueada, o SearchGEO não interrompe globalmente o audit. O fail-fast global é aplicado somente quando todo o universo auditado permanece tecnicamente bloqueado após a reconciliação disponível. As URLs problemáticas continuam identificadas individualmente.
...
Quando um provider compatível está ativo, o SearchGEO pode fazer **uma análise complementar de infraestrutura** com base exclusivamente na evidência técnica persistida.

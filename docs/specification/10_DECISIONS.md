...
O índice público é o **Search & AI Readiness Index - Índice de Prontidão Search & IA**, versão pública **001** e ID técnico `SARI-001`; o método vigente é o **Método de Pontuação de Prontidão**, versão pública **001** e contrato técnico `SCORE-GEO-004`.
...
A Web API e o SaaS Pilot Web são superfícies do mesmo domínio de produto e não mantêm uma segunda **regra de avaliação** nem uma segunda persistência.
...
## D-016 - Monitoring, observability e resultados
...
Associação temporal não é causalidade. Observed Generative Visibility, Search Console, Bing, CrUX, GA4, logs e demais resultados externos não entram automaticamente no score de readiness.
...
`report-catalog/sari.html` é a superfície canônica do SARI. `report-catalog/methodology.html` é a superfície canônica da metodologia. A versão metodológica pertence a `scoring_version`, banco, manifests, metadados e conteúdo do relatório; não a um filename público alternativo.

Toda auditoria concluída com sucesso materializa as superfícies canônicas definidas por `CATALOG-REPORT-002` e `src/rasai/catalog_report_contract.py`. A seleção dos CATs governa o conteúdo aplicável, enquanto as páginas de governança do contrato permanecem estáveis.
...
Navegação, ordem, rótulos e verificação de completude devem derivar de `CATALOG_REPORT_PAGES` e do mesmo contrato compartilhado.
...
Segredos não devem ser incluídos em documentação de exemplo real, banco de evidência, artefatos, relatórios, agendamentos ou logs.

...
from rasai.console_ui import CYAN, DIM, GRAY, paint

_INSTALLED = False
_ANSI_RE = re.compile(r"\x1b\[[0-9;]*m")
# Public configuration IDs use the canonical eight-digit shape.
...
def _install_catalog_copy() -> None:
    ...
    def catalog_menu(console_module: ModuleType, state: Any, catalog: Any) -> None:
        ...
        def refined_input(prompt: str = "") -> str:
            raw = previous_input(prompt)
            normalized = str(raw or "").strip().upper()
            if prompt.strip() == "Número/ID ou ação:" and normalized == "D":
                from rasai import console_catalog_plan as plan

                if plan.deselect_catalog(state, catalog):
                    state.error = ""
                    # Feed V to the wrapped menu after the removal so every composed
                    # catalog layer terminates immediately and returns to PREPARAR AUDITORIA.
                    return "V"
            return raw

        builtins.print = refined_print
        builtins.input = refined_input
        try:

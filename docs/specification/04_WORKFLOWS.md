...
Os princípios centrais são Evidence First, isolamento de falhas, escopo explícito de dispositivo, IA opcional e separação entre readiness, resultados observados, Web Performance, acessibilidade e Apdex.
...
Falha de provider é estado operacional, não finding do website. Regras de Avaliação de Prontidão permanecem independentes de provider.
...
O runtime de `rasai audit` gera `report-catalog/` como projeção HTML. `report/`, `report.html` e `cat-09.html` não pertencem a esse output; recomendações, findings, evidências, causa raiz, precisão, scoring, integrações, IA e telemetria são materializados/persistidos por seus contratos funcionais.

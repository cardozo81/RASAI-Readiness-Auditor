...
REPORT_MARKER_START = "<!-- rasai-external-metrics-integrity:start -->"
REPORT_MARKER_END = "<!-- rasai-external-metrics-integrity:end -->"
_CATEGORY_COLUMNS = {
...
def refresh_external_metrics_integrity_artifact(
    *,
    audit_id: str,
    workspace: AuditWorkspace,
) -> ExternalMetricsIntegrity | None:
    """Rematerialize the integrity artifact from the current persisted M21 state.

    This is intentionally read-only with respect to audit.db. It exists for RPR
    flows that already persisted their effective Web Performance state but need every
    derived artifact to represent that same final observation. No external request is
    repeated and no scoring formula is invoked.
    """
    if not workspace.database.is_file():
        return None
    con = sqlite3.connect(workspace.database)
    ...
    try_append_operational_event(
        workspace,
        "EXTERNAL_METRICS_INTEGRITY_REFRESHED",
        audit_id=audit_id,
        contexts=len(contexts),
        source="PERSISTED_EFFECTIVE_STATE",
        network_calls=0,
        artifact=ARTIFACT,
    )
    return integrity

...
def _consistent_catalog_sources(
    database: Path,
    data: Any,
    catalog_id: str,
    *,
    ...
    if original is None:
        return []
    rows = list(original(database, data, catalog_id, connection=connection))
    ...
    try:
        if _table_exists(con, "page_snapshots") and _table_exists(con, "pages"):
            count = con.execute(
        ...
        if _table_exists(con, "rule_executions"):
...
def _consistent_web_metric_rows(
    database: Path,
    audit_id: str,
    *,
    connection: sqlite3.Connection | None = None,
    ...
    rows = list(original(database, audit_id, connection=connection)) if callable(original) else []
    labels = {str(row[0]) for row in rows if row}
    metric_connection = connection
    own_metric_connection = metric_connection is None
    if metric_connection is None:
        metric_connection = sqlite3.connect(database)
        metric_connection.row_factory = sqlite3.Row
    try:
        if _table_exists(metric_connection, "standards_metric_observations"):
            for raw in metric_connection.execute(
        ...
        if own_metric_connection:
            metric_connection.close()

    obs = observability_database_path(database.parent)
    if obs.is_file():
        observability_connection = sqlite3.connect(obs)
        observability_connection.row_factory = sqlite3.Row
        try:
            if _table_exists(observability_connection, "crux_history"):
                all_rows = [dict(row) for row in observability_connection.execute("SELECT * FROM crux_history ORDER BY period_end,record_id").fetchall()]
                if all_rows:

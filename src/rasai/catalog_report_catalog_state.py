...
def _friendly_component(value: Any) -> str:
    key=_norm(value)
    return _COMPONENT_LABELS.get(key, public_label(value) or str(value or "-").replace("_"," ").title())


def _friendly_service(value: Any) -> str:
    key=str(value or "")
    return _SERVICE_LABELS.get(key,_SERVICE_LABELS.get(key.casefold(),public_label(value) or key.replace("_"," ").title() or "-"))
...
def _catalog_source_specs(catalog_id: str) -> tuple[tuple[str,str],...]:
    ...
    return {
        ...
        "CAT-02":(
            ("web_performance_attempts","Tentativas PageSpeed/Lighthouse"),
        ...
        "CAT-10":(
            ...
            ("page_snapshots","Execução no navegador e capturas reutilizadas"),
        ),
    }[catalog_id]
...
def _catalog_sources(
    ...
    *,
    ...
    con=connection or sqlite3.connect(database)
    con.row_factory=sqlite3.Row
    try:
        out=[]
        for table,label in _catalog_source_specs(catalog_id):
            if catalog_id=="CAT-02" and table=="web_performance_attempts" and "service" in _columns(con,table):
                rows=_rows(
                    con,
                    """SELECT COUNT(*) FROM web_performance_attempts
                       WHERE audit_id=? AND UPPER(COALESCE(service,''))='PAGESPEED_INSIGHTS'""",
                    (data.audit_id,),
                )
                count=int(rows[0][0]) if rows else 0
            elif catalog_id=="CAT-02" and table=="web_performance_observations" and "accessibility_score" in _columns(con,table):
                rows=_rows(
                    con,
                    """SELECT COUNT(*) FROM web_performance_observations
...
def _catalog_status(
    ...
    *,
    connection: sqlite3.Connection | None = None,
    sources: list[tuple[str,str,int]] | tuple[tuple[str,str,int], ...] | None = None,
) -> tuple[str,str,str]:
    ...
    if catalog_id=="CAT-05":
        ...
        if row is not None and bool(row["requested"]) and bool(row["effective_enabled"]):
            ...
            if state in {"ERROR","FAILED_RETRYABLE","FAILED_PERMANENT","BLOCKED"} or (attempted==0 and reason in {"PRE_SCORING_COLLECTION_STATE_NOT_FOUND","COMMON_CRAWL_PRESEAL_DATASET_MISSING"}):
                return "PARCIAL","warn","A inteligência de busca / SERP possui evidência persistida, mas Common Crawl foi solicitado e não concluiu validamente a coleta planejada nesta AUD."
    run=_explicit_run(database,data,catalog_id,connection=connection)
    if run:
    ...
    effective_sources=list(sources) if sources is not None else _catalog_sources(
        database,
        data,
        catalog_id,
    ...
    if catalog_id=="CAT-10" and not run:
        ...
        if states & _STATUS_FAILURE:
            return (
                ("PARCIAL" if effective_sources or states & _STATUS_SUCCESS else "FALHA"),
                ("warn" if effective_sources or states & _STATUS_SUCCESS else "bad"),
...
def _cat08_rpr_ai_execution(
    ...
    *,
    ...
    con = connection if connection is not None else sqlite3.connect(database)
    con.row_factory=sqlite3.Row
    try:
        if not (_table_exists(con,"audit_reprocess_runs") and _table_exists(con,"ai_provider_attempts")):
            return None
        runs=_audit_rows(con,"audit_reprocess_runs",audit_id)
        for run in reversed(runs):
            if not run.get("completed_at"):
                continue
            try:
                cfg=json.loads(str(run.get("configuration") or "{}"))
            except (TypeError,ValueError,json.JSONDecodeError):
                cfg={}
            if not isinstance(cfg,Mapping) or cfg.get("ai_used") is not True:
                continue
            columns=_columns(con,"ai_provider_attempts")
            if "started_at" not in columns or "semantic_contract_version" not in columns:
                continue
            row=con.execute(
                """SELECT provider,model,status,started_at
                   FROM ai_provider_attempts
                   WHERE audit_id=?
                     AND UPPER(COALESCE(semantic_contract_version,'')) LIKE 'IMPROVEMENT-INTELLIGENCE%'
                     AND started_at>=? AND started_at<=?
                   ORDER BY started_at DESC,rowid DESC LIMIT 1""",
                (audit_id,str(run.get("started_at") or ""),str(run.get("completed_at") or "")),
            ).fetchone()
            if row is not None:
                return {
                    "reprocess_id":str(run.get("reprocess_id") or ""),
...
def _configuration_rows(data: _ReportData, catalog_id: str) -> list[Sequence[Any]]:
    ...
    rows=[
        ...
        ("Uso de IA nesta capacidade",ai_usage,"Plano congelado / inteligência de busca" if cat05_competitive_ai else ("Plano congelado" if plan_available else "Não determinável")),
        ("Política de IA",ai_policy,"Inteligência de busca / SERP" if cat05_competitive_ai else "Catálogo"),
    ]
    ...
    if catalog_id=="CAT-03":
        ...
        if isinstance(environment,Mapping):
            fields=(
                ...
                ("Recursos de terceiros","RASAI_SECURITY_THIRD_PARTY"),
                ("Correlação em tempo de execução","RASAI_SECURITY_RUNTIME_CORRELATION"),
...
def _cat08_rpr_configuration_rows(
    database: Any,
    data: _ReportData,
    *,
    connection: sqlite3.Connection | None = None,
) -> list[Sequence[Any]]:
    rpr_ai=_cat08_rpr_ai_execution(database,data.audit_id,connection=connection)
    if not rpr_ai:
        return []
    rpr_id=str(rpr_ai.get("reprocess_id") or "-")
    return [
        ("IA executada no reprocessamento","Sim",f"Reprocessamento {rpr_id}"),
        (
            "Provedor / modelo efetivos no reprocessamento",
            f"{rpr_ai.get('provider') or '-'} / {rpr_ai.get('model') or '-'}",
            "Tentativa de IA persistida",
        ),
        (
            "Resultado da tentativa efetiva no reprocessamento",
            _status_label(rpr_ai.get("status")),
            "Tentativa de IA persistida",
        ),
    ]

...
def _install_improvement_runtime() -> None:
    ...
    def validate(config, env: Mapping[str, str] | None = None):
        ...
        # Missing credentials are operational readiness, not structural
        # configuration invalidity. The governed runtime below records the analysis
        # as not configured without attempting provider transport.
        ...
        *,
        ...
        if not candidates:
            ...
            missing_credential = bool(
                ...
                and not (os.environ.get(registration.key_env) or "").strip()
            )
            return "", [], (
                "AI_NOT_CONFIGURED"
...
def _install_improvement_saas() -> None:
    ...
    def validate_extension(payload, normalized):
        ...
        if enabled:
            ...
            # ai_provider=none is valid for the AUDIT execution. CAT-08 remains a
            # required diagnostic-closure item and is projected as pending when AI
            # cannot run; deterministic evidence/reporting must still continue.
        normalized.update(
            {
...
def _install_improvement_console() -> None:
    ...
    def single_url_ready(state):
        ...
        if selection == "none":
            return (
                True,
                "auditoria apta; CAT-08 permanecerá pendente porque a IA principal não está configurada",
            )
    ...
    def configure(console_module, state):
        ...
        if state.input_mode != "url":
            ...
            return
        selection = str(getattr(state, "ai_provider", "none") or "none").casefold()
        print(f"IA principal: {selection.upper()}")
        if selection == "none":
            print(
                "Sem provider: a auditoria determinística continuará e CAT-08 ficará "
                "pendente para fechamento integral até um reprocessamento com IA apta."
            )

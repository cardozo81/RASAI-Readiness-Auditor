...
| `POST` | `/api/v1/projects/{project_id}/consolidated-reports` | enfileira um **Relatório Consolidado Longitudinal**, formato público **001**, para uma URL e um dispositivo; `CONS-5` permanece como identificador técnico do formato |

O endpoint não possui renderer ou regra longitudinal próprios. A request é convertida em `REPORT_REFRESH`; o worker autorizado chama `rasai.consolidation.service.generate`. Assim, `source_revision_id`, revisão temporal pós-RPR, freshness/linkabilidade das fontes, comparabilidade e validação da IA pertencem ao artifact do **Relatório Consolidado Longitudinal** compartilhado (`CONS-5` no contrato técnico) e não ao contrato HTTP.
| `GET` | `/api/v1/execution-jobs/{job_id}/result` | serve o `report.html` consolidado concluído e autorizado |
...
| `GET` | `/api/v1/execution-jobs/{job_id}/result` | abre resultado de `REPORT_REFRESH` consolidado concluído |

...
def capability_status(state: Any, capability: CapabilityUI) -> tuple[str, str]:
    ...
    if capability.key == "deep-analysis":
        ...
        if str(getattr(state,"ai_provider","none")) == "none":
            return (
                "APTO COM LIMITAÇÕES",
                "URL única válida; CAT-08 executará a parte determinística e ficará pendente para fechamento integral por IA",
            )
        return "APTO", "URL única e IA principal configuradas"
    if capability.key == "remediation":
        ...
        if str(getattr(state,"ai_provider","none")) == "none":
            return (
                "APTO COM LIMITAÇÕES",
                "enriquecimento por IA solicitado sem provider; etapas determinísticas continuam e a camada de IA ficará pendente",
            )
        return "APTO", "enriquecimento por IA solicitado"
...
def capability_menu(console_module: ModuleType, state: Any, capability: CapabilityUI) -> str | None:
    ...
    while True:
        ...
        if specs:
            ...
            for spec in sorted(specs, key=lambda s: (owner_for(s).casefold(), s.name.casefold())):

...
def test_external_crawl_group_is_bounded_and_contract_sums_to_one() -> None:
    ...
    # Existing technical ratios are exactly their previous weights multiplied by .97.
    assert groups["PAGE_ACCESS"] / 0.97 == pytest.approx(0.30)
    assert groups["ROBOTS"] / 0.97 == pytest.approx(0.15)
    assert groups["SITEMAP"] / 0.97 == pytest.approx(0.05)
    assert groups["REDIRECT"] / 0.97 == pytest.approx(0.10)
    assert groups["SPA_ROUTE"] / 0.97 == pytest.approx(0.10)
    assert groups["SPA_NAVIGATION"] / 0.97 == pytest.approx(0.10)
    assert groups["INTERNAL_LINKS"] / 0.97 == pytest.approx(0.20)
...
def test_absence_preserves_the_previous_technical_ratio_exactly() -> None:
    # INTERNAL_LINKS historically represented 20% of DISCOVERY_ACCESS. With no
    # external evidence, scaling every technical group by .97 and normalizing the
    # applicable denominator must preserve that exact 80/100 result.
    score = _discovery_score(_base_discovery({"BR-GEO-050": RuleResult.FAIL}))
...
def test_positive_common_crawl_is_only_a_small_corroborative_uplift() -> None:
    ...
    assert without_external.value == pytest.approx(80.0)
    assert with_external.value == pytest.approx(80.6)
    assert with_external.value - without_external.value == pytest.approx(0.6)
    assert with_external.value - without_external.value <= 3.0 + 1e-9
    assert with_external.coverage == pytest.approx(1.0)


def test_external_crawl_failure_is_not_an_admissible_runtime_signal() -> None:
    # Production materialization is positive-only: absence/no-data/error never creates
    # a FAIL BR-GEO-060. The scoring contract therefore exposes this rule exclusively
    # as external corroboration, not as a mandatory readiness requirement.
    contract = rule_contract("BR-GEO-060")
    assert contract is not None
    assert contract.evidence_role == EVIDENCE_ROLE_EXTERNAL_CORROBORATIVE

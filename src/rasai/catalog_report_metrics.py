...
def _web_observation(
    ...
    *,
    ...
    con,own_connection=_metric_connection(database,connection)
    try:return _last(con,"web_performance_observations",audit_id)
    finally:
        if own_connection:con.close()
...
def _web_metric_rows(
    ...
    *,
    ...
    row=_web_observation(database,audit_id,connection=connection); out=[]
...
def _apdex_result_rows(
    ...
    *,
    ...
    row=_apdex_summary(database,audit_id,experience=experience,connection=connection)
    if not row:return []
...
def _passive_security_metric_rows(
    database: Path,
    audit_id: str,
    *,
    connection: sqlite3.Connection | None = None,
...
def _catalog_metrics(
    ...
    *,
    ...
    if catalog_id=="CAT-02":
        obs=_web_observation(database,data.audit_id,connection=connection)
        return [("Lighthouse · Acessibilidade",f"{float(obs['accessibility_score']):.0f} / 100","Índice")] if obs.get("accessibility_score") is not None else []
    if catalog_id=="CAT-04":return _web_metric_rows(database,data.audit_id,connection=connection)
    if catalog_id=="CAT-05":return _serp_metric_rows(database,data.audit_id,connection=connection)
    if catalog_id=="CAT-06":return _apdex_result_rows(database,data.audit_id,experience=False,connection=connection)
    if catalog_id=="CAT-07":return _apdex_result_rows(database,data.audit_id,experience=True,connection=connection)
    if catalog_id=="CAT-10":return _passive_security_metric_rows(database,data.audit_id,connection=connection)
    return []

...
def _apply_reprocess_ai_policy(config: ImprovementConfig) -> ImprovementConfig:
    """Overlay only execution-local RPR AI selection onto CAT-08 configuration."""
    ...
    return replace(
        ...
        model=(
            ""
            if normalized_provider in {"auto", "none"}
            else str(model or "")
        ),
        reasoning=(
            ""
...
def _project_fulfillment_result(
    ...
    *,
    ...
    if normalized == "COMPLETE":
        ...
        return
    if normalized_reason in {"AI_NOT_CONFIGURED", "AI_PROVIDER_NOT_CONFIGURED"}:
        set_work_item_status(
            workspace,
            audit_id=audit_id,
            component=_COMPONENT,
            ...
            error_message="CAT-08 requer IA para fechamento integral, mas nenhum provider apto foi configurado nesta execução",
            retryable=True,
        )
        return
    set_work_item_status(
        ...
        error_code=normalized_reason or normalized or "IMPROVEMENT_INTELLIGENCE_UNAVAILABLE",
...
def _governed_improvement_hook(*, audit_id: str, workspace: Any, evidence_snapshot: Any):
    ...
    try:
        ...
        # A reprocess execution owns its provider choice independently from the

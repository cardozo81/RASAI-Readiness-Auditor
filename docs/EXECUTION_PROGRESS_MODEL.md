...
Auditoria, reprocessamento e consolidado usam a mesma gramática visual de execução. O console redesenha a tela corrente, sem acumular as fases anteriores como uma lista infinita, e mantém o histórico cronológico completo nos logs.

Durante a execução, a superfície canônica apresenta:

```text
Operação             : operação técnica atualmente em andamento
Etapa                : X de Y [previstas, quando o plano ainda puder mudar]
Anterior             : etapa anterior e seu estado
Atual                : etapa funcional corrente
Próxima              : etapa seguinte ou CONDICIONAL
Andamento da etapa   : percentual medido quando existe unidade real
Pipeline total       : posição geral medida/projetada no trabalho planejado
Executando           : descrição objetiva do trabalho/subprocesso em curso
Detalhes             : contadores próprios da etapa quando disponíveis
```

A etapa atual pode expor unidades específicas. Synthetic Apdex, por exemplo, informa alvo de amostras válidas, válidas, inválidas, tentativas realizadas e limite máximo; integrações podem informar serviço, requisição/tentativa e status. A apresentação observa telemetria já existente e não altera quantidade de amostras, retries, concorrência, timeout, provider routing ou ordem do motor.

A coleta não é tratada como sinônimo de conclusão. Quando houver trabalho real depois dela, a mesma superfície mostra consolidação, cálculo, persistência e materialização de artefatos. **Gerando relatório** significa ler dados/evidências já persistidos, compor o HTML e gravar arquivos em disco; essa etapa não abre nova coleta apenas para produzir a apresentação.

A cor reforça, mas não substitui, o texto do estado. A convenção canônica é: **verde** para concluído/sucesso/elegível, **amarelo** para execução ativa, parcialidade, pendência ou não elegibilidade, **vermelho** para falha/bloqueio/indisponibilidade, **cinza** para aguardando/neutro e **ciano** para contexto e projeção informativa. A mesma semântica é aplicada a AUD, RPR e CONS sempre que a superfície usar o renderer canônico. Terminais sem suporte a cor continuam recebendo todo o significado em texto.

...
def _staged_secret_value(
    *,
    ...
    raw = reader(f"{label or name} [V=voltar sem salvar]: ")
    ...
    if confirm_continue(
        "Confirmar alteração",
        back_label="Cancelar e manter o valor atual",
    ):
        state.error = ""
        return validated
    state.error = ""
...
def _reset_scope(facade: Any, state: Any, title: str, specs: tuple[Any, ...]) -> None:
    ...
    if not confirm_sensitive(
        "RESETAR",
        action_label="Resetar as variáveis/defaults exibidos neste escopo",
        back_label="Voltar sem resetar",
...
def _install_environment_facade() -> None:
    ...
    def variable_menu(state: Any, spec: Any) -> None:
        while True:
            ...
            facade.base_environment.render_header(state)
            facade._breadcrumb(spec.category, facade.context_for(spec), friendly_label(spec))
            print(facade.paint(f"Variável técnica: {spec.name}", facade.GRAY))
            ...
            try:
                if spec.accepted:
                    ...
                    validated = _staged_secret_value(
                        ...
                        label=f"Novo valor - {friendly_label(spec)}",
                    )
        ...
        while True:
            ...
            if action == "S":
                validated = _staged_secret_value(
                    ...
                    label=f"Chave de API - {registration.display_name}",
                )
                if validated is None:
                    continue

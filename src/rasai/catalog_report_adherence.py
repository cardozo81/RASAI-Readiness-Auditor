    ...
    def apdex_samples_html(database: Any, data: Any, *, experience: bool) -> str:
        ...
        for index, sample in enumerate(samples, 1):
            ...
            if experience:
                ...
                classification=analysis._state_text(sample.get("classification"),analysis._classification_label(sample.get("classification")))
                measurement=analysis._state_text(sample.get("status"),analysis._status_label(sample.get("status")))
                rows.append((sample.get("run_index", index), displayed_at, analysis._device_label(sample.get("device")), classification, analysis._fmt_number(duration, "ms"), analysis._fmt_number(sample.get("lcp_ms"), "ms"), sample.get("request_failed_count") or 0, measurement, analysis._modal_button(modal_id, "Ver amostra")))
                fields = (
                    ("Amostra", sample.get("sample_id")), (timestamp_label, displayed_at), ("URL", sample.get("url")), ("URL final", sample.get("final_url")),
                    ("Classificação", classification), ("Duração da ação", analysis._fmt_number(sample.get("user_action_duration_ms"), "ms")),
        ...
        if experience and run:
            ...
            duration_value=f"{duration_score:.3f}" if duration_score is not None else "-"
            lead += analysis._metric("Apdex por duração", duration_value, f"{duration_valid} amostra(s); sem aplicar a política de erros")
            effective_value=f"{effective_score:.3f}" if effective_score is not None else "-"
            effective_tone=page._catalog_metric_tone("CAT-07","Apdex",effective_value) if effective_score is not None else "neutral"

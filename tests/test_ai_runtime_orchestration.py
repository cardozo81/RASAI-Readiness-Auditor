...
def test_exchange_log_redacts_secrets_private_reasoning_and_transient_context() -> None:
    ...
    assert "provider private reasoning" not in (exchange.response_payload or "")
    assert '"reasoning_text":"[PRIVATE_REASONING_NOT_PERSISTED]"' in (exchange.response_payload or "")

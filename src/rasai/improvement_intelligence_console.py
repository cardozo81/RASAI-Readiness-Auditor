...
from rasai.console_input_contract import EditCancelled, prompt_text, prompt_yes_no
...
def _config_from_state(state: Any) -> ImprovementConfig:
    ...
    if selection not in {"none", "auto", ""}:
        ...
        if registration is None:
            raise ValueError(f"provider de IA desconhecido para análise profunda: {selection}")
        ...
        if reasoning and reasoning not in registration.reasoning_values:
            raise ValueError(
                f"esforço {reasoning!r} não é suportado por {registration.display_name}"
            )
...
def _single_url_ready(state: Any) -> tuple[bool, str]:
    ...
    if selection != "none" and capability is None:
        return False, f"provider de IA desconhecido para análise profunda: {selection}"
    ...
    if selection == "none":
        return (
            True,
            "auditoria determinística apta; CAT-08 ficará pendente porque a IA necessária ao fechamento não está configurada",
        )
    if capability is not None and not capability.available:
        return (
            True,
            "auditoria determinística apta; análise profunda ficará pendente enquanto a IA principal estiver indisponível"
...
def configure(console_module: ModuleType, state: Any) -> None:
    ...
    snapshot = {
        ...
        if name.startswith("improvement_")
    ...
    try:
        ...
        raw_domains = input(
            "Domínios da análise profunda - números separados por vírgula "
            "[ENTER=manter atuais; todos=todos; V=voltar sem salvar]: "
        ).strip().casefold()
        if raw_domains == "v":
            raise EditCancelled()
        if raw_domains:
        ...
        maximum = prompt_text(
            ...
            current=state.improvement_max_recommendations,
        )
        if maximum:
            value = int(maximum)
            if value < 1 or value > 100:
            ...
            state.improvement_max_recommendations = value
        timeout = prompt_text(
            "Timeout da chamada profunda em segundos",
            current=f"{state.improvement_timeout:g}",
        )
        if timeout:
            value = float(timeout)
            if value <= 0:
        ...
        state.error = ""
        state.operation = "LOCAL:IMPROVEMENT_EDIT_CANCELLED"
        return

...
## 3. Qualidade da evidência e projeção

O CLI vigente não possui subcomando `rasai quality report`. A qualidade estrutural de uma AUD é projetada pelas superfícies de governança do `report-catalog/`, principalmente:

```text
report-catalog/index.html
report-catalog/execution-evidence.html
report-catalog/metrics.html
```

Essas páginas permanecem somente leitura sobre os dados persistidos e não criam um segundo score de readiness.

### Saúde da auditoria

A saúde da auditoria avalia qualidade de coleta/dados, não readiness do website. As verificações podem incluir integridade SQLite, estado de conclusão, cobertura de snapshots, RuleExecutions inconclusivas/com erro, disponibilidade da versão de scoring, existência de artefatos, materialização de relatórios, integridade do sidecar e proveniência.
...
A prioridade operacional combina características persistidas do finding, escopo afetado, confiança da evidência e esforço de remediação. Não altera Severity nem valores SARI/SCORE.
...
A verificação de correção usa evidência de comparação persistida e não pode afirmar impacto posterior em Search/IA.

## 6. Linha do tempo de evidências
...
A cobertura deve incluir migração/identidade do sidecar, preservação de `NULL`, comparabilidade temporal, limites do release gate, detecção de controles de conteúdo, avaliação de atualização baseada em horário persistido, projeção da qualidade estrutural no `report-catalog/`, semântica da verificação de correção e preservação de `scoring_version`.

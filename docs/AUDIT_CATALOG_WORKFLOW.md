...
A documentação funcional uniforme de cada CAT e das demais páginas do relatório está em [`CATALOG_AND_REPORT_SURFACES.md`](CATALOG_AND_REPORT_SURFACES.md). Todos os CATs usam a mesma estrutura documental; documentos de domínio são apenas aprofundamentos.
...
- `CAT-05` pode usar a IA principal somente quando o operador habilitar explicitamente **IA competitiva**; essa análise exige comparação de conteúdo determinística previamente habilitada e não altera a evidência SERP;
- `CAT-08` exige IA para o fechamento integral de seu diagnóstico, mas a parte determinística e o relatório podem ser materializados sem chamada de IA;
...
- `CAT-01`, `CAT-02`, `CAT-04`, `CAT-06` e `CAT-07` não usam IA por seleção; `CAT-05` também não chama IA apenas por ser selecionado, mas possui o opt-in específico **IA competitiva**;
...
Quando `CAT-08` está selecionado, IA continua obrigatória para o fechamento integral, porém a autorização de chamada é uma decisão da execução. `2. Executar auditoria sem IA` mantém CAT-08 selecionado e pendente, sem remover o catálogo nem falsificar conclusão.

## 8. Custo e quota

A estimativa de consumo/custo continua sob os contratos canônicos existentes.

Quando o plano selecionado pode usar IA ou outro recurso tarifado/limitado, a execução passa pelo preview/acknowledgement antes de iniciar o AUD. A estimativa usa a configuração efetiva naquele momento; AUTO continua sendo resolvido pelo orquestrador.

A tela financeira expõe `1. Executar auditoria com IA`, `2. Executar auditoria sem IA`, `A. Ajustar configuração de IA` e `V. Voltar sem executar`. A escolha `1` ou `2` é a autorização final da execução não destrutiva; não existe uma segunda confirmação da mesma intenção. Executar sem IA não apaga a configuração usada por auditorias futuras.
...
`IA competitiva` é um opt-in de IA opcional dentro do CAT-05. Ao ativá-la, o console também habilita a execução opcional de IA para o plano corrente. A opção exige **Comparação de conteúdo** ativa. A indisponibilidade da IA principal não bloqueia a coleta/comparação determinística; a camada competitiva por IA fica pendente/não executada conforme a autorização e readiness da execução. Se a comparação de conteúdo for desativada, a IA competitiva dependente é desativada junto para impedir uma combinação inválida.

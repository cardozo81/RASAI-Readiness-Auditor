...
def _run_report_refresh(store: Any, job: Any, audits_root: Path) -> WorkerResult:
    surface = (_text(job.payload, "surface", "portfolio") or "portfolio").casefold()
    if surface == "portfolio":
        if set(job.payload) - {"surface"}:
            raise ValueError("atualização do portfólio aceita somente o campo surface")
        ...
        if set(job.payload) - allowed:
            raise ValueError("solicitação de consolidado contém campos não suportados")

        baseline_audit_id = _text(job.payload, "baseline_audit_id")
        current_audit_id = _text(job.payload, "current_audit_id")
        selection_mode = (_text(job.payload, "selection_mode", "ALL") or "ALL").upper()
        if not baseline_audit_id or not current_audit_id:
            raise ValueError("relatório consolidado exige duas auditorias-marco")

        manual_ids_raw = job.payload.get("audit_ids", [])
        if not isinstance(manual_ids_raw, list) or any(not isinstance(item, str) for item in manual_ids_raw):
            raise ValueError("audit_ids deve ser uma lista de IDs de auditoria")
        manual_ids = tuple(item.strip() for item in manual_ids_raw if item.strip())

        use_ai = job.payload.get("use_ai", False)
        if not isinstance(use_ai, bool):
            raise ValueError("use_ai deve ser booleano")
        provider = _text(job.payload, "ai_provider")
        # use_ai=True with provider absent/none is a valid request that materializes
        # the deterministic CONS and records the unavailable AI layer explicitly.

        timeout_raw = job.payload.get("ai_timeout_seconds", 180.0)
        if isinstance(timeout_raw, bool) or not isinstance(timeout_raw, (int, float)):
            raise ValueError("ai_timeout_seconds deve ser numérico")
        ...
        if len(authorized) < 2:
            raise ValueError("o escopo SaaS não possui pelo menos duas auditorias autorizadas")
        ...
        filters = normalize_filter(
            ...
            ai_model=_text(job.payload, "ai_model") if use_ai else None,
            ai_reasoning=_text(job.payload, "ai_reasoning") if use_ai else None,
            ai_timeout_seconds=float(timeout_raw) if use_ai else None,
        )
        result = generate(audits_root, filters)
        try:
            result_ref = result.report_path.relative_to(audits_root).as_posix()
        except ValueError:
            result_ref = result.report_path.name

        try:
            manifest = json.loads(result.manifest_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            manifest = {}
        confidence = manifest.get("consolidation_confidence") if isinstance(manifest, dict) else {}
        specialist = manifest.get("specialist_ai") if isinstance(manifest, dict) else {}
        metadata = {
            "surface": "consolidated",
            "url": selection.url,
            "device": selection.device,
            "baseline_audit_id": selection.baseline_audit_id,
            "current_audit_id": selection.current_audit_id,
            "audit_ids": list(selection.audit_ids),
            "audit_count": len(selection.audit_ids),
            "selection_mode": selection.selection_mode,
            "generation_mode": manifest.get("generation_mode") if isinstance(manifest, dict) else None,
            "ai_status": specialist.get("status") if isinstance(specialist, dict) else None,
            "confidence": confidence.get("label") if isinstance(confidence, dict) else None,
            "report_dir": result.report_dir.name,
            "reused": result.reused,
        }
    else:
        raise ValueError("surface de relatório não suportada")
    ...
    if hasattr(store, "record_usage_once"):
        try:
            ...
            pass
    return WorkerResult(result_ref=result_ref, metadata=metadata)

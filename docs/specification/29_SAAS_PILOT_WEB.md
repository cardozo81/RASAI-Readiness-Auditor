...
`WEB-PILOT-018` - o SaaS deve expor candidatos e histórico de consolidados a partir do mesmo domínio longitudinal usado pelo console, sem implementar algoritmo paralelo de seleção.

`WEB-PILOT-019` - a criação de consolidado deve usar `baseline_audit_id`, `current_audit_id`, `selection_mode=ALL|SUCCESS_ONLY|MANUAL` e `audit_ids` quando aplicável. BASE e ATUAL são definidos cronologicamente; mesma URL, mesmo dispositivo e tenancy devem ser revalidados no servidor/worker.

`WEB-PILOT-020` - IA deve ser opcional no consolidado. `use_ai=false` não exige provider; `use_ai=true` solicita a camada de IA, mas provider ausente/indisponível não pode bloquear a materialização determinística. Modelo/reasoning/timeout permanecem fora de qualquer mecanismo de segredo inline e a limitação deve ser registrada explicitamente.

`WEB-PILOT-021` - relatórios materializados de AUD e CONS devem possuir ação de abertura nas superfícies correspondentes. AUD somente pode servir `report-catalog/**` da auditoria autorizada; CONS somente pode servir `audits/consolidated/CONS-*/report.html` pertencente ao conjunto autorizado.
...
- listar candidatos elegíveis para consolidação longitudinal;
- gerar consolidado a partir de duas auditorias-marco e política `ALL`, `SUCCESS_ONLY` ou `MANUAL`;
- listar consolidados materializados e abrir o respectivo `report.html`;
...
Para auditorias, são permitidos somente arquivos resolvidos dentro de:

```text
<Path persistido do AUD>/report-catalog/
```

Para consolidados, somente o artifact:

```text
<audits_root>/consolidated/CONS-*/report.html
```

pode ser servido pela rota de resultado do CONS autorizado.

O servidor deve normalizar/resolver o path e confirmar que o resultado permanece descendente da raiz esperada. A existência de um `audit_id` ou `cons_id` não autoriza acesso ao workspace bruto nem a artifacts fora dessas superfícies.
...
O consolidado usa `REPORT_REFRESH` com `surface=consolidated`. A API grava apenas a solicitação validada; o worker resolve novamente as auditorias autorizadas, aplica a política de seleção e chama o mesmo domínio de consolidação usado pelo console local.
...
- autorização do limite de reports de AUD e CONS;
- bloqueio de tentativa de acesso fora de `report-catalog/` e fora de `consolidated/CONS-*/report.html`;
- paridade de seleção longitudinal `ALL/SUCCESS_ONLY/MANUAL` entre Console, API e worker;
- IA opcional no consolidado sem provider obrigatório quando `use_ai=false`;

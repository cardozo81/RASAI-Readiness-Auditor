...
def test_cat02_source_contract_excludes_generic_dom_observations() -> None:
    from rasai.catalog_report_catalog_state import _catalog_source_specs

    sources={table for table,_label in _catalog_source_specs("CAT-02")}

    assert "web_performance_attempts" in sources
    assert "web_performance_observations" in sources
    assert "element_observations" not in sources
    assert "finding_element_observations" not in sources

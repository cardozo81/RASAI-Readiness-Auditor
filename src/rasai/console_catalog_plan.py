...
class _Plan:
    ...
    # AI authorization belongs to the current execution. A REQUIRED catalog can remain
    # selected with this False; it then stays pending for diagnostic closure.
...
def _plan(state: Any) -> _Plan:
    ...
    explicit_ai_work = bool(
        ...
        or getattr(state, "search_ai_competitive", False)
    )
...
def set_selected_catalog_ids(state: Any, catalog_ids: tuple[str, ...] | list[str]) -> None:
    ...
    # audit. Keep the operator's execution choice independent from catalog selection.
...
def ai_execution_enabled(state: Any) -> bool:
    """Return whether AI calls are authorized for this audit execution.

    Required-AI catalogs remain required for diagnostic closure even when this returns
    False. This flag controls execution authorization, not applicability/closure.
    """
    selected = _plan(state).selected
    has_ai = any(
        item.id in selected and item.ai_mode in {AI_REQUIRED, AI_OPTIONAL}
        for item in CATALOGS
    )
    return has_ai and bool(_plan(state).ai_enabled)


def set_ai_execution_enabled(state: Any, enabled: bool) -> None:
    # AI_REQUIRED is a closure requirement. It must not force provider execution when
    # the operator intentionally runs the deterministic portion without AI.
    _plan(state).ai_enabled = bool(enabled)


@contextmanager
def execution_ai_choice(state: Any, enabled: bool) -> Iterator[None]:
    """Temporarily authorize/suppress AI only for the current audit execution."""
    plan = _plan(state)
    previous = bool(plan.ai_enabled)
    plan.ai_enabled = bool(enabled)
    try:
        yield
    finally:
        plan.ai_enabled = previous
...
def _search_status(state: Any) -> tuple[str, str]:
    ...
    if gsc_required and not gsc_ready:
        return (
            "APTO COM LIMITAÇÕES",
            f"GSC solicitado/obrigatório ainda não está apto; a auditoria continuará e o fechamento ficará pendente: {gsc_detail}",
        )
    if queries and str(serp_status).upper() in blocked_values:
        return (
            "APTO COM LIMITAÇÕES",
            f"SERP solicitado ainda não está apto; a auditoria continuará e o requisito permanecerá pendente: {serp_detail}",
        )
    ...
    if str(gsc_status).upper() in blocked_values and gsc_required:
        return (
            "APTO COM LIMITAÇÕES",
            "GSC solicitado ainda não está apto; a auditoria continuará: " + gsc_detail,
        )
...
def catalog_status(state: Any, catalog: AuditCatalog) -> tuple[str, str]:
    ...
    if catalog.id == "CAT-08":
        ...
        if not ai_execution_enabled(state):
            return (
                "APTO COM LIMITAÇÕES",
                "a parte determinística pode executar; a IA necessária ao fechamento do CAT-08 não foi autorizada nesta auditoria",
            )
        ai_ready, ai_detail = ai_provider_readiness(state)
        if not ai_ready:
            return (
                "APTO COM LIMITAÇÕES",
                "a auditoria pode executar; CAT-08 permanecerá pendente até a IA estar disponível"
...
def plan_status(state: Any) -> tuple[str, str]:
    ...
    ai_ready, ai_detail = ai_provider_readiness(state)
    if required and not ai_execution_enabled(state):
        limitations.append(
            "fechamento integral por IA ficará pendente para "
            + ", ".join(item.id for item in required)
            + ": IA não autorizada nesta execução"
        )
    elif required and not ai_ready:
        limitations.append(
            "fechamento integral por IA ficará pendente para "
            + ", ".join(item.id for item in required)
            + (f": {ai_detail}" if ai_detail else ": IA indisponível")
        )
    elif optional and ai_execution_enabled(state) and not ai_ready:
        limitations.append(
            "IA opcional foi solicitada, mas está indisponível; a execução determinística continuará"
            + (f": {ai_detail}" if ai_detail else "")
        )
    if blockers:
...
def project_plan(state: Any) -> Iterator[None]:
    selected = _plan(state).selected
    names = (
        "web_performance", "search_queries", "search_ai_competitive",
        "synthetic_apdex", "apdex_experience",
    ...
    try:
        ...
        if not ai_execution_enabled(state):
            if hasattr(state, "search_ai_competitive"):
                state.search_ai_competitive = False
            if hasattr(state, "ai_provider"):

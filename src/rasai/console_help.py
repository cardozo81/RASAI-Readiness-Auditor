...
SPECIFIC_ENV_HELP: dict[str, tuple[str, str]] = {
    ...
    "RASAI_WEB_PERFORMANCE": ("Default para Web Performance externo.", COST_QUOTA + " quando true."),
    "RASAI_WEB_PERFORMANCE_MAX_PAGES": ("Teto de páginas do Web Performance.", COST_VOLUME),
    "RASAI_WEB_PERFORMANCE_TIMEOUT_SECONDS": ("Timeout por integração Web Performance.", "Não gera custo sozinho."),
    "RASAI_WEB_PERFORMANCE_FIELD_SOURCE": ("Fonte de field data: auto, pagespeed, crux ou none.", COST_QUOTA + " quando fonte externa é consultada."),
    "RASAI_LIGHTHOUSE_CATEGORIES": ("Categorias Lighthouse solicitadas pelo Web Performance externo.", "Pode alterar trabalho externo; não representa preço monetário por si só."),
    "RASAI_PAGESPEED_API_KEY": ("Credencial Google para PageSpeed Insights API.", COST_QUOTA + "; eventual billing pertence ao projeto Google Cloud."),
    "RASAI_CRUX_API_KEY": ("Credencial para consulta direta à Chrome UX Report API.", COST_QUOTA + "; eventual billing pertence ao projeto Google Cloud."),
    "RASAI_PLAYWRIGHT_CHROMIUM_EXECUTABLE": ("Caminho opcional para Chromium local.", COST_NONE),
}

...
_PERF_FIELDS: tuple[tuple[str, str, str], ...] = (
    ...
    ("fcp_lab_ms", "FCP de laboratório", "ms"),
    ("speed_index_lab_ms", "Speed Index", "ms"),
    ("lcp_lab_ms", "LCP de laboratório", "ms"),
    ("tbt_lab_ms", "Total Blocking Time", "ms"),
    ("cls_lab", "CLS de laboratório", ""),
    ("lcp_p75_ms", "LCP de campo (p75)", "ms"),
    ("inp_p75_ms", "INP de campo (p75)", "ms"),
    ("cls_p75", "CLS de campo (p75)", ""),
)
...
def summarize_findings(rows: tuple[dict[str, Any], ...]) -> FindingSummary:
    affected = {
        str(row.get("url") or row.get("page_id") or "").strip()
        for row in rows
        if str(row.get("url") or row.get("page_id") or "").strip()
    }

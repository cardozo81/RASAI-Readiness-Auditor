"""Project Web Performance externo web performance evidence into the static report site."""
...
def enrich_m21_report_site(*, audit_id: str, workspace: AuditWorkspace) -> Path:
    ...
    # Acessibilidade automatizada e diagnósticos Web consumes only the already-persisted Web Performance externo artifacts. It creates an
...
def _performance_page(data: dict[str, Any], report_dir: Path) -> str:
    ...
    if not enabled:
        notice = "<div class='notice'><strong>Coleta externa desabilitada.</strong> Esse é o comportamento padrão para evitar tráfego/quota externa não solicitado. SCORE-GEO-003 continua disponível normalmente.</div>"
    elif status in {"PARTIAL", "UNAVAILABLE"}:
        notice = "<div class='notice warn'><strong>Coleta externa incompleta.</strong> Falha, quota, ausência de amostra CrUX ou indisponibilidade do serviço não é defeito do website e não reduz SCORE-GEO-003.</div>"
    else:
        notice = "<div class='notice'><strong>Evidência externa coletada.</strong> Lighthouse/CrUX permanecem métricas independentes do índice heurístico RASAi.</div>"

    nav = _nav(report_dir)
    return f"""<!doctype html><html lang='pt-BR'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Web Performance - RASAi - Search & AI Readiness Auditor</title><link rel='stylesheet' href='css/site.css'></head><body>{nav}<main class='app-main'><header class='hero'><div class='eyebrow'>Web Performance externo + Acessibilidade automatizada e diagnósticos Web · domínio Web Performance</div><h1>Core Web Vitals, Lighthouse e diagnóstico técnico</h1><p class='lead'>Camada externa e não destrutiva. Mede laboratório via Lighthouse e experiência real via CrUX quando disponível. Performance permanece separada de GEO e de Acessibilidade.</p><div class='metric-grid'>{_metric('Coleta externa', 'Habilitada' if enabled else 'Desabilitada')}{_metric('Status', status)}{_metric('Páginas consideradas', pages)}{_metric('Contextos', f'{successes}/{contexts}')}{_metric('Limite configurado', limit_label)}{_metric('Field source', field_source)}</div></header>{notice}<section class='panel'><div class='kicker'>Separação metodológica</div><h2>O que estes números significam</h2><p class='intro'><strong>Core Web Vitals</strong> são métricas de experiência real agregada: LCP, INP e CLS no percentil 75. <strong>Lighthouse</strong> é teste de laboratório e seu score 0-100 usa metodologia própria do Chrome. Nenhum destes valores representa probabilidade de citação em IA e nenhum é incorporado automaticamente ao Overall Readiness do SCORE-GEO-003.</p><div class='notice'><strong>Thresholds oficiais de “boa” experiência:</strong> LCP ≤ 2,5 s · INP ≤ 200 ms · CLS ≤ 0,1. A avaliação conjunta fica incompleta quando uma das três métricas não possui dados suficientes.</div><p class='intro'>Categorias solicitadas ao Lighthouse: <code>{escape(categories)}</code>. Uma única chamada pode transportar evidências de mais de um domínio para reduzir consumo externo, mas Acessibilidade automatizada e diagnósticos Web as projeta em páginas e interpretações separadas.</p></section><section class='panel'><div class='kicker'>Resultados</div><h2>Por página e dispositivo</h2>{''.join(cards) if cards else '<p class="intro">Nenhuma observação externa persistida.</p>'}</section><section class='panel'><div class='kicker'>Operação externa</div><h2>Tentativas de coleta</h2><p class='intro'>Esta telemetria é de serviços de medição, não de IA. Chaves de API nunca são persistidas nem exibidas.</p><div class='table-wrap'><table><thead><tr><th>URL</th><th>Device</th><th>Serviço</th><th>Status</th><th>HTTP</th><th>Duração</th><th>Erro</th></tr></thead><tbody>{''.join(attempt_rows) if attempt_rows else '<tr><td colspan="7">Nenhuma chamada externa.</td></tr>'}</tbody></table></div></section><section class='panel'><div class='kicker'>Governança de consumo</div><h2>Custos, quota e IA</h2><p class='intro'>Web Performance externo/Acessibilidade automatizada e diagnósticos Web não criam chamadas a LLM. OpenAI, DeepSeek e MiMo continuam restritos às finalidades já configuradas em Análise semântica por IA, roteamento e telemetria/Sugestões e remediação de conteúdo por IA. O consumo desta camada é somente PageSpeed/CrUX e ocorre apenas quando explicitamente habilitado.</p><p class='intro'>PageSpeed pode ser chamado sem chave em baixo volume, mas uma chave é recomendada para automação frequente. CrUX API direta exige chave. O limite de páginas e o timeout são configuráveis para controlar quota e duração.</p></section><footer class='footer'>Web Performance é evidência complementar. SCORE-GEO-003 permanece índice heurístico independente e reprodutível.</footer></main></body></html>\n"""

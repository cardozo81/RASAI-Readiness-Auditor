...
RECOMMENDATION_IMPACT_CONTRACT = "benefit-degradation-v1"
...
class ImprovementConfig:
    ...
    def validate(self, env: Mapping[str, str] | None = None) -> "ImprovementConfig":
        ...
        if provider in {"", "none", "auto"}:
            # Provider readiness is an execution outcome, not structural invalidity.
            # The canonical orchestrator resolves AUTO when installed; standalone NONE/AUTO
            # remain safe and materialize an AI_NOT_CONFIGURED limitation without transport.
    ...
    def fingerprint(self) -> str:
        payload = {
            ...
            "recommendation_impact_contract": RECOMMENDATION_IMPACT_CONTRACT,
        }
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "current_degradation": {
                            ...
                            "maxLength": 3000,
                            "description": (
                                "Evidence-bound current degradation, limitation or risk caused by "
                                "the observed condition. Do not invent impact or causality."
                            ),
                        },
                        "expected_benefit": {
                            "type": "string",
                            ...
                            "maxLength": 3000,
                            "description": (
                                "Evidence-bound qualitative benefit expected if the recommendation "
                                "is applied. Do not promise ranking, traffic, revenue, conversion "
                                "or a numeric gain."
                            ),
                        },
                        "evidence_ids": {
                        ...
                        "verification": {"type": "string", "maxLength": 2500},
                    },
                },
            },
        },
...
def _validate_ai_payload(
    ...
    for raw in payload["recommendations"][:maximum]:
        ...
        if not set(evidence_ids).issubset(set(finding["evidence_ids"])):
            raise ValueError("evidence outside finding")

        confidence = float(raw.get("confidence"))
        effort = str(raw.get("effort") or "MEDIUM").upper()
        if not 0 <= confidence <= 1 or effort not in {"LOW", "MEDIUM", "HIGH"}:
            raise ValueError("invalid confidence/effort")
        ...
        if rationale:
            rationale_parts.append("Justificativa técnica: " + rationale)

        recommendations.append(
            {
                "recommendation_id": new_id("IIR"),
                "finding_id": finding_id,
                "domain": finding["domain"],
                "severity": finding["severity"],
                "title": str(raw.get("title") or finding["title"])[:500],
                ...
                "original_html": finding.get("original_html"),
                "suggested_html": (
                    str(raw["suggested_html"])[:8000]

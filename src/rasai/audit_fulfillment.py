...
def ensure_schema(workspace: AuditWorkspace | Path | str) -> None:
    with _connect(workspace) as connection:
        ...
        columns = {
            str(row["name"])
            for row in connection.execute("PRAGMA table_info(audit_reprocess_runs)").fetchall()
        }
        if "configuration" not in columns:
            connection.execute(
                "ALTER TABLE audit_reprocess_runs ADD COLUMN configuration TEXT NOT NULL DEFAULT '{}'"
            )
        connection.commit()
    ...
    *,
    ...
    if configuration is None:
        try:
            ...
            configuration = current_policy().as_configuration()
        except (ImportError, AttributeError):
            configuration = {}
    safe_configuration = _safe_mapping(configuration)
    with _connect(workspace) as connection:
        connection.execute(
            """INSERT INTO audit_reprocess_runs(
                reprocess_id,audit_id,contract_version,status,attempted_items,successful_items,
                remaining_items,source,configuration,started_at,completed_at,note
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                reprocess_id,
                audit_id,
...
def update_reprocess_run_configuration(
    workspace: AuditWorkspace | Path | str,
    reprocess_id: str,
    values: Mapping[str, Any],
) -> None:
    """Merge non-secret execution facts into one durable RPR record."""
    ensure_schema(workspace)
    safe_values = _safe_mapping(values)
    with _connect(workspace) as connection:
        row = connection.execute(
        ...
        try:
            current = json.loads(str(row["configuration"] or "{}"))
        except (TypeError, ValueError, json.JSONDecodeError):
            current = {}
        if not isinstance(current, dict):
            current = {}
...
def _banner(summary: FulfillmentSummary) -> str:
    if summary.is_final:
        ...
        text = (
            ...
            "Enquanto houver pendências, bloqueios ou validade temporal não satisfeita, o fechamento "
            "diagnóstico e o score final permanecem pendentes. Um AUD íntegro pode ser usado em relatório "
            "consolidado apenas como fonte não conclusiva, com suas limitações explicitadas; valores parciais "
            "não constituem score final."
        )

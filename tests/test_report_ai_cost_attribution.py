...
def _create_db(path: Path) -> None:
    ...
    try:
        ...
        connection.executemany(
            ...
            (
                ("AUD-1", "A1", "https://example.test/a", "MOBILE", "OPENAI", "model-a", "M18-SEMANTIC-22-v1", "SUCCESS", 100, 10, 20, 5, 120, 0.10, "USD"),
...
def test_cost_attribution_is_additive_visible_and_idempotent(tmp_path: Path) -> None:
    ...
    for filename, cost in expected_costs.items():
        ...
        assert "data-ai-cost-attribution='true'" in html
        assert cost in html

    references = (report_dir / "references.html").read_text(encoding="utf-8")
    assert "data-ai-cost-attribution='true'" in references
    ...
    assert "1.50000000 USD" in ai_usage
    assert "Detalhamento expansível por página" in ai_usage

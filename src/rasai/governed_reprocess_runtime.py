...
def _pending_item(workspace: Any, audit_id: str, component: str):
    return next(
        (
            ...
            and str(item.component) == component
            and item_executable(item)
        ),
        None,
    )
...
def _selected_required_pending(workspace: Any, audit_id: str) -> tuple[Any, ...]:
    return tuple(item for item in _required_pending(workspace, audit_id) if item_selected(item))
    ...
    *,
    ...
    try:
        ...
        for table, entity_type, id_field in specs:
            ...
            if "audit_id" not in columns or id_field not in columns:
                continue
            rows = [
                dict(row)
                for row in connection.execute(
...
def _archive_improvement_intelligence(workspace: Any, audit_id: str) -> int:
    return _archive_effective_rows(
        workspace,
        audit_id,
        component="IMPROVEMENT_INTELLIGENCE",
        specs=(
            ("improvement_intelligence_runs", "improvement_run", "audit_id"),
            ("improvement_intelligence_findings", "improvement_finding", "finding_id"),
            (
...
def _directed_analysis_context_changed(workspace: Any, audit_id: str) -> bool:
    from rasai.directed_analysis import build_strategic_context

    connection = sqlite3.connect(workspace.database)
    try:
        if not _table_exists(connection, "directed_analysis_runs"):
            return False
        row = connection.execute(
            """SELECT input_context_hash FROM directed_analysis_runs
    ...
    if row is None:
        return False
    _context, _actions, current_hash = build_strategic_context(
        audit_id=audit_id,
        workspace=workspace,
    )
...
def _registered_ai_purposes(
    ...
    """Return registered AI purposes authorized by this RPR.

    Provider execution is never globally blocked by an unrelated pending integration;
    each registered AI hook evaluates its own evidence/dependency contract.
    """
...
def _outcomes_used_ai(outcomes: Mapping[str, Mapping[str, Any]]) -> bool:
    """Return whether governed AI reached an effective provider execution attempt."""
    non_usage_statuses = {
        "DISABLED",
        "NOT_CONFIGURED",
        "NOT_ELIGIBLE",
    ...
    for outcome in outcomes.values():
        ...
        provider = str(outcome.get("provider") or "").strip().casefold()
        if status in non_usage_statuses or reason in non_usage_reasons:
            continue
        if provider in {"none", ""} and status in {"COMPLETE_WITH_LIMITATIONS", "ERROR"}:
            continue
        return True
    return False
    ...
    *,
    ...
    if str(summary.processing_status).upper() == "COMPLETE":
        if _directed_analysis_context_changed(workspace, audit_id):
            _archive_directed_analysis(workspace, audit_id)
        directed_finalizer(audit_id=audit_id, workspace=workspace)
    # The catalog projection is intentionally deferred until the owner of the
    # reprocess run closes audit_reprocess_runs. Rendering here would seal a snapshot
    # while the RPR is still RUNNING and immediately make its source fingerprint stale.
    project_report_validity(audit_id=audit_id, workspace=workspace)

    # use_ai is authorization; ai_used records whether an AI provider execution was
    # effectively attempted. NOT_CONFIGURED/NOT_REQUESTED/SKIPPED states therefore do
            ...
            *,
            ...
            try:
                with _defer_reprocess_finish(
                    ...
                    effective_ai_used = bool(
                        getattr(result, "ai_used", False) or governed_ai_used
                    )
                    try:
                        result = replace(
                            result,
                    ...
                    try_append_operational_event(
                        workspace,
                        "AUDIT_REPROCESS_POLICY_FINALIZED",
                        audit_id=audit_id,
                        reprocess_id=reprocess_id,
                        selected_items=selected_count,
                        unselected_items=unselected_count,
                        use_ai=effective_policy.use_ai,
                        ai_used=effective_ai_used,
                        ai_provider=effective_policy.ai_provider,
                        ai_model=effective_policy.ai_model,
                        ai_reasoning=effective_policy.ai_reasoning,
                    )
                    return result
            except Exception:
                if owns_reprocess:

...
Uma integração não selecionada não pode bloquear a auditoria. `CONFIGURAR` continua válido em superfícies técnicas de configuração/capacidade. Indisponibilidade operacional de integração/IA selecionada é projetada como `APTO COM LIMITAÇÕES` quando o restante da auditoria pode prosseguir; `BLOQUEADO` fica reservado a dependências estruturais reais que impedem a própria execução daquela etapa.
...
### Hierarquia visual da variável técnica

O nome amigável da configuração é a referência principal para o operador. A linha secundária:

```text
Variável técnica: RASAI_...
```

é renderizada em **cinza** no terminal para indicar que se trata de referência técnica, não do rótulo operacional principal. A regra vale para listagens, busca de configurações, detalhe de variável, revisão de alteração e dependências de integrações.

### Referência técnica secundária

Quando uma configuração possui identificador técnico, o console mantém a descrição humanizada como informação principal e mostra a referência em uma linha secundária:

```text
Descrição humanizada ...
Variável técnica: RASAI_EXAMPLE_SETTING
```

A linha `Variável técnica:` usa sempre fonte cinza no terminal. Esse tratamento é visual e não altera o nome real da variável nem sua persistência.

### Entrada de secrets

A edição de qualquer configuração sensível usa o leitor mascarado canônico do console. O prompt mostra somente `*` enquanto o operador digita ou cola o valor.

A máscara é exclusivamente visual. O editor entrega o valor real ao validador e ao mecanismo de sessão/Windows User; os caracteres `*` nunca substituem o segredo armazenado.

Em terminal interativo suportado, a quantidade de asteriscos acompanha os caracteres recebidos. Se o terminal não permitir controle seguro de eco, o console usa entrada totalmente oculta, nunca texto em claro.
...
Quando só existem consumidores opcionais (`CAT-03`/`CAT-09`), o plano usa **Executar sem IA (recomendado)** por padrão. Se a IA principal estiver configurada/apta, o operador pode alternar para **Executar com IA**. Em `CAT-08`, a IA é requisito do fechamento integral, não pré-condição para executar a parte determinística: `ai_provider=none` é aceito e deixa a análise profunda pendente. Os demais catálogos não ativam IA por si só. Nenhum enriquecimento advisory pode alterar evidência/scoring determinístico.

A escolha local `U` altera apenas o plano da próxima auditoria. Provider, modelo, credencial, AUTO e política global continuam pertencendo ao mesmo fluxo canônico de Inteligência Artificial. A tela financeira oferece apenas um atalho `A. Ajustar configuração de IA` para abrir esse fluxo, sem duplicar implementação ou armazenamento.

Quando houver consumo, o fluxo canônico de estimativa/aceite executa antes do AUD e separa claramente: `C` = executar esta AUD com IA autorizada; `N` = executar esta AUD sem IA; `A` = ajustar IA e recalcular; `V` = voltar sem executar.
...
A última tela não é o local para descobrir dependências básicas. Cada catálogo recalcula readiness enquanto o operador configura. `R. Executar auditoria` permanece bloqueado por dependências estruturais reais; indisponibilidade operacional recuperável de IA/API é apresentada como limitação de fechamento e não impede a execução quando o pipeline determinístico continua válido.

...
def test_device_page_exposes_evidence_role_not_duplicate_score() -> None:
    ...
    assert "Evidências SearchGEO por dispositivo" in html
...
def test_searchgeo_page_has_explicit_internal_methodological_nature() -> None:
    html = enrich_indicator_provenance_html(_shell(), page_name="searchgeo.html")
    assert "Heurística SearchGEO evidence-based" in html
...
def test_external_metric_pages_are_not_presented_as_searchgeo_score() -> None:
    ...
    assert "sem convertê-los em SGRI-001" in html
...
def test_references_panel_contains_source_logic_and_internal_boundary() -> None:
    ...
    assert "SearchGEO Readiness Index (SGRI-001)" in html
    ...
    assert "não homologa automaticamente o SGRI-001" in html
...
def test_inventory_has_explicit_classification_and_no_fake_external_source_for_internal_index() -> None:
    assert INDICATORS
    score = next(item for item in INDICATORS if item.indicator.startswith("SearchGEO Readiness Index"))
